...
module vfs_class
   ...
   type :: vfs
      ...
      type(cclabel)                             :: ccl_recon     !< CCL structures
   ...
   contains
      ...
      procedure :: build_r2p_cylinder                     !< R2P+Cylinder reconstruction of the interface
...
contains
   ...
   subroutine initialize(this,cfg,reconstruction_method,transport_method,name)
      ...
      case (r2p_cylinder)
         ...
         allocate(this%local_struct_type_recon(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%local_struct_type_recon=0
         allocate(this%liquid_gas_flip(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%liquid_gas_flip=0
         ...
         allocate(this%lig_edge_sensor(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%lig_edge_sensor=0
         allocate(this%recon_type(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%recon_type=0
         ! By default, use thin structure removal
         this%thin_thld_min=1.0e-4_WP !< This removes any thin structure with thickness below dx/1000
         ! By default, use flotsam removal
         this%flotsam_thld=1.0e-3_WP  !< This considers any separated structure around dx/10 and below as bogus
         ...
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               if (this%recon_type(i,j,k).eq.1) then
                  ...
                  do kk=k-2,k+2
                     do jj=j-2,j+2
                        do ii=i-2,i+2
                           ...
                           if (this%ccl_recon%id(ii,jj,kk).eq.this%ccl_recon%id(i,j,k)) then
                  ...
                  call reconstructCylinder3D(nh_cyl,this%liquid_gas_flip(i,j,k),this%liquid_gas_interface(i,j,k))
               else if (this%recon_type(i,j,k).eq.3) then
                  ! Prepare R2P data
                  ind=0; call emptyNeighborhood(nh_r2p)
                  do kk=k-1,k+1; do jj=j-1,j+1; do ii=i-1,i+1
                     call addMember(nh_r2p,neighborhood_cells(ind),separated_volume_moments(ind))
         ...
         do k=this%cfg%kmin_,this%cfg%kmax_
            do j=this%cfg%jmin_,this%cfg%jmax_
               do i=this%cfg%imin_,this%cfg%imax_
                  ...
                  do kk = k-nneigh_thickness,k+nneigh_thickness
                     do jj = j-nneigh_thickness,j+nneigh_thickness
                        do ii = i-nneigh_thickness,i+nneigh_thickness
                           tmpvol1 = tmpvol1 + this%VF(ii,jj,kk)
                           tmpvol2 = tmpvol2 + (1.0_WP-this%VF(ii,jj,kk))
                           if (this%recon_type(ii,jj,kk).eq.2.or.this%recon_type(ii,jj,kk).eq.3.or.this%recon_type(ii,jj,kk).eq.0) then
                              tmparea = tmparea + this%SD(ii,jj,kk)*this%cfg%vol(i,j,k)
                           else
                              tmparea = tmparea + this%SD(ii,jj,kk)*this%cfg%vol(i,j,k)*(2.0/sqrt(pi))
                           end if
                        end do
                     end do
                  end do
                  ...
                  ! Calculate thickness
                  if (this%VF(i,j,k).le.VFlo.and.this%liquid_gas_flip(i,j,k).eq.1) then
                     this%local_thickness_recon(i,j,k) = 0.0_WP
                  else if (this%VF(i,j,k).ge.VFhi.and.this%liquid_gas_flip(i,j,k).eq.-1) then
                     this%local_thickness_recon(i,j,k) = 0.0_WP
                     ...
                     do kk = k-nneigh_moi,k+nneigh_moi
                        do jj = j-nneigh_moi,j+nneigh_moi
                           do ii = i-nneigh_moi,i+nneigh_moi
                              if (this%liquid_gas_flip(i,j,k).eq.1) then
                                 tmpvol = tmpvol + this%VF(ii,jj,kk)*this%cfg%vol(i,j,k)
                                 tmpxvol = tmpxvol + this%Lbary(:,ii,jj,kk)*this%VF(ii,jj,kk)*this%cfg%vol(i,j,k)
                              else
                                 tmpvol = tmpvol + (1.0_WP-this%VF(ii,jj,kk))*this%cfg%vol(i,j,k)
                                 tmpxvol = tmpxvol + this%Gbary(:,ii,jj,kk)*(1.0_WP-this%VF(ii,jj,kk))*this%cfg%vol(i,j,k)
                              end if
                           end do
                        end do
                     end do
                     ...
                     do kk = k-nneigh_moi,k+nneigh_moi
                        do jj = j-nneigh_moi,j+nneigh_moi
                           do ii = i-nneigh_moi,i+nneigh_moi
                              ...
                              if (this%liquid_gas_flip(i,j,k).eq.1) then
                                 tmpL = this%Lbary(:,ii,jj,kk) - tmpxvol
                                 fluid_vol = this%cfg%vol(ii,jj,kk)*this%VF(ii,jj,kk)
                                 ...
                                 tmpL = this%Gbary(:,ii,jj,kk) - tmpxvol
                                 fluid_vol = this%cfg%vol(ii,jj,kk)*(1.0_WP-this%VF(ii,jj,kk))
                                 A(1,1)=A(1,1)+fluid_vol*(tmpL(2)**2+tmpL(3)**2)
                                 A(2,2)=A(2,2)+fluid_vol*(tmpL(1)**2+tmpL(3)**2)
                                 A(3,3)=A(3,3)+fluid_vol*(tmpL(1)**2+tmpL(2)**2)
                                 A(1,2)=A(1,2)-fluid_vol*tmpL(1)*tmpL(2)
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               do kk=k-1,k+1; do jj=j-1,j+1; do ii=i-1,i+1
                  new_size=new_size+getSize(this%triangle_moments_storage(ii,jj,kk))
               end do; end do; end do
               ! Allocate local storage
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               if (this%thickness(i,j,k).gt.this%thin_thld_max*this%cfg%meshsize(i,j,k)) this%film_edge_sensor(i,j,k)=0.0_WP
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               do kk=k-1,k+1; do jj=j-1,j+1; do ii=i-1,i+1
                  surface_area  =  surface_area+this%SD(ii,jj,kk)*this%cfg%vol(ii,jj,kk)
                  s_tmp  (i,j,k)=s_tmp  (i,j,k)+this%SD(ii,jj,kk)*this%cfg%vol(ii,jj,kk)*this%film_edge_sensor  (ii,jj,kk)
         ...
         do j=this%cfg%jmin_,this%cfg%jmax_
            main_loop: do i=this%cfg%imin_,this%cfg%imax_
               ...
               do kk=k-1,k+1
                  do jj=j-1,j+1
                     do ii=i-1,i+1
                        if (this%mask(ii,jj,kk).ne.0.or.this%lig_edge_sensor(i,j,k).eq.2.0_WP) then 
                        ...
                        if (this%ccl_recon%id(ii,jj,kk).ne.this%ccl_recon%id(i,j,k).or.(ii.eq.i.and.jj.eq.j.and.kk.eq.k)) then
                           ...
                           moments(0,di,dj,dk)=this%VF(ii,jj,kk)
                           moments(1,di,dj,dk)=(this%Lbary(1,ii,jj,kk)-this%cfg%xm(ii))/this%cfg%dx(ii)
                           moments(2,di,dj,dk)=(this%Lbary(2,ii,jj,kk)-this%cfg%ym(jj))/this%cfg%dy(jj)
                           moments(3,di,dj,dk)=(this%Lbary(3,ii,jj,kk)-this%cfg%zm(kk))/this%cfg%dz(kk)
                           moments(4,di,dj,dk)=(this%Gbary(1,ii,jj,kk)-this%cfg%xm(ii))/this%cfg%dx(ii)
                           moments(5,di,dj,dk)=(this%Gbary(2,ii,jj,kk)-this%cfg%ym(jj))/this%cfg%dy(jj)
                           moments(6,di,dj,dk)=(this%Gbary(3,ii,jj,kk)-this%cfg%zm(kk))/this%cfg%dz(kk)
               ...
               do kk=k-1,k+1
                  do jj=j-1,j+1
                     do ii=i-1,i+1
                        ...
                        dj = jj - j
                        dk = kk - k
                        if (this%mask(ii,jj,kk).ne.0.or.this%lig_edge_sensor(i,j,k).eq.2) then 
                           this%lig_edge_sensor(i,j,k)=2.0_WP
                           cycle main_loop
                        end if
                        if (this%ccl_recon%id(ii,jj,kk).ne.this%ccl_recon%id(i,j,k).or.(ii.eq.i.and.jj.eq.j.and.kk.eq.k)) then
