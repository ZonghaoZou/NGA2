 ...
 !> Initialization of dispersion simulation
   ...
   subroutine simulation_init(this,cfga2d)
      ...
      allocate_work_arrays: block
         allocate(this%gradU(1:3,1:3,this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_));this%gradU=0.0_WP
         allocate(this%resU(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_));this%resU=0.0_WP
         allocate(this%resV(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_));this%resV=0.0_WP
         allocate(this%resW(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_));this%resW=0.0_WP
     ...
     handle_restart: block
         ...
         if (this%restarted) then
            ...
            ! Zero velocity at IB walls to prevent NaN divergence on restart
            zero_ib_walls: block
               integer :: i,j,k
               do k=this%cfg%kmin_,this%cfg%kmax_
                  do j=this%cfg%jmin_,this%cfg%jmax_
                     do i=this%cfg%imin_,this%cfg%imax_
                        if (this%fs%umask(i,j,k).eq.1) this%fs%U(i,j,k)=0.0_WP
                        if (this%fs%vmask(i,j,k).eq.1) this%fs%V(i,j,k)=0.0_WP
                        if (this%fs%wmask(i,j,k).eq.1) this%fs%W(i,j,k)=0.0_WP
                     end do
                  end do
               end do
               call this%cfg%sync(this%fs%U)
               call this%cfg%sync(this%fs%V)
               call this%cfg%sync(this%fs%W)
            end block zero_ib_walls
            ! Apply all other boundary conditions
            call this%fs%apply_bcond(this%time%t,this%time%dt)
            ! Compute MFR through all boundary conditions
            call this%fs%get_mfr()
