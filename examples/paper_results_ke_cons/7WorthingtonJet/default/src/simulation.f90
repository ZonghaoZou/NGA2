...
contains
   ...
   subroutine simulation_init
      ...
      !    smesh=surfmesh(nvar=4,name='plic')
      !    smesh%varname(1)='curvature'
      !    smesh%varname(2)='we_local'
      !    smesh%varname(3)='Re_local'
      !    smesh%varname(4)='Fr_local'
      !    call vf%update_surfmesh(smesh)
      !    np=0
      ...
      !                   smesh%var(2,np)=fs%rho_l*(Ui(i,j,k)**2+Vi(i,j,k)**2+Wi(i,j,k)**2)*cfg%meshsize(i,j,k)/fs%sigma
      !                   smesh%var(3,np)=fs%rho_l*sqrt(Ui(i,j,k)**2+Vi(i,j,k)**2+Wi(i,j,k)**2)*cfg%meshsize(i,j,k)/fs%visc_l
      !                   smesh%var(4,np)=(Ui(i,j,k)**2+Vi(i,j,k)**2+Wi(i,j,k)**2)/(9.81_WP*cfg%meshsize(i,j,k))
      !                end if
      !             end do
      !          end do 
      !       end do 
