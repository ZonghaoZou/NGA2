...
module tpcons_class
   ...
   type :: tpcons
      ...
      ! Face densities for Rhie-chow correction
      real(WP), dimension(:,:,:), allocatable :: rho_x
      real(WP), dimension(:,:,:), allocatable :: rho_y
      real(WP), dimension(:,:,:), allocatable :: rho_Z
      ...
      real(WP), dimension(:,:,:), allocatable :: Uf       !< U face velocity array
      real(WP), dimension(:,:,:), allocatable :: Vf       !< V face velocity array
      real(WP), dimension(:,:,:), allocatable :: Wf       !< W face velocity array
      ...
      ! Metrics for the spatial collocated grid
      real(WP), dimension(:,:,:,:), allocatable :: itp_x,itp_y,itp_z !< Interpolation            cell center to cell face
      real(WP), dimension(:,:,:,:), allocatable :: grd_x,grd_y,grd_z !< Gradient operator from   cell center to cell face
      real(WP), dimension(:,:,:,:), allocatable :: div_x,div_y,div_z !< Divergence operator from cell face to cell center
   ...
   contains
      ...
      procedure :: get_pgrad_wide
      procedure :: construction_correction
   end type tpcons
...
contains
   ...
   subroutine initialize(this,cfg,name)
      ...
      allocate(this%rho_x(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%rho_x=0.0_WP
      allocate(this%rho_y(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%rho_y=0.0_WP
      allocate(this%rho_z(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%rho_z=0.0_WP
   ...
   subroutine init_metrics(this)
      ...
      allocate(this%grd_x(-1: 0,this%cfg%imino_+1:this%cfg%imaxo_,this%cfg%jmino_  :this%cfg%jmaxo_,this%cfg%kmino_  :this%cfg%kmaxo_)) !< Face-centered (x)
      allocate(this%grd_y(-1: 0,this%cfg%imino_  :this%cfg%imaxo_,this%cfg%jmino_+1:this%cfg%jmaxo_,this%cfg%kmino_  :this%cfg%kmaxo_)) !< Face-centered (y)
      allocate(this%grd_z(-1: 0,this%cfg%imino_  :this%cfg%imaxo_,this%cfg%jmino_  :this%cfg%jmaxo_,this%cfg%kmino_+1:this%cfg%kmaxo_)) !< Face-centered (z)

      ! Create gradient operator perpendicular to cell face [x ,ym,zm]
      do k=this%cfg%kmino_  ,this%cfg%kmaxo_
         do j=this%cfg%jmino_  ,this%cfg%jmaxo_
            do i=this%cfg%imino_+1,this%cfg%imaxo_
               this%grd_x(:,i,j,k)=this%cfg%dxmi(i)*[-1.0_WP,+1.0_WP] !< FV gradient from [xm,ym,zm]
            end do
         end do
      end do
      ! Create gradient operator perpendicular to cell face [xm,y ,zm]
      do k=this%cfg%kmino_  ,this%cfg%kmaxo_
         do j=this%cfg%jmino_+1,this%cfg%jmaxo_
            do i=this%cfg%imino_  ,this%cfg%imaxo_
               this%grd_y(:,i,j,k)=this%cfg%dymi(j)*[-1.0_WP,+1.0_WP] !< FV gradient from [xm,ym,zm]
            end do
         end do
      end do
      ! Create gradient operator perpendicular to cell face [xm,ym,z ]
      do k=this%cfg%kmino_+1,this%cfg%kmaxo_
         do j=this%cfg%jmino_  ,this%cfg%jmaxo_
            do i=this%cfg%imino_  ,this%cfg%imaxo_
               this%grd_z(:,i,j,k)=this%cfg%dzmi(k)*[-1.0_WP,+1.0_WP] !< FV gradient from [xm,ym,zm]
            end do
         end do
      end do


      allocate(this%div_x( 0:+1,this%cfg%imino_  :this%cfg%imaxo_,this%cfg%jmino_  :this%cfg%jmaxo_,this%cfg%kmino_  :this%cfg%kmaxo_)) !< Cell-centered
      allocate(this%div_y( 0:+1,this%cfg%imino_  :this%cfg%imaxo_,this%cfg%jmino_  :this%cfg%jmaxo_,this%cfg%kmino_  :this%cfg%kmaxo_)) !< Cell-centered
      allocate(this%div_z( 0:+1,this%cfg%imino_  :this%cfg%imaxo_,this%cfg%jmino_  :this%cfg%jmaxo_,this%cfg%kmino_  :this%cfg%kmaxo_)) !< Cell-centered
   ...
   subroutine adjust_metrics(this)
      ...
      ! This was commented out in the tpcons class, but included for interpolating velocities to the face
      do k=this%cfg%kmin_,this%cfg%kmax_+1
        do j=this%cfg%jmin_,this%cfg%jmax_+1
           do i=this%cfg%imin_,this%cfg%imax_+1
              ! Linear interpolation in x
              if (this%cfg%VF(i,j,k).eq.0.0_WP.and.this%cfg%VF(i-1,j,k).gt.0.0_WP) this%itp_x(:,i,j,k)=[1.0_WP,0.0_WP]
              if (this%cfg%VF(i,j,k).gt.0.0_WP.and.this%cfg%VF(i-1,j,k).eq.0.0_WP) this%itp_x(:,i,j,k)=[0.0_WP,1.0_WP]
              ! Linear interpolation in y
              if (this%cfg%VF(i,j,k).eq.0.0_WP.and.this%cfg%VF(i,j-1,k).gt.0.0_WP) this%itp_y(:,i,j,k)=[1.0_WP,0.0_WP]
              if (this%cfg%VF(i,j,k).gt.0.0_WP.and.this%cfg%VF(i,j-1,k).eq.0.0_WP) this%itp_y(:,i,j,k)=[0.0_WP,1.0_WP]
              ! Linear interpolation in z
              if (this%cfg%VF(i,j,k).eq.0.0_WP.and.this%cfg%VF(i,j,k-1).gt.0.0_WP) this%itp_z(:,i,j,k)=[1.0_WP,0.0_WP]
              if (this%cfg%VF(i,j,k).gt.0.0_WP.and.this%cfg%VF(i,j,k-1).eq.0.0_WP) this%itp_z(:,i,j,k)=[0.0_WP,1.0_WP]
           end do
        end do
      end do
   ...
   subroutine get_dmomdt(this,drhoUdt,drhoVdt,drhoWdt)
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_+1
         do j=this%cfg%jmin_,this%cfg%jmax_+1
            do i=this%cfg%imin_,this%cfg%imax_+1
               FX(i,j,k)=-this%rhoU(i,j,k)*sum(this%itp_x(:,i,j,k)*this%U(i-1:i,j,k)) !& 
               ! &         -sum(this%itp_x(:,i,j,k)*this%P(i-1:i,j,k))
               FY(i,j,k)=-this%rhoV(i,j,k)*sum(this%itp_y(:,i,j,k)*this%U(i,j-1:j,k)) 
               FZ(i,j,k)=-this%rhoW(i,j,k)*sum(this%itp_z(:,i,j,k)*this%U(i,j,k-1:k))
            end do 
         end do 
      end do
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               drhoUdt(i,j,k)=sum(this%div_x(:,i,j,k)*FX(i:i+1,j,k))+&
               &              sum(this%div_y(:,i,j,k)*FY(i,j:j+1,k))+&
               &              sum(this%div_z(:,i,j,k)*FZ(i,j,k:k+1))!+this%Pjx(i,j,k)
            end do
         end do
      end do
      ! Sync it
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_+1
         do j=this%cfg%jmin_,this%cfg%jmax_+1
            do i=this%cfg%imin_,this%cfg%imax_+1
               FX(i,j,k)=-this%rhoU(i,j,k)*sum(this%itp_x(:,i,j,k)*this%V(i-1:i,j,k))
               FY(i,j,k)=-this%rhoV(i,j,k)*sum(this%itp_y(:,i,j,k)*this%V(i,j-1:j,k)) !& 
               ! &         -sum(this%itp_y(:,i,j,k)*this%P(i,j-1:j,k)) 
               FZ(i,j,k)=-this%rhoW(i,j,k)*sum(this%itp_z(:,i,j,k)*this%V(i,j,k-1:k))
            end do 
         end do 
      end do
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               drhoVdt(i,j,k)=sum(this%div_x(:,i,j,k)*FX(i:i+1,j,k))+&
               &              sum(this%div_y(:,i,j,k)*FY(i,j:j+1,k))+&
               &              sum(this%div_z(:,i,j,k)*FZ(i,j,k:k+1))!+this%Pjy(i,j,k)
            end do
         end do
      end do
      ! Sync it
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_+1
         do j=this%cfg%jmin_,this%cfg%jmax_+1
            do i=this%cfg%imin_,this%cfg%imax_+1
               FX(i,j,k)=-this%rhoU(i,j,k)*sum(this%itp_x(:,i,j,k)*this%W(i-1:i,j,k))
               FY(i,j,k)=-this%rhoV(i,j,k)*sum(this%itp_y(:,i,j,k)*this%W(i,j-1:j,k)) 
               FZ(i,j,k)=-this%rhoW(i,j,k)*sum(this%itp_z(:,i,j,k)*this%W(i,j,k-1:k)) !& 
               ! &         -sum(this%itp_z(:,i,j,k)*this%P(i,j,k-1:k)) 
            end do 
         end do 
      end do
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               drhoWdt(i,j,k)=sum(this%div_x(:,i,j,k)*FX(i:i+1,j,k))+&
               &              sum(this%div_y(:,i,j,k)*FY(i,j:j+1,k))+&
               &              sum(this%div_z(:,i,j,k)*FZ(i,j,k:k+1))!+this%Pjz(i,j,k)
            end do
         end do
      end do
      ! Sync it
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               do s1=0,1
                  do s2=-1,0
                     this%psolv%opr(this%psolv%stmap(s1+s2,0,0),i,j,k)=this%psolv%opr(this%psolv%stmap(s1+s2,0,0),i,j,k)+this%div_x(s1,i,j,k)*this%grd_x(s2,i+s1,j,k)/this%rho_x(i+s1,j,k)
                     this%psolv%opr(this%psolv%stmap(0,s1+s2,0),i,j,k)=this%psolv%opr(this%psolv%stmap(0,s1+s2,0),i,j,k)+this%div_y(s1,i,j,k)*this%grd_y(s2,i,j+s1,k)/this%rho_y(i,j+s1,k)
                     this%psolv%opr(this%psolv%stmap(0,0,s1+s2),i,j,k)=this%psolv%opr(this%psolv%stmap(0,0,s1+s2),i,j,k)+this%div_z(s1,i,j,k)*this%grd_z(s2,i,j,k+s1)/this%rho_z(i,j,k+s1)
                  end do
               end do
   ...
   !> Calculate the velocity divergence based on Rhie-Chow correction
   !> Use u_f = \bar{u_f} + 
   subroutine construction_correction(this,dt)
      implicit none
      class(tpcons), intent(inout) :: this
      real(WP), intent(in) :: dt
      real(WP), dimension(:,:,:), allocatable :: P_x,P_y,P_z
      integer :: i,j,k

      ! Allocate flux arrays
      allocate(P_x(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_));P_x=0.0_WP
      allocate(P_y(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_));P_y=0.0_WP
      allocate(P_z(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_));P_z=0.0_WP

      this%Uf=0.0_WP;this%Vf=0.0_WP;this%Wf=0.0_WP
      ! Get face interpolated face velocities and densities from centered velocities and densities
      get_interpolated_quantities: block
         do k=this%cfg%kmino_  ,this%cfg%kmaxo_; do j=this%cfg%jmino_  ,this%cfg%jmaxo_; do i=this%cfg%imino_+1,this%cfg%imaxo_
            this%Uf(i,j,k)   =sum(this%itp_x(:,i,j,k)*this%U  (i-1:i,j,k))
            P_x(i,j,k)       =sum(this%itp_x(:,i,j,k)*this%P  (i-1:i,j,k))
            this%rho_x(i,j,k)=sum(this%itp_x(:,i,j,k)*this%rho(i-1:i,j,k))
         end do; end do; end do
         do k=this%cfg%kmino_  ,this%cfg%kmaxo_; do j=this%cfg%jmino_+1,this%cfg%jmaxo_; do i=this%cfg%imino_  ,this%cfg%imaxo_
            this%Vf(i,j,k)   =sum(this%itp_y(:,i,j,k)*this%V  (i,j-1:j,k))
            P_y(i,j,k)       =sum(this%itp_y(:,i,j,k)*this%P  (i,j-1:j,k))
            this%rho_y(i,j,k)=sum(this%itp_y(:,i,j,k)*this%rho(i,j-1:j,k))
         end do; end do; end do
         do k=this%cfg%kmino_+1,this%cfg%kmaxo_; do j=this%cfg%jmino_  ,this%cfg%jmaxo_; do i=this%cfg%imino_  ,this%cfg%imaxo_
            this%Wf(i,j,k)   =sum(this%itp_z(:,i,j,k)*this%W  (i,j,k-1:k))
            P_z(i,j,k)       =sum(this%itp_z(:,i,j,k)*this%P  (i,j,k-1:k))
            this%rho_z(i,j,k)=sum(this%itp_z(:,i,j,k)*this%rho(i,j,k-1:k))
         end do; end do; end do
         
         if (.not.this%cfg%xper.and.this%cfg%iproc.eq.1) then
            this%Uf(this%cfg%imino,:,:)=this%U(this%cfg%imino,:,:)
            P_x(this%cfg%imino,:,:)=this%P(this%cfg%imino,:,:)
            this%rho_x(this%cfg%imino,:,:)=this%rho(this%cfg%imino,:,:)
         end if
         if (.not.this%cfg%yper.and.this%cfg%jproc.eq.1) then
            this%Vf(:,this%cfg%jmino,:)=this%V(:,this%cfg%jmino,:)
            P_y(:,this%cfg%jmino,:)=this%P(:,this%cfg%jmino,:)
            this%rho_y(:,this%cfg%jmino,:)=this%rho(:,this%cfg%jmino,:)
         end if
         if (.not.this%cfg%zper.and.this%cfg%kproc.eq.1) then
            this%Wf(:,:,this%cfg%kmino)=this%W(:,:,this%cfg%kmino)
            P_z(:,:,this%cfg%kmino)=this%P(:,:,this%cfg%kmino)
            this%rho_z(:,:,this%cfg%kmino)=this%rho(:,:,this%cfg%kmino)
         end if
         
         call this%cfg%sync(this%Uf); call this%cfg%sync(this%rho_x); call this%cfg%sync(P_x)
         call this%cfg%sync(this%Vf); call this%cfg%sync(this%rho_y); call this%cfg%sync(P_y)
         call this%cfg%sync(this%Wf); call this%cfg%sync(this%rho_z); call this%cfg%sync(P_z)
      end block get_interpolated_quantities


      get_corrected_velocities: block
         !> The first pressure gradient term invovles taking the gradient of interpolated face pressure divided by the cell center density and interpolate to the cell face
         !> The second pressure gradient term takes the gradient of the cell centered pressure such that it locates at the cell face
         do k=this%cfg%kmin_  ,this%cfg%kmax_+1; do j=this%cfg%jmin_  ,this%cfg%jmax_+1; do i=this%cfg%imin_,this%cfg%imax_+1
            this%Uf(i,j,k)=this%Uf(i,j,k)+dt*(sum(this%itp_x(:,i,j,k)*[sum(this%div_x(:,i-1,j,k)*P_x(i-1:i,j,k))/this%rho(i-1,j,k),sum(this%div_x(:,i,j,k)*P_x(i:i+1,j,k))/this%rho(i,j,k)]) &
                     -sum(this%grd_x(:,i,j,k)*this%P(i-1:i,j,k))/this%rho_x(i,j,k))
            this%Vf(i,j,k)=this%Vf(i,j,k)+dt*(sum(this%itp_y(:,i,j,k)*[sum(this%div_y(:,i,j-1,k)*P_y(i,j-1:j,k))/this%rho(i,j-1,k),sum(this%div_y(:,i,j,k)*P_y(i,j:j+1,k))/this%rho(i,j,k)]) &
                     -sum(this%grd_y(:,i,j,k)*this%P(i,j-1:j,k))/this%rho_y(i,j,k))
            this%Wf(i,j,k)=this%Wf(i,j,k)+dt*(sum(this%itp_z(:,i,j,k)*[sum(this%div_z(:,i,j,k-1)*P_z(i,j,k-1:k))/this%rho(i,j,k-1),sum(this%div_z(:,i,j,k)*P_z(i,j,k:k+1))/this%rho(i,j,k)]) &
                     -sum(this%grd_z(:,i,j,k)*this%P(i,j,k-1:k))/this%rho_z(i,j,k))
         end do; end do; end do
         call this%cfg%sync(this%Uf)
         call this%cfg%sync(this%Vf)
         call this%cfg%sync(this%Wf)
      end block get_corrected_velocities

   end subroutine construction_correction
   

   ! Get divergence based on face-centered velocities
   subroutine get_div(this,src)
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_; do j=this%cfg%jmin_,this%cfg%jmax_; do i=this%cfg%imin_,this%cfg%imax_
         this%div(i,j,k)=sum(this%div_x(:,i,j,k)*this%Uf(i:i+1,j,k))+&
         &               sum(this%div_y(:,i,j,k)*this%Vf(i,j:j+1,k))+&
         &               sum(this%div_z(:,i,j,k)*this%Wf(i,j,k:k+1))
      end do; end do; end do
   ...
   subroutine add_surface_tension_jump(this,dt,div,vf,contact_model)
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               do s1=0,1
                  div(i,j,k)=div(i,j,k)+dt*this%divp_x(s1,i,j,k)*this%dPjx(i+s1,j,k)/this%rho_x(i+s1,j,k)
                  div(i,j,k)=div(i,j,k)+dt*this%divp_y(s1,i,j,k)*this%dPjy(i,j+s1,k)/this%rho_y(i,j+s1,k)
                  div(i,j,k)=div(i,j,k)+dt*this%divp_z(s1,i,j,k)*this%dPjz(i,j,k+s1)/this%rho_z(i,j,k+s1)
               end do
            end do
         end do
      end do
   ...
   subroutine get_pgrad(this,P,Pgradx,Pgrady,Pgradz)
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               Pgradx(i,j,k)=sum(this%grd_x(:,i,j,k)*P(i-1:i,j,k))!-this%dPjx(i,j,k)
               Pgrady(i,j,k)=sum(this%grd_y(:,i,j,k)*P(i,j-1:j,k))!-this%dPjy(i,j,k)
               Pgradz(i,j,k)=sum(this%grd_z(:,i,j,k)*P(i,j,k-1:k))!-this%dPjz(i,j,k)
            end do
         end do
      end do
      ! Sync it
   ...
   !> Calculate the pressure gradient based on P
   subroutine get_pgrad_wide(this,P,Pgradx,Pgrady,Pgradz)
      implicit none
      class(tpcons), intent(inout) :: this
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(in)  :: P      !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(out) :: Pgradx !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      ...
      get_interpolated_quantities: block
         ...
         if (.not.this%cfg%xper.and.this%cfg%iproc.eq.1) P_x(this%cfg%imino,:,:)=this%P(this%cfg%imino,:,:)
         if (.not.this%cfg%yper.and.this%cfg%jproc.eq.1) P_y(:,this%cfg%jmino,:)=this%P(:,this%cfg%jmino,:)
         if (.not.this%cfg%zper.and.this%cfg%kproc.eq.1) P_z(:,:,this%cfg%kmino)=this%P(:,:,this%cfg%kmino)
         
         call this%cfg%sync(P_x); call this%cfg%sync(P_y); call this%cfg%sync(P_z)
      end block get_interpolated_quantities

      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               Pgradx(i,j,k)=sum(this%div_x(:,i,j,k)*P_x(i:i+1,j,k))!-this%dPjx(i,j,k)
               Pgrady(i,j,k)=sum(this%div_y(:,i,j,k)*P_y(i,j:j+1,k))!-this%dPjy(i,j,k)
               Pgradz(i,j,k)=sum(this%div_z(:,i,j,k)*P_z(i,j,k:k+1))!-this%dPjz(i,j,k)
            end do
         end do
      end do
      ! Sync it
   ...
   subroutine solve_implicit(this,dt,resU,resV,resW)
      ...
      ! The implicit U,V,W are all the same
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               this%implicit%opr(1,i,j,k)=this%implicit%opr(1,i,j,k)+dt*(this%div_x(1,i,j,k)*this%itp_x(-1,i+1,j,k)*rhoUp+&
               &                                                         this%div_x(0,i,j,k)*this%itp_x( 0,i  ,j,k)*rhoUm+&
               &                                                         this%div_y(1,i,j,k)*this%itp_y(-1,i,j+1,k)*rhoVp+&
               &                                                         this%div_y(0,i,j,k)*this%itp_y( 0,i,j  ,k)*rhoVm+&
               &                                                         this%div_z(1,i,j,k)*this%itp_z(-1,i,j,k+1)*rhoWp+&
               &                                                         this%div_z(0,i,j,k)*this%itp_z( 0,i,j,k  )*rhoWm)*0.5_WP
               this%implicit%opr(2,i,j,k)=this%implicit%opr(2,i,j,k)+dt*(this%div_x(1,i,j,k)*this%itp_x( 0,i+1,j,k)*rhoUp)*0.5_WP
               this%implicit%opr(3,i,j,k)=this%implicit%opr(3,i,j,k)+dt*(this%div_x(0,i,j,k)*this%itp_x(-1,i  ,j,k)*rhoUm)*0.5_WP
               this%implicit%opr(4,i,j,k)=this%implicit%opr(4,i,j,k)+dt*(this%div_y(1,i,j,k)*this%itp_y( 0,i,j+1,k)*rhoVp)*0.5_WP
               this%implicit%opr(5,i,j,k)=this%implicit%opr(5,i,j,k)+dt*(this%div_y(0,i,j,k)*this%itp_y(-1,i,j  ,k)*rhoVm)*0.5_WP
               this%implicit%opr(6,i,j,k)=this%implicit%opr(6,i,j,k)+dt*(this%div_z(1,i,j,k)*this%itp_z( 0,i,j,k+1)*rhoWp)*0.5_WP
               this%implicit%opr(7,i,j,k)=this%implicit%opr(7,i,j,k)+dt*(this%div_z(0,i,j,k)*this%itp_z(-1,i,j,k  )*rhoWm)*0.5_WP
            end do
         end do
      end do
      ! do k=this%cfg%kmin_,this%cfg%kmax_
   ...
   subroutine addsrc_gravity(this,resU,resV,resW)
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               if (this%umask(i,j,k).eq.0) resU(i,j,k)=resU(i,j,k)+this%gravity(1)*this%rho(i,j,k)
               if (this%vmask(i,j,k).eq.0) resV(i,j,k)=resV(i,j,k)+this%gravity(2)*this%rho(i,j,k)
               if (this%wmask(i,j,k).eq.0) resW(i,j,k)=resW(i,j,k)+this%gravity(3)*this%rho(i,j,k)
            end do
         end do
      end do
