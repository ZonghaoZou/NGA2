...
contains
   ...
   subroutine simulation_init
      ...
      create_and_initialize_vof: block
         ...
         use vfs_class, only: lvira,VFhi,VFlo,remap,r2p,plicnet,flux
   ...
   subroutine simulation_run
      ...
      do while (.not.time%done())
         ...
         !     integer :: ierr
         !     real(WP) :: vol_l, V_drop_local, vol_l_local

         !     V_drop_local = sum(Vi*vf%VF*fs%cfg%vol)
         !     vol_l_local  = sum(vf%VF*fs%cfg%vol)
             
         !     call MPI_ALLREDUCE(V_drop_local, V_drop, 1, MPI_REAL_WP, MPI_SUM, cfg%comm, ierr)
         !     call MPI_ALLREDUCE(vol_l_local, vol_l, 1, MPI_REAL_WP, MPI_SUM, cfg%comm, ierr)
             
         !     if (vol_l > 0.0_WP) then
         !         V_drop = V_drop / vol_l
         !     else
         !         V_drop = 0.0_WP
         !     end if
