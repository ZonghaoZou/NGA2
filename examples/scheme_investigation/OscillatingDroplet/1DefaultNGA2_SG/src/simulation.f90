...
module simulation
   ...
   !> NEW: Global Statistics for Monitor
   ...
   real(WP), public :: lmax=0.0_WP
   real(WP), public :: lymax=0.0_WP
...
contains
   ...
   subroutine simulation_init
      ...
      create_monitor: block
         ...
         call mfile%add_column(xc, 'Center of Mass X')
         call mfile%add_column(lmax,'Semi-major Axis (X)')
         call mfile%add_column(lymax,'Semi-minor Axis (Y)')
         call mfile%write()
         ! Create CFL monitor
   ...
   contains
      ...
      function levelset_drop(xyz,t) result(G)
         ...
         real(WP) :: G, r2
         r2 = sum((xyz-center)**2)
         if (r2.gt.epsilon(1.0_WP)) then
            G = radius - sqrt(r2) + lambda*radius*0.5_WP*(3.0_WP*((xyz(1)-center(1))**2)/r2 - 1.0_WP)
         else
            G = radius
         end if
   ...
   subroutine get_stats
      ...
      ! ========================================================================
      ! PASS 1: Integrals (TKE, Volume, Center of Mass)
      ! ========================================================================
      TKE     = 0.0_WP
      vol_liq = 0.0_WP
      mom_x   = 0.0_WP
      ...
      do k = fs%cfg%kmin_, fs%cfg%kmax_
         do j = fs%cfg%jmin_, fs%cfg%jmax_
            do i = fs%cfg%imin_, fs%cfg%imax_
               ...
               ! Accumulate volume weighted by VOF
               if (vf%VF(i,j,k) > epsilon(1.0_WP)) then
                  vol_liq = vol_liq + vf%VF(i,j,k) * cell_vol
                  
                  ! Accumulate first moment of volume (Volume * Position)
                  ! Using cell centers (sufficiently accurate for CoM drift)
               ...
               ! --- Total Kinetic Energy (Liquid + Gas) ---
               ! Density in cell (Arithmetic average)
               rho_cell = fs%rho_l * vf%VF(i,j,k) + fs%rho_g * (1.0_WP - vf%VF(i,j,k))
               
               ! Velocity squared (using interpolated cell-centered velocities Ui,Vi,Wi)
               v_sq = Ui(i,j,k)**2 + Vi(i,j,k)**2 + Wi(i,j,k)**2
               
               ! KE = 0.5 * integral(rho * u^2 dV)
      ...
      ! Reduce Integrals (Sum across all processors)
      ...
      ! Calculate Instantaneous Center of Mass (CoM)
      if (vol_liq > epsilon(1.0_WP)) then
         xc = mom_x / vol_liq
         yc = mom_y / vol_liq
         zc = mom_z / vol_liq
      else
         ! Fallback to domain center if volume vanishes (should not happen)
         xc = center(1)
         yc = center(2)
         zc = center(3)
      end if
               ...
               if(vf%VF(i,j,k).gt.VFlo .and. vf%VF(i,j,k).lt.VFhi) then
                  ...
                  do nplane=1,getNumberOfPlanes(vf%liquid_gas_interface(i,j,k))
                     if (getNumberOfVertices(vf%interface_polygon(nplane,i,j,k)).gt.0) then
                        ...
                        ! --- Track Extents Corrected for Drift ---
                        
                        ! Semi-major axis (assuming perturbation along X)
                        ! Distance from CoM X to interface X
                        if (abs(ploc(1) - xc) .gt. lmax) then
                           lmax = abs(ploc(1) - xc)
                        end if
                        
                        ! Semi-minor axis (perpendicular to X)
                        ! Distance from CoM Y to interface Y
      ...
      ! Reduce Extents (Max across all processors)
