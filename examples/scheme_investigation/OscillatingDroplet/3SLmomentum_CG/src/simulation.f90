...
module simulation
   ...
   real(WP) :: TKE      ! Total Kinetic Energy
   real(WP) :: vol_liq  ! Total Liquid Volume
   real(WP) :: xc, yc, zc ! Instantaneous Center of Mass
   real(WP) :: lmax=0.0_WP
   real(WP) :: lymax=0.0_WP
      ...
      do k = fs%cfg%kmin_, fs%cfg%kmax_
         do j = fs%cfg%jmin_, fs%cfg%jmax_
            do i = fs%cfg%imin_, fs%cfg%imax_
               ...
               ! Total Kinetic Energy (using cell-centered fs%U,fs%V,fs%W)
               rho_cell = fs%rho_l * vf%VF(i,j,k) + fs%rho_g * (1.0_WP - vf%VF(i,j,k))
               v_sq = fs%U(i,j,k)**2 + fs%V(i,j,k)**2 + fs%W(i,j,k)**2
               ...
               if(vf%VF(i,j,k).gt.VFlo .and. vf%VF(i,j,k).lt.VFhi) then
                  do nplane=1,getNumberOfPlanes(vf%liquid_gas_interface(i,j,k))
                     if (getNumberOfVertices(vf%interface_polygon(nplane,i,j,k)).gt.0) then
                        ...
                        if (abs(ploc(1) - xc) .gt. lmax)  lmax  = abs(ploc(1) - xc)
                        if (abs(ploc(2) - yc) .gt. lymax) lymax = abs(ploc(2) - yc)
                     end if
                  end do
               end if
            end do
