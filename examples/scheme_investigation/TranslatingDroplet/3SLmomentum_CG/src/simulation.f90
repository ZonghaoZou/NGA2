...
module simulation
   ...
   real(WP) :: vel_perturb_max=0.0_WP
   real(WP) :: vel_perturb_rms=0.0_WP
   real(WP) :: vel_perturb_mean=0.0_WP
   real(WP) :: Ca_max=0.0_WP
   real(WP) :: Ca_rms=0.0_WP
   real(WP) :: Ca_mean=0.0_WP
...
contains
   ...
   subroutine simulation_init
      ...
      ! Create a two-phase collocated flow solver
      ...
      initialize_velocity: block
         ! Initialize with uniform translation velocity (cell-centered and face)
         fs%U=U0; fs%V=U0; fs%W=U0
         fs%Uf=U0; fs%Vf=U0; fs%Wf=U0
         ! Initialize density
         fs%rho=fs%rho_l*vf%VF+fs%rho_g*(1.0_WP-vf%VF)
         call fs%update_faceRHO(vf=vf,rho=fs%rho)
   ...
   !> For collocated scheme: fs%U, fs%V, fs%W are cell-centered
      ...
      do k = fs%cfg%kmin_, fs%cfg%kmax_
         do j = fs%cfg%jmin_, fs%cfg%jmax_
            do i = fs%cfg%imin_, fs%cfg%imax_
               ...
               ! For collocated: fs%U, fs%V, fs%W are cell-centered
               ...
               U_perturb_sq = (fs%U(i,j,k) - U0)**2 + (fs%V(i,j,k) - U0)**2 + (fs%W(i,j,k) - U0)**2
      ...
      do k=cfg%kmin_,cfg%kmax_
         do j=cfg%jmin_,cfg%jmax_
            do i=cfg%imin_,cfg%imax_
               ncell=ncell+1.0_WP
               ! For collocated: fs%U, fs%V, fs%W are cell-centered
               Utmp=(fs%U(i,j,k)-U0)**2+(fs%V(i,j,k)-U0)**2+(fs%W(i,j,k)-U0)**2
               U_sum=U_sum+Utmp
