...
contains
   ...
   subroutine get_Ca
      ...
      do k=cfg%kmin_,cfg%kmax_
         do j=cfg%jmin_,cfg%jmax_
            do i=cfg%imin_,cfg%imax_
               ...
               Utmp=vel(1,i,j,k)**2+vel(2,i,j,k)**2+vel(3,i,j,k)**2
               Umidtmp=Ui(i,j,k)**2+Vi(i,j,k)**2+Wi(i,j,k)**2 
               U_sum=U_sum+Utmp
               Umid_sum=Umid_sum+Umidtmp
               U_max=max(U_max,Utmp)
               Umid_max=max(Umid_max,Umidtmp)
            end do
         end do
      end do
      call MPI_ALLREDUCE(MPI_IN_PLACE,U_sum,1,MPI_REAL_WP,MPI_SUM,vf%cfg%comm,ierr)
      call MPI_ALLREDUCE(MPI_IN_PLACE,Umid_sum,1,MPI_REAL_WP,MPI_SUM,vf%cfg%comm,ierr)
      ...
      call MPI_ALLREDUCE(MPI_IN_PLACE,Umid_max,1,MPI_REAL_WP,MPI_MAX,vf%cfg%comm,ierr)
      U_sum=U_sum/ncell; Ca_rms_U=sqrt(U_sum)*fs%visc_l/fs%sigma
      Umid_sum=Umid_sum/ncell; Ca_rms_Umid=sqrt(Umid_sum)*fs%visc_l/fs%sigma
      Ca_max_U=sqrt(U_max)*fs%visc_l/fs%sigma
      Ca_max_Umid=sqrt(Umid_max)*fs%visc_l/fs%sigma
