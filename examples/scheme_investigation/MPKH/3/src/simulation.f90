...
!> Rayleigh-Taylor instability — Scheme 3: SL Momentum (Collocated Grid)
!>
...
subroutine postproc_data()
   ...
   subroutine simulation_init
      ...
      create_flow_solver: block
         ...
         use tpcons_class,    only: dirichlet,slip
         ! Create flow solver
            ...
            do k=fs%cfg%kmin_,fs%cfg%kmax_
               do j=fs%cfg%jmin_,fs%cfg%jmax_
                  do i=fs%cfg%imin_,fs%cfg%imax_+1
                     call eval_kh(fs%cfg%xm(i), fs%cfg%ym(j), eps_kh, u_tmp, v_tmp, G_tmp)
                     fs%U(i,j,k) = u_tmp
                  end do
               end do
            end do
