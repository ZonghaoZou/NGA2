...
meshsize="8";
filename=folderpick+"/Run_CASE2_nx"+meshsize+"/monitor/simulation";
startRow = 3;
...
figure('Color','w')
plot(time_8*omega0,(Asim_8-pi)/(2*pi),'LineWidth',LW2,'LineStyle','--','Color','k')
hold on
plot(time_16*omega0,(Asim_16-pi)/(2*pi),'LineWidth',LW2,'LineStyle',':','Color','k')
hold on
plot(time_32*omega0,(Asim_32-pi)/(2*pi),'LineWidth',LW2,'LineStyle','-.','Color','k')
hold on
plot(time_64*omega0,(Asim_64-pi)/(2*pi),'LineWidth',LW2,'LineStyle','-','Color','k')
hold on
plot(t*omega0,Aex,'LineWidth',LW1,'Marker','.','Markersize',14.0,'Color','k')
xlim([0,20])
ylim([-0.01,0.01])
yticks([-0.01,-0.005,0,0.005,0.01])
% legend({"8","16","32","64","Prosperetti"}, 'Location', 'southeast', 'Interpreter', 'latex');
set(gca,'Fontsize',20)
...
%   + (z3*A0*omega0^2)/(Z3*(z3^2-nu))*exp((z3^2-nu)*t).*erfc(real(z3)*sqrt(t)) ...
%   + (z4*A0*omega0^2)/(Z4*(z4^2-nu))*exp((z4^2-nu)*t).*erfc(real(z4)*sqrt(t)) )/(2*pi);
% error_128= ((Asim_128-pi)/(2*pi)-Aex)/perturb;


figure
plot(time_8*omega0,error_8,'LineWidth',LW2,'LineStyle','--','Color','k')
hold on
plot(time_16*omega0,error_16,'LineWidth',LW2,'LineStyle',':','Color','k')
hold on
plot(time_32*omega0,error_32,'LineWidth',LW2,'LineStyle','-.','Color','k')
hold on
plot(time_64*omega0,error_64,'LineWidth',LW2,'LineStyle','-','Color','k')
xlim([0,20])
ylim([-0.35,0.35])
% legend({"8","16","32","64"}, 'Location', 'southeast', 'Interpreter', 'latex');
set(gca,'Fontsize',20)
set(gca,'fontname','Times New Roman')
...
rms_32=rms(error_32);
rms_64=rms(error_64);
% rms_128=rms(error_128);
dx=[2*pi/8 2*pi/16 2*pi/32 2*pi/64];
rms_list=[rms_8 rms_16 rms_32 rms_64];

x_1st=[2*pi/12 2*pi/(sqrt(3)*12)];
y_1st=[0.05  0.05/sqrt(3)];
x_2nd=[2*pi/8 2*pi/(sqrt(3)*8)];
y_2nd=[0.04 0.04/3];
figure
loglog(dx, rms_list,'LineWidth',LW2,'Color',blue)
hold on
loglog(x_2nd, y_2nd,'LineWidth',LW2,'Color','k')
hold on
loglog(x_1st, y_1st,'LineWidth',LW2,'Color','k')
xlim([0.08,1])
ylim([0.008,0.1])
xticks([0.1,1])
