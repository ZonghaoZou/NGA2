...
module tpcons_class
   ...
   type :: tpcons
      ...
      real(WP), dimension(:,:,:), allocatable :: fluxR1    !< dPressure jump to add to -ddP/dz
      real(WP), dimension(:,:,:), allocatable :: fluxR2    !< dPressure jump to add to -ddP/dz
      real(WP), dimension(:,:,:), allocatable :: fluxR3    !< dPressure jump to add to -ddP/dz
      real(WP), dimension(:,:,:), allocatable :: fluxR4    !< dPressure jump to add to -ddP/dz
      real(WP), dimension(:,:,:), allocatable :: fluxR5    !< dPressure jump to add to -ddP/dz
      real(WP), dimension(:,:,:), allocatable :: fluxR6    !< dPressure jump to add to -ddP/dz
      real(WP), dimension(:,:,:), allocatable :: fluxR7    !< dPressure jump to add to -ddP/dz
      real(WP), dimension(:,:,:), allocatable :: fluxR8    !< dPressure jump to add to -ddP/dz
      real(WP), dimension(:,:,:), allocatable :: fluxR9    !< dPressure jump to add to -ddP/dz
...
contains
   ...
   subroutine initialize(this,cfg,name)
      ...
      allocate(this%fluxR1(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%fluxR1=0.0_WP
      allocate(this%fluxR2(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%fluxR2=0.0_WP
      allocate(this%fluxR3(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%fluxR3=0.0_WP
      allocate(this%fluxR4(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%fluxR4=0.0_WP
      allocate(this%fluxR5(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%fluxR5=0.0_WP
      allocate(this%fluxR6(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%fluxR6=0.0_WP
      allocate(this%fluxR7(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%fluxR7=0.0_WP
      allocate(this%fluxR8(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%fluxR8=0.0_WP
      allocate(this%fluxR9(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%fluxR9=0.0_WP
   ...
   subroutine add_bcond(this,name,type,locator,face,dir,canCorrect)
      ...
      ! 1. Store Direction and Correction capabilities (TPCONS specific)
      ...
      case default
         call die('[tpcons add_bcond] Unknown bcond face - expecting x, y, or z')
      end select
      ...
      case (convective)
      case default
         call die('[tpcons add_bcond] Unknown bcond type')
      end select
   ...
   subroutine apply_bcond(this,dt,scope)
      ...
      case default
         call die('[tpcons apply_bcond] Unknown scope - use cell, face, or all')
      end select
   ...
   subroutine get_dmomdt(this,vf,drhoUdt,drhoVdt,drhoWdt)
      ...
      real(WP) :: flux_con,flux_sl,flux_ratio
      ...
      this%fluxR1=0.0_WP; this%fluxR2=0.0_WP; this%fluxR3=0.0_WP; this%fluxR4=0.0_WP; this%fluxR5=0.0_WP; this%fluxR6=0.0_WP; this%fluxR7=0.0_WP; this%fluxR8=0.0_WP; this%fluxR9=0.0_WP
      ! Flux of rhoU
      do k=this%cfg%kmin_,this%cfg%kmax_+1
         do j=this%cfg%jmin_,this%cfg%jmax_+1
            do i=this%cfg%imin_,this%cfg%imax_+1
               if (minval(abs(vf%bandold(i-1:i,j,k))).le.advect_band) then
                  flux_con=-this%rhoU(i,j,k)*sum(this%itpr_x(:,i,j,k)*this%U(i-1:i,j,k))
                  flux_sl =-vf%MFX(1,i,j,k)
                  flux_ratio=1.0_WP
                  this%fluxR1(i,j,k)=flux_ratio
                  FX(i,j,k)=flux_ratio*flux_sl+(1.0_WP-flux_ratio)*flux_con!flux_con+flux_ratio*(flux_sl-flux_con)
                  ! FX(i,j,k)=-this%rhoU(i,j,k)*sum(this%itpr_x(:,i,j,k)*this%U(i-1:i,j,k))
               else
                  FX(i,j,k)=-this%rhoU(i,j,k)*sum(this%itpr_x(:,i,j,k)*this%U(i-1:i,j,k))
               end if
               if (minval(abs(vf%bandold(i,j-1:j,k))).le.advect_band) then
                  flux_con=-this%rhoV(i,j,k)*sum(this%itpr_y(:,i,j,k)*this%U(i,j-1:j,k)) 
                  flux_sl =-vf%MFY(1,i,j,k)
                  flux_ratio=1.0_WP
                  this%fluxR2(i,j,k)=flux_ratio
                  FY(i,j,k)=flux_ratio*flux_sl+(1.0_WP-flux_ratio)*flux_con
                  ! FY(i,j,k)=-this%rhoV(i,j,k)*sum(this%itpr_y(:,i,j,k)*this%U(i,j-1:j,k)) 
               else
                  FY(i,j,k)=-this%rhoV(i,j,k)*sum(this%itpr_y(:,i,j,k)*this%U(i,j-1:j,k)) 
               end if
               if (minval(abs(vf%bandold(i,j,k-1:k))).le.advect_band) then
                  flux_con=-this%rhoW(i,j,k)*sum(this%itpr_z(:,i,j,k)*this%U(i,j,k-1:k)) 
                  flux_sl =-vf%MFZ(1,i,j,k)
                  flux_ratio=1.0_WP
                  this%fluxR3(i,j,k)=flux_ratio
                  FZ(i,j,k)=flux_ratio*flux_sl+(1.0_WP-flux_ratio)*flux_con
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_+1
         do j=this%cfg%jmin_,this%cfg%jmax_+1
            do i=this%cfg%imin_,this%cfg%imax_+1
               if (minval(abs(vf%bandold(i-1:i,j,k))).le.advect_band) then
                  flux_con=-this%rhoU(i,j,k)*sum(this%itpr_x(:,i,j,k)*this%V(i-1:i,j,k))
                  flux_sl =-vf%MFX(2,i,j,k)
                  flux_ratio=1.0_WP
                  this%fluxR4(i,j,k)=flux_ratio
                  FX(i,j,k)=flux_ratio*flux_sl+(1.0_WP-flux_ratio)*flux_con
                  ! FX(i,j,k)=-this%rhoU(i,j,k)*sum(this%itpr_x(:,i,j,k)*this%V(i-1:i,j,k))
               else
                  FX(i,j,k)=-this%rhoU(i,j,k)*sum(this%itpr_x(:,i,j,k)*this%V(i-1:i,j,k))
               end if
               if (minval(abs(vf%bandold(i,j-1:j,k))).le.advect_band) then
                  flux_con=-this%rhoV(i,j,k)*sum(this%itpr_y(:,i,j,k)*this%V(i,j-1:j,k))
                  flux_sl =-vf%MFY(2,i,j,k)
                  flux_ratio=1.0_WP
                  this%fluxR5(i,j,k)=flux_ratio
                  FY(i,j,k)=flux_ratio*flux_sl+(1.0_WP-flux_ratio)*flux_con
               ...
               if (minval(abs(vf%bandold(i,j,k-1:k))).le.advect_band) then
                  flux_con=-this%rhoW(i,j,k)*sum(this%itpr_z(:,i,j,k)*this%V(i,j,k-1:k))
                  flux_sl =-vf%MFZ(2,i,j,k)
                  flux_ratio=1.0_WP
                  this%fluxR6(i,j,k)=flux_ratio
                  FZ(i,j,k)=flux_ratio*flux_sl+(1.0_WP-flux_ratio)*flux_con
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_+1
         do j=this%cfg%jmin_,this%cfg%jmax_+1
            do i=this%cfg%imin_,this%cfg%imax_+1
               if (minval(abs(vf%bandold(i-1:i,j,k))).le.advect_band) then
                  flux_con=-this%rhoU(i,j,k)*sum(this%itpr_x(:,i,j,k)*this%W(i-1:i,j,k))
                  flux_sl =-vf%MFX(3,i,j,k)
                  flux_ratio=1.0_WP
                  this%fluxR7(i,j,k)=flux_ratio
                  FX(i,j,k)=flux_ratio*flux_sl+(1.0_WP-flux_ratio)*flux_con
               ...
               if (minval(abs(vf%bandold(i,j-1:j,k))).le.advect_band) then
                  flux_con=-this%rhoV(i,j,k)*sum(this%itpr_y(:,i,j,k)*this%W(i,j-1:j,k))
                  flux_sl =-vf%MFY(3,i,j,k)
                  flux_ratio=1.0_WP
                  this%fluxR8(i,j,k)=flux_ratio
                  FY(i,j,k)=flux_ratio*flux_sl+(1.0_WP-flux_ratio)*flux_con
               ...
               if (minval(abs(vf%bandold(i,j,k-1:k))).le.advect_band) then
                  flux_con=-this%rhoW(i,j,k)*sum(this%itpr_z(:,i,j,k)*this%W(i,j,k-1:k))
                  flux_sl =-vf%MFZ(3,i,j,k)
                  flux_ratio=1.0_WP
                  this%fluxR9(i,j,k)=flux_ratio
                  FZ(i,j,k)=flux_ratio*flux_sl+(1.0_WP-flux_ratio)*flux_con
   ...
   subroutine update_pgrad_all(this,vf,dt)
      use vfs_class, only: vfs
      implicit none
      class(tpcons), intent(inout) :: this
      class(vfs), intent(in) :: vf
      ...
      this%Uf=this%Uf-dt*PgradX/this%RHOX
      this%Vf=this%Vf-dt*PgradY/this%RHOY
      this%Wf=this%Wf-dt*PgradZ/this%RHOZ
      
   end subroutine update_pgrad_all


   subroutine get_cell_pgrad(this,vf,P,Pgradx,Pgrady,Pgradz,apply_hydrostatic)
      use vfs_class, only: vfs
      implicit none
      class(tpcons), intent(inout) :: this
      class(vfs), intent(in) :: vf
      ...
      logical, intent(in), optional :: apply_hydrostatic
      integer :: i,j,k
      logical :: do_hydrostatic
      real(WP), dimension(:,:,:), allocatable :: PX,PY,PZ
      ! Handle optional flag
      do_hydrostatic=.false.
      if (present(apply_hydrostatic)) do_hydrostatic=apply_hydrostatic
      ! Allocate flux arrays
      ...
      ! Density weighted surface tension jump
   ...
   subroutine get_strainrate(this,SR)
      ...
      ! Compute dudx, dvdy, and dwdz first
	   do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               SR(1,i,j,k)=sum(this%grdu_x(:,i,j,k)*this%Uf(i:i+1,j,k))
               SR(2,i,j,k)=sum(this%grdv_y(:,i,j,k)*this%Vf(i,j:j+1,k))
               SR(3,i,j,k)=sum(this%grdw_z(:,i,j,k)*this%Wf(i,j,k:k+1))
   ...
   subroutine get_cfl(this,dt,cflc,cfl)
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               my_CFLc_x=max(my_CFLc_x,abs(this%Uf(i,j,k))*this%cfg%dxmi(i))
               my_CFLc_y=max(my_CFLc_y,abs(this%Vf(i,j,k))*this%cfg%dymi(j))
               my_CFLc_z=max(my_CFLc_z,abs(this%Wf(i,j,k))*this%cfg%dzmi(k))
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               ! +X face
               this%implicit%opr(1,i,j,k)=this%implicit%opr(1,i,j,k)+dt*this%divp_x(+1,i,j,k)*this%itpr_x(-1,i+1,j,k)*rhoUp*0.5_WP*(1.0_WP-this%fluxR1(i+1,j,k))
               this%implicit%opr(2,i,j,k)=this%implicit%opr(2,i,j,k)+dt*this%divp_x(+1,i,j,k)*this%itpr_x( 0,i+1,j,k)*rhoUp*0.5_WP*(1.0_WP-this%fluxR1(i+1,j,k))
               this%implicit%opr(1,i,j,k)=this%implicit%opr(1,i,j,k)+dt*this%divp_x( 0,i,j,k)*this%itpr_x( 0,i  ,j,k)*rhoUm*0.5_WP*(1.0_WP-this%fluxR1(i  ,j,k))
               this%implicit%opr(3,i,j,k)=this%implicit%opr(3,i,j,k)+dt*this%divp_x( 0,i,j,k)*this%itpr_x(-1,i  ,j,k)*rhoUm*0.5_WP*(1.0_WP-this%fluxR1(i  ,j,k))
               this%implicit%opr(1,i,j,k)=this%implicit%opr(1,i,j,k)+dt*this%divp_y(+1,i,j,k)*this%itpr_y(-1,i,j+1,k)*rhoVp*0.5_WP*(1.0_WP-this%fluxR2(i,j+1,k))
               this%implicit%opr(4,i,j,k)=this%implicit%opr(4,i,j,k)+dt*this%divp_y(+1,i,j,k)*this%itpr_y( 0,i,j+1,k)*rhoVp*0.5_WP*(1.0_WP-this%fluxR2(i,j+1,k))
               this%implicit%opr(1,i,j,k)=this%implicit%opr(1,i,j,k)+dt*this%divp_y( 0,i,j,k)*this%itpr_y( 0,i,j  ,k)*rhoVm*0.5_WP*(1.0_WP-this%fluxR2(i,j  ,k))
               this%implicit%opr(5,i,j,k)=this%implicit%opr(5,i,j,k)+dt*this%divp_y( 0,i,j,k)*this%itpr_y(-1,i,j  ,k)*rhoVm*0.5_WP*(1.0_WP-this%fluxR2(i,j  ,k))
               this%implicit%opr(1,i,j,k)=this%implicit%opr(1,i,j,k)+dt*this%divp_z(+1,i,j,k)*this%itpr_z(-1,i,j,k+1)*rhoWp*0.5_WP*(1.0_WP-this%fluxR3(i,j,k+1))
               this%implicit%opr(6,i,j,k)=this%implicit%opr(6,i,j,k)+dt*this%divp_z(+1,i,j,k)*this%itpr_z( 0,i,j,k+1)*rhoWp*0.5_WP*(1.0_WP-this%fluxR3(i,j,k+1))
               this%implicit%opr(1,i,j,k)=this%implicit%opr(1,i,j,k)+dt*this%divp_z( 0,i,j,k)*this%itpr_z( 0,i,j,k  )*rhoWm*0.5_WP*(1.0_WP-this%fluxR3(i,j,k  ))
               this%implicit%opr(7,i,j,k)=this%implicit%opr(7,i,j,k)+dt*this%divp_z( 0,i,j,k)*this%itpr_z(-1,i,j,k  )*rhoWm*0.5_WP*(1.0_WP-this%fluxR3(i,j,k  ))
            end do
         end do
      end do
      ! Viscosity
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               rhoUm=this%rhoU(i  ,j,k); rhoVm=this%rhoV(i,j  ,k); rhoWm=this%rhoW(i,j,k  )
               ! +X face
                  this%implicit%opr(1,i,j,k)=this%implicit%opr(1,i,j,k)+dt*this%divp_x(+1,i,j,k)*this%itpr_x(-1,i+1,j,k)*rhoUp*0.5_WP*(1.0_WP-this%fluxR4(i+1,j,k))
                  this%implicit%opr(2,i,j,k)=this%implicit%opr(2,i,j,k)+dt*this%divp_x(+1,i,j,k)*this%itpr_x( 0,i+1,j,k)*rhoUp*0.5_WP*(1.0_WP-this%fluxR4(i+1,j,k))
                  this%implicit%opr(1,i,j,k)=this%implicit%opr(1,i,j,k)+dt*this%divp_x( 0,i,j,k)*this%itpr_x( 0,i  ,j,k)*rhoUm*0.5_WP*(1.0_WP-this%fluxR4(i  ,j,k))
                  this%implicit%opr(3,i,j,k)=this%implicit%opr(3,i,j,k)+dt*this%divp_x( 0,i,j,k)*this%itpr_x(-1,i  ,j,k)*rhoUm*0.5_WP*(1.0_WP-this%fluxR4(i  ,j,k))
                  this%implicit%opr(1,i,j,k)=this%implicit%opr(1,i,j,k)+dt*this%divp_y(+1,i,j,k)*this%itpr_y(-1,i,j+1,k)*rhoVp*0.5_WP*(1.0_WP-this%fluxR5(i,j+1,k))
                  this%implicit%opr(4,i,j,k)=this%implicit%opr(4,i,j,k)+dt*this%divp_y(+1,i,j,k)*this%itpr_y( 0,i,j+1,k)*rhoVp*0.5_WP*(1.0_WP-this%fluxR5(i,j+1,k))
                  this%implicit%opr(1,i,j,k)=this%implicit%opr(1,i,j,k)+dt*this%divp_y( 0,i,j,k)*this%itpr_y( 0,i,j  ,k)*rhoVm*0.5_WP*(1.0_WP-this%fluxR5(i,j  ,k))
                  this%implicit%opr(5,i,j,k)=this%implicit%opr(5,i,j,k)+dt*this%divp_y( 0,i,j,k)*this%itpr_y(-1,i,j  ,k)*rhoVm*0.5_WP*(1.0_WP-this%fluxR5(i,j  ,k))
                  this%implicit%opr(1,i,j,k)=this%implicit%opr(1,i,j,k)+dt*this%divp_z(+1,i,j,k)*this%itpr_z(-1,i,j,k+1)*rhoWp*0.5_WP*(1.0_WP-this%fluxR6(i,j,k+1))
                  this%implicit%opr(6,i,j,k)=this%implicit%opr(6,i,j,k)+dt*this%divp_z(+1,i,j,k)*this%itpr_z( 0,i,j,k+1)*rhoWp*0.5_WP*(1.0_WP-this%fluxR6(i,j,k+1))
                  this%implicit%opr(1,i,j,k)=this%implicit%opr(1,i,j,k)+dt*this%divp_z( 0,i,j,k)*this%itpr_z( 0,i,j,k  )*rhoWm*0.5_WP*(1.0_WP-this%fluxR6(i,j,k  ))
                  this%implicit%opr(7,i,j,k)=this%implicit%opr(7,i,j,k)+dt*this%divp_z( 0,i,j,k)*this%itpr_z(-1,i,j,k  )*rhoWm*0.5_WP*(1.0_WP-this%fluxR6(i,j,k  ))
            end do
         end do
      end do
      ! Viscosity
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               rhoUp=this%rhoU(i+1,j,k); rhoVp=this%rhoV(i,j+1,k); rhoWp=this%rhoW(i,j,k+1)
               rhoUm=this%rhoU(i  ,j,k); rhoVm=this%rhoV(i,j  ,k); rhoWm=this%rhoW(i,j,k  )
                  this%implicit%opr(1,i,j,k)=this%implicit%opr(1,i,j,k)+dt*this%divp_x(+1,i,j,k)*this%itpr_x(-1,i+1,j,k)*rhoUp*0.5_WP*(1.0_WP-this%fluxR7(i+1,j,k))
                  this%implicit%opr(2,i,j,k)=this%implicit%opr(2,i,j,k)+dt*this%divp_x(+1,i,j,k)*this%itpr_x( 0,i+1,j,k)*rhoUp*0.5_WP*(1.0_WP-this%fluxR7(i+1,j,k)) 
                  this%implicit%opr(1,i,j,k)=this%implicit%opr(1,i,j,k)+dt*this%divp_x( 0,i,j,k)*this%itpr_x( 0,i  ,j,k)*rhoUm*0.5_WP*(1.0_WP-this%fluxR7(i  ,j,k))
                  this%implicit%opr(3,i,j,k)=this%implicit%opr(3,i,j,k)+dt*this%divp_x( 0,i,j,k)*this%itpr_x(-1,i  ,j,k)*rhoUm*0.5_WP*(1.0_WP-this%fluxR7(i  ,j,k))
                  this%implicit%opr(1,i,j,k)=this%implicit%opr(1,i,j,k)+dt*this%divp_y(+1,i,j,k)*this%itpr_y(-1,i,j+1,k)*rhoVp*0.5_WP*(1.0_WP-this%fluxR8(i,j+1,k))
                  this%implicit%opr(4,i,j,k)=this%implicit%opr(4,i,j,k)+dt*this%divp_y(+1,i,j,k)*this%itpr_y( 0,i,j+1,k)*rhoVp*0.5_WP*(1.0_WP-this%fluxR8(i,j+1,k))
                  this%implicit%opr(1,i,j,k)=this%implicit%opr(1,i,j,k)+dt*this%divp_y( 0,i,j,k)*this%itpr_y( 0,i,j  ,k)*rhoVm*0.5_WP*(1.0_WP-this%fluxR8(i,j  ,k))
                  this%implicit%opr(5,i,j,k)=this%implicit%opr(5,i,j,k)+dt*this%divp_y( 0,i,j,k)*this%itpr_y(-1,i,j  ,k)*rhoVm*0.5_WP*(1.0_WP-this%fluxR8(i,j  ,k))
                  this%implicit%opr(1,i,j,k)=this%implicit%opr(1,i,j,k)+dt*this%divp_z(+1,i,j,k)*this%itpr_z(-1,i,j,k+1)*rhoWp*0.5_WP*(1.0_WP-this%fluxR9(i,j,k+1))
                  this%implicit%opr(6,i,j,k)=this%implicit%opr(6,i,j,k)+dt*this%divp_z(+1,i,j,k)*this%itpr_z( 0,i,j,k+1)*rhoWp*0.5_WP*(1.0_WP-this%fluxR9(i,j,k+1))
                  this%implicit%opr(1,i,j,k)=this%implicit%opr(1,i,j,k)+dt*this%divp_z( 0,i,j,k)*this%itpr_z( 0,i,j,k  )*rhoWm*0.5_WP*(1.0_WP-this%fluxR9(i,j,k  ))
                  this%implicit%opr(7,i,j,k)=this%implicit%opr(7,i,j,k)+dt*this%divp_z( 0,i,j,k)*this%itpr_z(-1,i,j,k  )*rhoWm*0.5_WP*(1.0_WP-this%fluxR9(i,j,k  ))
            end do
         end do
      end do
      ! Viscosity
   ...
   subroutine update_faceRHO(this,vf,rho)
      use vfs_class, only: vfs
      implicit none
      class(tpcons), intent(inout) :: this
      class(vfs), intent(in) :: vf
      ...
      this%RHOX=1.0_WP; this%RHOY=1.0_WP; this%RHOZ=1.0_WP
      do k=this%cfg%kmin_,this%cfg%kmax_+1
         do j=this%cfg%jmin_,this%cfg%jmax_+1
            do i=this%cfg%imin_,this%cfg%imax_+1
               ! Update face pressure and density in X
               rho_f(1)=0.0_WP; vol_r=sum(vf%Gvol(0,:,:,i  ,j,k)+vf%Lvol(0,:,:,i  ,j,k))
               if (vol_r.gt.0.0_WP) rho_f(1)=(sum(vf%Gvol(0,:,:,i  ,j,k))*this%rho_g&
                                             +sum(vf%Lvol(0,:,:,i  ,j,k))*this%rho_l)
               rho_f(0)=0.0_WP; vol_l=sum(vf%Gvol(1,:,:,i-1,j,k)+vf%Lvol(1,:,:,i-1,j,k))
               if (vol_l.gt.0.0_WP) rho_f(0)=(sum(vf%Gvol(1,:,:,i-1,j,k))*this%rho_g&
                                             +sum(vf%Lvol(1,:,:,i-1,j,k))*this%rho_l)
               if (sum(rho_f).gt.0.0_WP) then
                  this%RHOX(i,j,k)=sum(this%itpr_x(:,i,j,k)*rho_f)/sum(this%itpr_x(:,i,j,k)*[vol_l,vol_r])
               end if
               ! Update face pressure and density in Y
               rho_f(1)=0.0_WP; vol_r=sum(vf%Gvol(:,0,:,i,j  ,k)+vf%Lvol(:,0,:,i,j  ,k))
               if (vol_r.gt.0.0_WP) rho_f(1)=(sum(vf%Gvol(:,0,:,i,j  ,k))*this%rho_g&
                                             +sum(vf%Lvol(:,0,:,i,j  ,k))*this%rho_l)
               rho_f(0)=0.0_WP; vol_l=sum(vf%Gvol(:,1,:,i,j-1,k)+vf%Lvol(:,1,:,i,j-1,k))
               if (vol_l.gt.0.0_WP) rho_f(0)=(sum(vf%Gvol(:,1,:,i,j-1,k))*this%rho_g&
                                             +sum(vf%Lvol(:,1,:,i,j-1,k))*this%rho_l)
               if (sum(rho_f).gt.0.0_WP) then
                  this%RHOY(i,j,k)=sum(this%itpr_y(:,i,j,k)*rho_f)/sum(this%itpr_y(:,i,j,k)*[vol_l,vol_r])
               end if
               ! Update face pressure and density in Z
               rho_f(1)=0.0_WP; vol_r=sum(vf%Gvol(:,:,0,i,j,k  )+vf%Lvol(:,:,0,i,j,k  ))
               if (vol_r.gt.0.0_WP) rho_f(1)=(sum(vf%Gvol(:,:,0,i,j,k  ))*this%rho_g&
                                             +sum(vf%Lvol(:,:,0,i,j,k  ))*this%rho_l)
               rho_f(0)=0.0_WP; vol_l=sum(vf%Gvol(:,:,1,i,j,k-1)+vf%Lvol(:,:,1,i,j,k-1))
               if (vol_l.gt.0.0_WP) rho_f(0)=(sum(vf%Gvol(:,:,1,i,j,k-1))*this%rho_g&
                                             +sum(vf%Lvol(:,:,1,i,j,k-1))*this%rho_l)
               if (sum(rho_f).gt.0.0_WP) then
                  this%RHOZ(i,j,k)=sum(this%itpr_z(:,i,j,k)*rho_f)/sum(this%itpr_z(:,i,j,k)*[vol_l,vol_r])
               end if
            end do
         end do
      end do
         ...
         do k=this%cfg%kmino_+1,this%cfg%kmaxo_
            do j=this%cfg%jmino_+1,this%cfg%jmaxo_
               do i=this%cfg%imino_+1,this%cfg%imaxo_
                  ...
                  ! VISC_x at [x,ym,zm] - face x
                  liq_vol=sum(vf%Lvol(0,:,:,i,j,k))+sum(vf%Lvol(1,:,:,i-1,j,k))
                  gas_vol=sum(vf%Gvol(0,:,:,i,j,k))+sum(vf%Gvol(1,:,:,i-1,j,k))
                  tot_vol=gas_vol+liq_vol
                  this%visc_x(i,j,k)=0.0_WP
                  if (tot_vol.gt.0.0_WP) this%visc_x(i,j,k)=this%visc_g*this%visc_l/(this%visc_l*gas_vol/tot_vol+this%visc_g*liq_vol/tot_vol+epsilon(1.0_WP))
                  ! VISC_y at [xm,y,zm] - face y
                  liq_vol=sum(vf%Lvol(:,0,:,i,j,k))+sum(vf%Lvol(:,1,:,i,j-1,k))
                  gas_vol=sum(vf%Gvol(:,0,:,i,j,k))+sum(vf%Gvol(:,1,:,i,j-1,k))
                  tot_vol=gas_vol+liq_vol
                  this%visc_y(i,j,k)=0.0_WP
                  if (tot_vol.gt.0.0_WP) this%visc_y(i,j,k)=this%visc_g*this%visc_l/(this%visc_l*gas_vol/tot_vol+this%visc_g*liq_vol/tot_vol+epsilon(1.0_WP))
                  ! VISC_z at [xm,ym,z] - face z
                  liq_vol=sum(vf%Lvol(:,:,0,i,j,k))+sum(vf%Lvol(:,:,1,i,j,k-1))
                  gas_vol=sum(vf%Gvol(:,:,0,i,j,k))+sum(vf%Gvol(:,:,1,i,j,k-1))
                  tot_vol=gas_vol+liq_vol
                  this%visc_z(i,j,k)=0.0_WP
                  if (tot_vol.gt.0.0_WP) this%visc_z(i,j,k)=this%visc_g*this%visc_l/(this%visc_l*gas_vol/tot_vol+this%visc_g*liq_vol/tot_vol+epsilon(1.0_WP))
               end do
            end do
         end do
         ...
         do k=this%cfg%kmino_+1,this%cfg%kmaxo_
            do j=this%cfg%jmino_+1,this%cfg%jmaxo_
               do i=this%cfg%imino_+1,this%cfg%imaxo_
                  ...
                  tot_vol=gas_vol+liq_vol
                  this%visc_x(i,j,k)=0.0_WP
                  if (tot_vol.gt.0.0_WP) this%visc_x(i,j,k)=(this%visc_l*liq_vol+this%visc_g*gas_vol)/tot_vol
                  ! VISC_y at [xm,y,zm] - face y
                  liq_vol=sum(vf%Lvol(:,0,:,i,j,k))+sum(vf%Lvol(:,1,:,i,j-1,k))
                  gas_vol=sum(vf%Gvol(:,0,:,i,j,k))+sum(vf%Gvol(:,1,:,i,j-1,k))
                  tot_vol=gas_vol+liq_vol
                  this%visc_y(i,j,k)=0.0_WP
                  if (tot_vol.gt.0.0_WP) this%visc_y(i,j,k)=(this%visc_l*liq_vol+this%visc_g*gas_vol)/tot_vol
                  ! VISC_z at [xm,ym,z] - face z
                  liq_vol=sum(vf%Lvol(:,:,0,i,j,k))+sum(vf%Lvol(:,:,1,i,j,k-1))
                  gas_vol=sum(vf%Gvol(:,:,0,i,j,k))+sum(vf%Gvol(:,:,1,i,j,k-1))
                  tot_vol=gas_vol+liq_vol
                  this%visc_z(i,j,k)=0.0_WP
                  if (tot_vol.gt.0.0_WP) this%visc_z(i,j,k)=(this%visc_l*liq_vol+this%visc_g*gas_vol)/tot_vol
               end do
            end do
         end do
      case default
   ...
   subroutine viscosity_gravity_explict(this,vf,dt)
      ...
      contains
         subroutine viscosity_gravity_substep()
            ...
            real(WP), dimension(:,:,:), allocatable :: Uf_y,Uf_z,Vf_x,Vf_z,Wf_x,Wf_y
            real(WP), dimension(:,:,:), allocatable :: VISCforceX,VISCforceY,VISCforceZ
            allocate(Uf_y(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_));Uf_y=0.0_WP
            allocate(Uf_z(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_));Uf_z=0.0_WP
            allocate(Vf_x(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_));Vf_x=0.0_WP
            allocate(Vf_z(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_));Vf_z=0.0_WP
            allocate(Wf_x(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_));Wf_x=0.0_WP
            allocate(Wf_y(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_));Wf_y=0.0_WP
            allocate(VISCforceX(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_));VISCforceX=0.0_WP
            allocate(VISCforceY(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_));VISCforceY=0.0_WP
            allocate(VISCforceZ(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_));VISCforceZ=0.0_WP

            call this%apply_bcond(dt, scope='cell')

            ! Get face velocities to find viscous stress
            call this%update_faceU(vf,this%U,this%V,this%W,this%Uf,this%Vf,this%Wf)
            call this%update_faceU(vf,this%V,this%W,this%U,Vf_x,Wf_y,Uf_z,use_masks=.false.)
            call this%update_faceU(vf,this%W,this%U,this%V,Wf_x,Uf_y,Vf_z,use_masks=.false.)
            ! Apply boundary conditions to face velocities
            
            call this%apply_bcond(dt, scope='face')

            do k=this%cfg%kmin_,this%cfg%kmax_
               do j=this%cfg%jmin_,this%cfg%jmax_
                 do i=this%cfg%imin_,this%cfg%imax_
                   
                   dMUdx= sum(this%divp_x(:,i,j,k)*this%visc_x(i:i+1,j,k))
                   dMUdy= sum(this%divp_y(:,i,j,k)*this%visc_y(i,j:j+1,k))
                   dMUdz= sum(this%divp_z(:,i,j,k)*this%visc_z(i,j,k:k+1))
      
                   dUdx = sum(this%divp_x(:,i,j,k)*this%Uf(i:i+1,j,k))
                   ...
                   dVdy = sum(this%divp_y(:,i,j,k)*this%Vf(i,j:j+1,k))
                   ...
                   dWdz = sum(this%divp_z(:,i,j,k)*this%Wf(i,j,k:k+1))

                   div2U = 2.0_WP*( &
                     this%divp_x(1,i,j,k)*(this%divp_x(1,i,j,k)*this%Uf(i+1,j,k) + this%divp_x(0,i,j,k)*this%U (i,j,k)) +&
                     this%divp_x(0,i,j,k)*(this%divp_x(1,i,j,k)*this%U (i,j,k)   + this%divp_x(0,i,j,k)*this%Uf(i,j,k)) +&
                     this%divp_y(1,i,j,k)*(this%divp_y(1,i,j,k)*Uf_y   (i,j+1,k) + this%divp_y(0,i,j,k)*this%U (i,j,k)) +&
                     this%divp_y(0,i,j,k)*(this%divp_y(1,i,j,k)*this%U (i,j,k)   + this%divp_y(0,i,j,k)*Uf_y   (i,j,k)) +&
                     this%divp_z(1,i,j,k)*(this%divp_z(1,i,j,k)*Uf_z   (i,j,k+1) + this%divp_z(0,i,j,k)*this%U (i,j,k)) +&
                     this%divp_z(0,i,j,k)*(this%divp_z(1,i,j,k)*this%U (i,j,k)   + this%divp_z(0,i,j,k)*Uf_z   (i,j,k)) )
                   div2V = 2.0_WP*( &
                     this%divp_x(1,i,j,k)*(this%divp_x(1,i,j,k)*Vf_x   (i+1,j,k) + this%divp_x(0,i,j,k)*this%V (i,j,k)) +&
                     this%divp_x(0,i,j,k)*(this%divp_x(1,i,j,k)*this%V (i,j,k)   + this%divp_x(0,i,j,k)*Vf_x   (i,j,k)) +&
                     this%divp_y(1,i,j,k)*(this%divp_y(1,i,j,k)*this%Vf(i,j+1,k) + this%divp_y(0,i,j,k)*this%V (i,j,k)) +&
                     this%divp_y(0,i,j,k)*(this%divp_y(1,i,j,k)*this%V (i,j,k)   + this%divp_y(0,i,j,k)*this%Vf(i,j,k)) +&
                     this%divp_z(1,i,j,k)*(this%divp_z(1,i,j,k)*Vf_z   (i,j,k+1) + this%divp_z(0,i,j,k)*this%V (i,j,k)) +&
                     this%divp_z(0,i,j,k)*(this%divp_z(1,i,j,k)*this%V (i,j,k)   + this%divp_z(0,i,j,k)*Vf_z   (i,j,k)) )
                   div2W = 2.0_WP*( &
                     this%divp_x(1,i,j,k)*(this%divp_x(1,i,j,k)*Wf_x   (i+1,j,k) + this%divp_x(0,i,j,k)*this%W (i,j,k)) +&
                     this%divp_x(0,i,j,k)*(this%divp_x(1,i,j,k)*this%W (i,j,k)   + this%divp_x(0,i,j,k)*Wf_x   (i,j,k)) +&
                     this%divp_y(1,i,j,k)*(this%divp_y(1,i,j,k)*Wf_y   (i,j+1,k) + this%divp_y(0,i,j,k)*this%W (i,j,k)) +&
                     this%divp_y(0,i,j,k)*(this%divp_y(1,i,j,k)*this%W (i,j,k)   + this%divp_y(0,i,j,k)*Wf_y   (i,j,k)) +&
                     this%divp_z(1,i,j,k)*(this%divp_z(1,i,j,k)*this%Wf(i,j,k+1) + this%divp_z(0,i,j,k)*this%W (i,j,k)) +&
                     this%divp_z(0,i,j,k)*(this%divp_z(1,i,j,k)*this%W (i,j,k)   + this%divp_z(0,i,j,k)*this%Wf(i,j,k)) )

                     ! Compute d/dx_j (tau_ij) for i = 1
                     ! (x-direction)
                     VISCforceX(i,j,k)=dMUdx*(dUdx+dUdx) & ! j = 1
                     +dMUdy*(dUdy+dVdx) & ! j = 2
                     +dMUdz*(dUdz+dWdx) & ! j = 3
                     ...
                     ! (y-direction)
                     VISCforceY(i,j,k)=dMUdx*(dVdx+dUdy) & ! j = 1
                     +dMUdy*(dVdy+dVdy) & ! j = 2
                     +dMUdz*(dVdz+dWdy) & ! j = 3
                     ...
                     ! (z-direction)
                     VISCforceZ(i,j,k)=dMUdx*(dWdx+dUdz) & ! j = 1
                     +dMUdy*(dWdy+dVdz) & ! j = 2
                     +dMUdz*(dWdz+dWdz) & ! j = 3
