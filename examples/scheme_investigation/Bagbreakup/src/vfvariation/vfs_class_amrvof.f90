...
contains
   ...
   subroutine transport_flux(this,dt,U,V,W,Uc,Vc,Wc,rho_l,rho_g)
      ...
      recursive function cut_tet_P(tetin,myind) result(myflux)
         implicit none
         real(WP), dimension(3,4), intent(in) :: tetin
         ...
         if      (maxval(myind(1,:))-minval(myind(1,:)).gt.0) then ! Cut by x planes
            dir=1; cut_ind=maxval(myind(1,:)); d(:)=tetin(1,:)-this%cfg%x(cut_ind)
         else if (maxval(myind(2,:))-minval(myind(2,:)).gt.0) then ! Cut by y planes
            dir=2; cut_ind=maxval(myind(2,:)); d(:)=tetin(2,:)-this%cfg%y(cut_ind)
         ...
         do n1=1,cut_ntets(case)
            ...
            if (abs(tet_vol(newtet)).lt.this%vol_epsilon) cycle
            ! Cut by next plane
      ...
      function cut_tet_plic(tetin,i0,j0,k0) result(myflux)
         implicit none
            real(WP), dimension(3,4), intent(in) :: tetin
            ...
            if (this%VF(i0,j0,k0).lt.VFlo) then
               my_vol=abs(tet_vol(tetin)); bary=0.25_WP*(tetin(:,1)+tetin(:,2)+tetin(:,3)+tetin(:,4))
            ...
            do n1=1,cut_nntet(case)-1
               my_vol=abs(tet_vol(vert(:,cut_vtet(:,n1,case))))
