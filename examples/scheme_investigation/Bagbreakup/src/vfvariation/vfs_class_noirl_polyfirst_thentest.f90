...
module vfs_class
   ...
   type :: vfs
      ...
      ! Momentum fluxes of each face
      real(WP), dimension(:,:,:,:), allocatable :: MFX    !< Superficial liquid fluxing velocity
      real(WP), dimension(:,:,:,:), allocatable :: MFY    !< Superficial liquid fluxing velocity
      real(WP), dimension(:,:,:,:), allocatable :: MFZ    !< Superficial liquid fluxing velocity
      ...
      integer, dimension(:,:,:), allocatable :: bandold   !< Band to localize workload around the interface
...
contains
   ...
   subroutine initialize(this,cfg,reconstruction_method,transport_method,name)
      ...
      ! Momentum Fluxes
      allocate(this%MFX(1:3,this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%MFX=0.0_WP
      allocate(this%MFY(1:3,this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%MFY=0.0_WP
      allocate(this%MFZ(1:3,this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%MFZ=0.0_WP
      ...
      allocate(this%bandold(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%bandold=0
   ...
   subroutine initialize_irl(this)
      ...
      this%vol_epsilon=volume_epsilon_factor*this%cfg%min_meshsize**3
   ...
   subroutine advance(this,dt,U,V,W,Uc,Vc,Wc,rho_l,rho_g)
      ...
      select case (this%transport_method)
      case (flux)
         call this%transport_flux_noirl(dt,U,V,W,Uc,Vc,Wc,rho_l,rho_g)
      case (flux_storage)
   ...
   subroutine transport_flux(this,dt,U,V,W,rho_l,rho_g)
      ...
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: W     !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      real(WP), intent(in) :: rho_l,rho_g
      integer :: i,j,k,index,n,iflux,nn
         ...
         do j=this%cfg%jmin_,this%cfg%jmax_+1
            do i=this%cfg%imin_,this%cfg%imax_+1
               ...
               if (minval(abs(this%band(i-1:i,j,k))).le.advect_band) then
                  ...
                  do n=0,8
                     face(:,n+1)=getPt(flux_polyhedron,n)
                  end do
                  SLflux=0.0_WP
                  do n=1,8
                     ...
                     do nn=1,4
                        ...
                        myijk(:,nn)=this%cfg%get_ijk_local(tetra(:,nn),[i,j,k])
                     end do
                     SLflux=SLflux-tet_sign(tetra)*tet2flux(tetra,myijk)
                  end do
                  call construct(this%face_flux(1,i,j,k),[SLflux(1),SLflux(3:5),SLflux(2),SLflux(6:8)])
               ...
               if (minval(abs(this%band(i,j-1:j,k))).le.advect_band) then
                  ...
                  call construct(this%face_flux(2,i,j,k),[SLflux(1),SLflux(3:5),SLflux(2),SLflux(6:8)])
                  ! Store superficial liquid and gas fluxing velocities for momentum solver
               ...
               if (minval(abs(this%band(i,j,k-1:k))).le.advect_band) then
                  ...
                  call construct(this%face_flux(3,i,j,k),[SLflux(1),SLflux(3:5),SLflux(2),SLflux(6:8)])
                  ! Store superficial liquid and gas fluxing velocities for momentum solver
      ...
      recursive function tet2flux(mytet,myind) result(myflux)
         ...
         else ! >>> MODIFICATION: This is no longer the final step.
         ...
         do n1=1,cut_nvert(case)
            ...
            vert_ind(:,4+n1,1)=this%cfg%get_ijk_local(vert(:,4+n1),[i,j,k])
            ! Enforce boundedness
      ...
      function tet2flux_plic(mytet,i0,j0,k0) result(myflux)
         ...
         real(WP), dimension(14) :: myflux
         real(WP), dimension(8, 8) :: octant_fluxes
         integer :: i_octant

         ! Call the new recursive function to get the detailed octant fluxes.
         octant_fluxes = tet2octant_flux(mytet, i0, j0, k0)
         ! Sum the results from all 8 octants to get the total flux for the cell.
         myflux = 0.0_WP
         do i_octant = 1, 8
            myflux(1:8) = myflux(1:8) + octant_fluxes(:, i_octant)
         end do

         ! X momentum Liquid
         myflux(9) =rho_l*(octant_fluxes(1,1)+octant_fluxes(1,3)+octant_fluxes(1,5)+octant_fluxes(1,7))*U(i0  ,j0,k0)&
         &         +rho_l*(octant_fluxes(1,2)+octant_fluxes(1,4)+octant_fluxes(1,6)+octant_fluxes(1,8))*U(i0+1,j0,k0)
         ! Y momentum Liquid
         myflux(10)=rho_l*(octant_fluxes(1,1)+octant_fluxes(1,2)+octant_fluxes(1,5)+octant_fluxes(1,6))*V(i0,j0  ,k0)&
         &         +rho_l*(octant_fluxes(1,3)+octant_fluxes(1,4)+octant_fluxes(1,7)+octant_fluxes(1,8))*V(i0,j0+1,k0)
         ! Z momentum Liquid
         myflux(11)=rho_l*(octant_fluxes(1,1)+octant_fluxes(1,2)+octant_fluxes(1,3)+octant_fluxes(1,4))*W(i0,j0,k0  )&
         &         +rho_l*(octant_fluxes(1,5)+octant_fluxes(1,6)+octant_fluxes(1,7)+octant_fluxes(1,8))*W(i0,j0,k0+1)

         ! X momentum Gas
         myflux(12)=rho_g*(octant_fluxes(2,1)+octant_fluxes(2,3)+octant_fluxes(2,5)+octant_fluxes(2,7))*U(i0  ,j0,k0)&
         &         +rho_g*(octant_fluxes(2,2)+octant_fluxes(2,4)+octant_fluxes(2,6)+octant_fluxes(2,8))*U(i0+1,j0,k0)
         ! Y momentum Gas
         myflux(13)=rho_g*(octant_fluxes(2,1)+octant_fluxes(2,2)+octant_fluxes(2,5)+octant_fluxes(2,6))*V(i0,j0  ,k0)&
         &         +rho_g*(octant_fluxes(2,3)+octant_fluxes(2,4)+octant_fluxes(2,7)+octant_fluxes(2,8))*V(i0,j0+1,k0)
         ! Z momentum Gas
         myflux(14)=rho_g*(octant_fluxes(2,1)+octant_fluxes(2,2)+octant_fluxes(2,3)+octant_fluxes(2,4))*W(i0,j0,k0  )&
         &         +rho_g*(octant_fluxes(2,5)+octant_fluxes(2,6)+octant_fluxes(2,7)+octant_fluxes(2,8))*W(i0,j0,k0+1)

      end function tet2flux_plic

      !> Recursive call to subdivide the tetradhedron into 8 octants
      recursive function tet2octant_flux(mytet, i0, j0, k0) result(octant_fluxes)
         implicit none
         real(WP), dimension(3,4), intent(in) :: mytet
         integer,  intent(in)    :: i0, j0, k0
         ! The result is an array holding the flux for each of the 8 octants.
         real(WP), dimension(8, 8) :: octant_fluxes
         real(WP) :: x_center, y_center, z_center
         real(WP), dimension(4) :: d
         integer :: case, n1, n2, v1, v2, octant_idx
         real(WP) :: denom, mu, my_vol
         real(WP), dimension(3,8) :: vert
         real(WP), dimension(3,4) :: newtet
         real(WP), dimension(3) :: a, b, c, bary
         real(WP), dimension(8, 8) :: sub_fluxes
         real(WP), parameter :: eps = 1.0e-12_WP 
         ! Define the three center-planes of the cell (i0,j0,k0)
         x_center = this%cfg%x(i0) + 0.5_WP * this%cfg%dx(i0)
         y_center = this%cfg%y(j0) + 0.5_WP * this%cfg%dy(j0)
         z_center = this%cfg%z(k0) + 0.5_WP * this%cfg%dz(k0)

         ! Check if the tetrahedron crosses one of the center planes.
         ! The check is now on physical coordinates, not indices.
         if (maxval(mytet(1,:)) > x_center + eps .and. minval(mytet(1,:)) < x_center - eps) then ! Cut by x-center plane
            d(:) = mytet(1,:) - x_center
         else if (maxval(mytet(2,:)) > y_center + eps .and. minval(mytet(2,:)) < y_center - eps) then ! Cut by y-center plane
            d(:) = mytet(2,:) - y_center
         else if (maxval(mytet(3,:)) > z_center + eps .and. minval(mytet(3,:)) < z_center - eps) then ! Cut by z-center plane
            d(:) = mytet(3,:) - z_center
         else
            ! Determine which octant it's in.
            bary = 0.25_WP * (mytet(:,1) + mytet(:,2) + mytet(:,3) + mytet(:,4))
            octant_idx = 1
            if (bary(1) > x_center) octant_idx = octant_idx + 1
            if (bary(2) > y_center) octant_idx = octant_idx + 2
            if (bary(3) > z_center) octant_idx = octant_idx + 4
            ! Initialize the output array.
            octant_fluxes = 0.0_WP
            ! Calculate the flux for this octant-bound tet and place it in the correct slot.
            octant_fluxes(:, octant_idx) = calculate_plic_flux(mytet, i0, j0, k0)
            return
         end if
         ! Find case of cut
         case = 1 + int(0.5_WP+sign(0.5_WP,d(1))) + 2*int(0.5_WP+sign(0.5_WP,d(2))) + 4*int(0.5_WP+sign(0.5_WP,d(3))) + 8*int(0.5_WP+sign(0.5_WP,d(4)))
         ...
         do n1=1,cut_ntets(case)
            ...
            my_vol=abs(a(1)*(b(2)*c(3)-c(2)*b(3))-a(2)*(b(1)*c(3)-c(1)*b(3))+a(3)*(b(1)*c(2)-c(1)*b(2)))/6.0_WP
            if (my_vol.lt.this%vol_epsilon) cycle
            ! Recurse and add the results from the sub-problem.
            sub_fluxes = tet2octant_flux(newtet, i0, j0, k0)
            octant_fluxes = octant_fluxes + sub_fluxes
         end do

      end function tet2octant_flux

      !> Function that cuts a tet by the local PLIC interface
      function calculate_plic_flux(mytet,i0,j0,k0) result(myflux)
         implicit none
            real(WP), dimension(3,4), intent(in) :: mytet
            integer , intent(in)    :: i0,j0,k0
            ...
            if (this%VF(i0,j0,k0).lt.VFlo) then
               ...
               a=vert(:,1)-vert(:,4); b=vert(:,2)-vert(:,4); c=vert(:,3)-vert(:,4)
               my_vol=abs(a(1)*(b(2)*c(3)-c(2)*b(3))-a(2)*(b(1)*c(3)-c(1)*b(3))+a(3)*(b(1)*c(2)-c(1)*b(2)))/6.0_WP
               bary=0.25_WP*(vert(:,1)+vert(:,2)+vert(:,3)+vert(:,4))
      ...
      end function calculate_plic_flux

   end subroutine transport_flux

   !> Perform flux-based transport of VF based on U/V/W and dt
   subroutine transport_flux_noirl(this,dt,U,V,W,Uc,Vc,Wc,rho_l,rho_g)
      use amrvof_geometry, only: tet_map,tet_sign,tet_vol,correct_flux_poly,flux_poly_moments
      implicit none
      class(vfs), intent(inout) :: this
      real(WP), intent(inout) :: dt  !< Timestep size over which to advance
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: U     !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: V     !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: W     !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: Uc    !< Cell-centered x-velocity
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: Vc    !< Cell-centered y-velocity
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: Wc    !< Cell-centered z-velocity
      real(WP), intent(in) :: rho_l,rho_g
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_+1
         do j=this%cfg%jmin_,this%cfg%jmax_+1
            do i=this%cfg%imin_,this%cfg%imax_+1
               ! X flux
               if (minval(abs(this%band(i-1:i,j,k))).le.advect_band) then
                   ! Construct and project face (9-vertex flux polyhedron)
                   ...
                   if (this%cons_correct) call correct_flux_poly(face,U(i,j,k)*dt*this%cfg%dy(j)*this%cfg%dz(k))
                   ! Compute cell indices via floor (safe, no OOB walk)
                   do nn=1,9
                      fijk(1,nn)=floor((face(1,nn)-this%cfg%x(this%cfg%imin_))/(this%cfg%x(this%cfg%imin_+1)-this%cfg%x(this%cfg%imin_)))+this%cfg%imin_
                      fijk(2,nn)=floor((face(2,nn)-this%cfg%y(this%cfg%jmin_))/(this%cfg%y(this%cfg%jmin_+1)-this%cfg%y(this%cfg%jmin_)))+this%cfg%jmin_
                   ...
                   if (bb_pure_liq.or.bb_pure_gas) then
                      ...
                      if (bb_pure_liq) then
                         ...
                         cijk=max([fijk(1,9),fijk(2,9),fijk(3,9)],[this%cfg%imino_,this%cfg%jmino_,this%cfg%kmino_])
                         cijk=min(cijk,[this%cfg%imaxo_,this%cfg%jmaxo_,this%cfg%kmaxo_])
                         SLflux(9 )=rho_l*fvol*Uc(cijk(1),cijk(2),cijk(3))
                         SLflux(10)=rho_l*fvol*Vc(cijk(1),cijk(2),cijk(3))
                         SLflux(11)=rho_l*fvol*Wc(cijk(1),cijk(2),cijk(3))
                      else
                         SLflux(2)=fvol; SLflux(6:8)=fbary
                         cijk=max([fijk(1,9),fijk(2,9),fijk(3,9)],[this%cfg%imino_,this%cfg%jmino_,this%cfg%kmino_])
                         cijk=min(cijk,[this%cfg%imaxo_,this%cfg%jmaxo_,this%cfg%kmaxo_])
                         SLflux(12)=rho_g*fvol*Uc(cijk(1),cijk(2),cijk(3))
                         SLflux(13)=rho_g*fvol*Vc(cijk(1),cijk(2),cijk(3))
                         SLflux(14)=rho_g*fvol*Wc(cijk(1),cijk(2),cijk(3))
                      end if
                   else
                      ! Per-tet decomposition with per-tet purity check
                      do n=1,8
               ...
               if (minval(abs(this%band(i,j-1:j,k))).le.advect_band) then
                   ...
                   face(:,9)=0.25_WP*(face(:,5)+face(:,6)+face(:,7)+face(:,8))
                   ! Solenoidal correction
                   if (this%cons_correct) call correct_flux_poly(face,V(i,j,k)*dt*this%cfg%dx(i)*this%cfg%dz(k))
                   ! Compute cell indices via floor (safe, no OOB walk)
                   do nn=1,9
                      fijk(1,nn)=floor((face(1,nn)-this%cfg%x(this%cfg%imin_))/(this%cfg%x(this%cfg%imin_+1)-this%cfg%x(this%cfg%imin_)))+this%cfg%imin_
                      fijk(2,nn)=floor((face(2,nn)-this%cfg%y(this%cfg%jmin_))/(this%cfg%y(this%cfg%jmin_+1)-this%cfg%y(this%cfg%jmin_)))+this%cfg%jmin_
               ...
               if (minval(abs(this%band(i,j,k-1:k))).le.advect_band) then
                   ...
                   face(:,9)=0.25_WP*(face(:,5)+face(:,6)+face(:,7)+face(:,8))
                   ! Solenoidal correction
                   if (this%cons_correct) call correct_flux_poly(face,W(i,j,k)*dt*this%cfg%dx(i)*this%cfg%dy(j))
                   ! Compute cell indices via floor (safe, no OOB walk)
                   do nn=1,9
                      fijk(1,nn)=floor((face(1,nn)-this%cfg%x(this%cfg%imin_))/(this%cfg%x(this%cfg%imin_+1)-this%cfg%x(this%cfg%imin_)))+this%cfg%imin_
                      fijk(2,nn)=floor((face(2,nn)-this%cfg%y(this%cfg%jmin_))/(this%cfg%y(this%cfg%jmin_+1)-this%cfg%y(this%cfg%jmin_)))+this%cfg%jmin_
      ...
      recursive function cut_tet_P(mytet,myind) result(myflux)
         ...
         integer,  dimension(3,8,2) :: vert_ind
         real(WP) :: mu, bb_vol
         real(WP), dimension(3) :: bb_bary
         real(WP), dimension(3,4) :: newtet
         integer,  dimension(3,4) :: newind
         integer :: bblo(3), bbhi(3), bb_ijk(3)

         myflux=0.0_WP

         ! Bounding-box pure-phase shortcut: skip all cutting if entirely one phase
         bblo=[minval(myind(1,:)),minval(myind(2,:)),minval(myind(3,:))]
         bbhi=[maxval(myind(1,:)),maxval(myind(2,:)),maxval(myind(3,:))]
         if (all(this%VF(bblo(1):bbhi(1),bblo(2):bbhi(2),bblo(3):bbhi(3)).gt.VFhi)) then
            ! Pure liquid
            bb_vol=abs(tetvol(mytet))
            bb_bary=0.25_WP*(mytet(:,1)+mytet(:,2)+mytet(:,3)+mytet(:,4))
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               ! if (sum(mysurf).gt.0.0_WP) this%curv(i,j,k)=2.0_WP
