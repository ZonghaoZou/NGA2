...
module ligament_class
   ...
   type :: ligament
      ...
      type(tpcons)        :: fs    !< Two-phase flow solver
      ...
      real(WP), dimension(:,:,:,:), allocatable :: SR           !< Velocity gradient
...
contains
    ...
    subroutine transfer_drops(this)
        ...
        do m=1,this%ccl%struct(n)%n_
            ...
            dvel(n,:)=dvel(n,:)+this%cfg%vol(i,j,k)*this%vf%VF(i,j,k)*[this%fs%U(i,j,k),this%fs%V(i,j,k),this%fs%W(i,j,k)]
   ...
   subroutine transfer_ligs(this)
      ...
      do m=1,this%ccl_lig%struct(n)%n_
          ...
          lvel(n,:)=lvel(n,:)+this%cfg%vol(i,j,k)*this%vf%VF(i,j,k)*[this%fs%U(i,j,k),this%fs%V(i,j,k),this%fs%W(i,j,k)]
   ...
   subroutine init(this)
      ...
      create_and_initialize_vof: block
         use vfs_class, only: remap,VFlo,VFhi,plicnet,r2pnet,r2p,flux
      ...
      create_flow_solver: block
         ...
         use tpcons_class,      only: dirichlet,clipped_neumann,bcond
         ! Create flow solver
         ...
         ! If use explicit WENO
         ! call this%input%read('Use WENO',this%WENO)
         ! this%vs=ddadi(cfg=this%cfg,name='Velocity',nst=7)
         call this%fs%setup(pressure_solver=this%ps)!,implicit_solver=this%vs)
         ! ! Configure implicit velocity solver
         ! if (.not. this%WENO) then
         !    this%vs=ddadi(cfg=this%cfg,name='Velocity',nst=7)
         !    ! Setup the solver
         !    call this%fs%setup(pressure_solver=this%ps,implicit_solver=this%vs)
         ! else
         !    this%vs=ddadi(cfg=this%cfg,name='Velocity',nst=13)
         !    call this%fs%setup_WENO(pressure_solver=this%ps,implicit_solver=this%vs)
         ! end if
      end block create_flow_solver

      ! Initialize our velocity field
      initialize_velocity: block
         use tpcons_class, only: bcond
         type(bcond), pointer :: mybc
         integer :: n,i,j,k
         ! Initialize density
         this%fs%rho=this%fs%rho_l*this%vf%VF+this%fs%rho_g*(1.0_WP-this%vf%VF); call this%fs%update_faceRHO(vf=this%vf,rho=this%fs%rho)
         ...
         ! call this%fs%update_faceU(this%vf,this%fs%U,this%fs%V,this%fs%W,this%fs%Uf,this%fs%Vf,this%fs%Wf)
         ! call this%fs%apply_bcond(this%time%dt,'face')
         ...
         ! this%fs%P=this%fs%psolv%sol
         ! this%fs%Uf=this%fs%Uf-this%time%dt*this%resU/this%fs%RHOX
         ! this%fs%Vf=this%fs%Vf-this%time%dt*this%resV/this%fs%RHOY
         ! this%fs%Wf=this%fs%Wf-this%time%dt*this%resW/this%fs%RHOZ
         ! call this%fs%get_cell_pgrad(this%vf,this%fs%psolv%sol,this%resU,this%resV,this%resW)
         ! this%fs%U=this%fs%U-this%resU/this%fs%rho; call this%cfg%sync(this%fs%U)
         ! this%fs%V=this%fs%V-this%resV/this%fs%rho; call this%cfg%sync(this%fs%V)
         ! this%fs%W=this%fs%W-this%resW/this%fs%rho; call this%cfg%sync(this%fs%W)
         ! call this%fs%apply_bcond(this%time%dt,'cell')
      ...
      ! ! Create an LES model
      ! create_sgs: block
      !    call this%input%read('Use SGS model',this%use_sgs)
      ...
      handle_restart: block
         ...
         if (this%restarted) then
            ...
            call this%fs%update_faceRHO(vf=this%vf,rho=this%fs%rho)
            ...
            call this%df%pull(name='Uf',var=this%fs%Uf)
            call this%df%pull(name='Vf',var=this%fs%Vf)
            call this%df%pull(name='Wf',var=this%fs%Wf)
            ...
            do n=1,mybc%itr%no_
               i=mybc%itr%map(1,n); j=mybc%itr%map(2,n); k=mybc%itr%map(3,n)
               this%fs%Uf(i,j,k)=1.0_WP
               this%fs%U(i,j,k)=0.5_WP
               this%fs%U(i-1,j,k)=0.5_WP
            end do
            ! Apply all other boundary conditions
            call this%fs%apply_bcond(this%time%dt,'cell')
            call this%fs%apply_bcond(this%time%dt,'face')
            ...
            call this%df%initialize(pg=this%cfg,iopartition=iopartition,filename=trim(this%cfg%name),nval=2,nvar=18)
            this%df%valname=['t ','dt']
            this%df%varname=['U   ','V   ','W   ','Uf  ','Vf  ','Wf  ','P   ','Pjx ','Pjy ','Pjz ','P11 ','P12 ','P13 ','P14 ','P21 ','P22 ','P23 ','P24 ']
         end if
      ...
      create_smesh: block
         ...
         this%smesh%varname(1)='thickness'
         this%smesh%varname(2)='fluxX1'
         this%smesh%varname(3)='fluxX2'
         this%smesh%varname(4)='fluxX3'
         this%smesh%varname(5)='fluxY1'
         this%smesh%varname(6)='fluxY2'
         this%smesh%varname(7)='fluxY3'
         this%smesh%varname(8)='fluxZ1'
         this%smesh%varname(9)='fluxZ2'
         this%smesh%varname(10)='fluxZ3'
               ...
               do i=this%vf%cfg%imin_,this%vf%cfg%imax_
                  ...
                  do nplane=1,getNumberOfPlanes(this%vf%liquid_gas_interface(i,j,k))
                     if (getNumberOfVertices(this%vf%interface_polygon(nplane,i,j,k)).gt.0) then
                        np=np+1; 
                        this%smesh%var(1,np)=this%vf%thickness(i,j,k)
                        this%smesh%var(2,np)=this%fs%fluxR1(i,j,k)
                        this%smesh%var(3,np)=this%fs%fluxR2(i,j,k)
                        this%smesh%var(4,np)=this%fs%fluxR3(i,j,k)
                        this%smesh%var(5,np)=this%fs%fluxR4(i,j,k)
                        this%smesh%var(6,np)=this%fs%fluxR5(i,j,k)
                        this%smesh%var(7,np)=this%fs%fluxR6(i,j,k)
                        this%smesh%var(8,np)=this%fs%fluxR7(i,j,k)
                        this%smesh%var(9,np)=this%fs%fluxR8(i,j,k)
                        this%smesh%var(10,np)=this%fs%fluxR9(i,j,k)
                        ...
                        ! this%smesh%var(9,np)=this%fs%indicator(i,j,k)
                     end if
                  end do
               end do
            end do
   ...
   subroutine step(this)
      ...
      if (this%use_drop_transfer.or.this%use_film_transfer.or.this%use_lig_transfer) then
        ...
        call this%lp%advance(dt=this%time%dt,U=this%fs%Uf,V=this%fs%Vf,W=this%fs%Wf,rho=this%resU,visc=this%resV)
     end if

     ! Remember old VOF
      this%vf%VFold=this%vf%VF;this%fs%rhoold=this%fs%rho
      this%vf%bandold=this%vf%band
      ...
      call this%vf%advance(dt=this%time%dt,U=this%fs%Uf,V=this%fs%Vf,W=this%fs%Wf,Uc=this%fs%U,Vc=this%fs%V,Wc=this%fs%W,rho_l=this%fs%rho_l,rho_g=this%fs%rho_g)
      ...
      !       call this%fs%get_strainrateUmid(this%SR)
      !       ! call this%sgs%get_visc(type=vreman,dt=this%time%dt,rho=this%resU,gradu=this%gradU)
      !       call this%sgs%get_visc(type=dynamic_smag,dt=this%time%dt,rho=this%resU,Ui=this%Ui,Vi=this%Vi,Wi=this%Wi,SR=this%SR,gradu=this%gradU)
      ...
      do while (this%time%it.le.this%time%itmax)
         ...
         call this%fs%get_dmomdt(this%vf,this%resU,this%resV,this%resW)
         ! Add momentum source terms
         call this%fs%addsrc_gravity(this%resU,this%resV,this%resW)
         ! Assemble explicit residual
         this%resU=-2.0_WP*this%fs%rho*this%fs%U+(this%fs%rho+this%fs%rhoold)*this%fs%Uold+this%time%dt*this%resU
         this%resV=-2.0_WP*this%fs%rho*this%fs%V+(this%fs%rho+this%fs%rhoold)*this%fs%Vold+this%time%dt*this%resV
         this%resW=-2.0_WP*this%fs%rho*this%fs%W+(this%fs%rho+this%fs%rhoold)*this%fs%Wold+this%time%dt*this%resW
         
         ! Form implicit residuals
         call this%fs%solve_implicit(this%vf,this%time%dt,this%resU,this%resV,this%resW)
         ! Compute predictor U
         ...
         call this%fs%update_laplacian()
         call this%fs%update_faceU(this%vf,this%fs%U,this%fs%V,this%fs%W,this%fs%Uf,this%fs%Vf,this%fs%Wf)
         call this%fs%update_pgrad_all(this%vf,this%time%dt)
         ...
         this%fs%Vf=this%fs%Vf-this%time%dt*this%resV/this%fs%RHOY
         this%fs%Wf=this%fs%Wf-this%time%dt*this%resW/this%fs%RHOZ
         call this%fs%get_cell_pgrad(this%vf,this%fs%psolv%sol,this%resU,this%resV,this%resW,.true.)
      ...
      if (this%save_evt%occurs()) then
         save_restart: block
            ...
            call this%df%push(name='Uf',var=this%fs%Uf )
            call this%df%push(name='Vf',var=this%fs%Vf )
            call this%df%push(name='Wf',var=this%fs%Wf )
