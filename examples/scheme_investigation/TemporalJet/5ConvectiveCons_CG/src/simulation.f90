...
contains
   ...
   subroutine simulation_init
      ...
      create_and_initialize_vof: block
         ...
         use vfs_class, only: VFlo,plicnet,remap,flux
         integer :: i,j,k
         ! Create a VOF solver
      ...
      initialize_velocity: block
         ...
         if (isPipe) then
            ...
            ! Interpolate the cell face velocities to cell center
            do k=fs%cfg%kmin_,fs%cfg%kmax_; do j=fs%cfg%jmin_,fs%cfg%jmax_; do i=fs%cfg%imin_,fs%cfg%imax_
               fs%U(i,j,k)=sum(fs%itpu_x(:,i,j,k)*pipe%fs%U(i:i+1,j,k))
               fs%V(i,j,k)=sum(fs%itpv_y(:,i,j,k)*pipe%fs%V(i,j:j+1,k))
               fs%W(i,j,k)=sum(fs%itpw_z(:,i,j,k)*pipe%fs%W(i,j,k:k+1))
            end do; end do; end do
         else
   ...
   subroutine simulation_run
      ...
      do while (.not.time%done())
         ...
         ! Turbulence modeling Check this
         if (use_sgs) then
            sgs_modeling: block
               ...
               do k=fs%cfg%kmino_+1,fs%cfg%kmaxo_; do j=fs%cfg%jmino_+1,fs%cfg%jmaxo_; do i=fs%cfg%imino_+1,fs%cfg%imaxo_
                  ...
                  fs%visc_x(i,j,k)=fs%visc_x(i,j,k)+sum(fs%itpr_x(:,i,j,k)*sgs%visc(i-1:i,j,k))
                  fs%visc_y(i,j,k)=fs%visc_y(i,j,k)+sum(fs%itpr_y(:,i,j,k)*sgs%visc(i,j-1:j,k))
                  fs%visc_z(i,j,k)=fs%visc_z(i,j,k)+sum(fs%itpr_z(:,i,j,k)*sgs%visc(i,j,k-1:k))
               end do; end do; end do
