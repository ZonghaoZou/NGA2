 ...
 contains
    ...
    subroutine initialize(this,cfg,reconstruction_method,transport_method,name)
       ...
       case (r2p,r2pnet)
          ...
          allocate(this%norm_pos(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%norm_pos=0.0_WP
          allocate(this%norm_neg(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%norm_neg=0.0_WP
          ...
          do j=this%cfg%jmin_,this%cfg%jmax_+1
             do i=this%cfg%imin_,this%cfg%imax_+1
                ...
                if (minval(abs(this%band(i-1:i,j,k))).le.advect_band) then
                   ...
                   ! if (crude_VF.lt.0.0_WP) then
                      ...
                      do n=0,getSize(detailed_face_flux)-1
                         ...
                         if (i.eq.30.and.j.eq.72.and.k.eq.64) then
                            print * , "Inside VOF, Vtmp is", Vtmp, "lvol is:", lvol, 'two V', V(ind(1),ind(2),ind(3)), V(ind(1),ind(2)+1,ind(3))
                         end if
                      end do
                   ...
                   ! end if
                      if (i.eq.30.and.j.eq.72.and.k.eq.64) then
                         print * , "Inside VOF, totallvol", getVolumePtr(this%face_flux(1,i,j,k),0)
                         print *, "Inside VOF, totalmomentum", lmom+gmom
                      end if
                ...
                if (minval(abs(this%band(i,j,k-1:k))).le.advect_band) then
                   ...
                   !       gvol=(1.0_WP-crude_VF)*vol_now; gbar=ctr_now
                   !       ind=this%cfg%get_ijk_local(ctr_now,[i,j,k])
                   !       if (ind(1).eq.30 .or.ind(1)+1.eq.30 .and. ind(1)+1.gt.this%cfg%imaxo_) then
                   !          print *, "Centroid", ctr_now
                   !          print *, "local index", ind
                   !          print *, "Cell X range", this%cfg%x(ind(1)-1),this%cfg%x(ind(1)),this%cfg%x(ind(1)+1)
                   !          print *, "processor range", this%cfg%imino_,this%cfg%imaxo_
                   !          print *, "flux range", this%cfg%imin_,this%cfg%imax_+1
                   !          print *, "flux face index", i,j,k 
                   !          do n=0,8
                   !             face(:,n+1)=getPt(flux_polyhedron,n)
                   !             print *, face(:,n+1)
                   !          end do
                   !          print *, "cetnroid again:", calculateCentroid(flux_polyhedron)
                   !       end if
                   !       ! if (ind(1).lt.this%cfg%imino_) ind(1)=
                   !       Utmp=(U(ind(1),ind(2),ind(3))+U(ind(1)+1,ind(2),ind(3)))*0.5_WP
                   !       Vtmp=(V(ind(1),ind(2),ind(3))+V(ind(1),ind(2)+1,ind(3)))*0.5_WP
                   !       Wtmp=(W(ind(1),ind(2),ind(3))+W(ind(1),ind(2),ind(3)+1))*0.5_WP
                   !       lmom=lmom+lvol*rho_l*[Utmp,Vtmp,Wtmp]
    ...
    subroutine build_r2pnet(this)
       ...
       do k=this%cfg%kmin_,this%cfg%kmax_; do j=this%cfg%jmin_,this%cfg%jmax_; do i=this%cfg%imin_,this%cfg%imax_
          ...
          if ((this%norm_pos(i,j,k)-this%norm_neg(i,j,k)).ge.0.5_WP.or.(((this%norm_pos(i,j,k)-this%norm_neg(i,j,k)).lt.0.5_WP).and.(this%norm_pos(i,j,k)+this%norm_neg(i,j,k).lt.0.8_WP))) then
             ! PLICNET
             ! Liquid-gas symmetry
