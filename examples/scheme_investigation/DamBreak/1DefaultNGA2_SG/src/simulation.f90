...
!> Dam break — Scheme 1: Default NGA2 (Staggered Grid)
!>
...
contains   
   ...
   contains
      ...
      !> G > 0 in liquid (water), G < 0 in gas (air)
      !> VF=1 → liquid (water), VF=0 → gas (air)
      function levelset_dambreak(xyz,t) result(G)
         implicit none
         real(WP), dimension(3),intent(in) :: xyz
         real(WP), intent(in) :: t
         real(WP) :: G
         ! Signed distance: positive inside the rectangle
   ...
   !> Front: rightmost x-position of interface polygon centroid
   !> Column height: highest y-position of interface polygon centroid
   subroutine get_front()
               ...
               if (vf%VF(i,j,k).gt.VFlo .and. vf%VF(i,j,k).lt.VFhi) then
                  do nplane=1,getNumberOfPlanes(vf%liquid_gas_interface(i,j,k))
                     if (getNumberOfVertices(vf%interface_polygon(nplane,i,j,k)).gt.0) then
                        ...
                        ! Track max x of interface (surge front)
                        if (ploc(1) .gt. front_x) front_x = ploc(1)
                        ! Track max y of interface (column height)
      ...
      ! Non-dimensional quantities (Martin & Moyce 1952)
