...
module nozzle_class
   ...
   real(WP), parameter :: nspread=1.0_WP            !< Spread IB velocity over a few cells (is it needed?)
   ...
   subroutine simulation_init(this)
      ...
      restart_and_save: block
         ...
         if (this%restarted) then
            ! Read in the file
            call this%df%initialize(pg=this%cfg,iopartition=iopartition,fdata='restart/data_nozzle_'//trim(timestamp))
         else
      ...
      set_ib_velocity: block
         ...
         call this%input%read('Swirl flow rate (SLPM)',Qswirl)
         ...
         do k=this%cfg%kmino_,this%cfg%kmaxo_
            do j=this%cfg%jmino_,this%cfg%jmaxo_
               do i=this%cfg%imino_,this%cfg%imaxo_
                  ...
                  &    abs(this%cfg%ym(j)+sidewall).lt.nspread*this%cfg%min_meshsize) this%Vib(i,j,k)=+Uaxial
                  ...
                  &    abs(this%cfg%zm(k)+sidewall).lt.nspread*this%cfg%min_meshsize) this%Wib(i,j,k)=+Uaxial
                  ...
                  &    abs(this%cfg%ym(j)+sidewall).lt.nspread*this%cfg%min_meshsize) this%Vib(i,j,k)=+Uswirl
                  ...
                  &    abs(this%cfg%zm(k)+sidewall).lt.nspread*this%cfg%min_meshsize) this%Wib(i,j,k)=+Uswirl
         ...
         ! Compute IB mass source
		   do k=this%fs%cfg%kmin_,this%fs%cfg%kmax_
               ...
               this%srcM(i,j,k)=this%fs%rho*(1.0_WP-this%cfg%VF(i,j,k))*& 
               & (sum(this%fs%divp_x(:,i,j,k)*[sum(this%fs%itpr_x(:,i,j,k)*this%Uib(i-1:i,j,k)),sum(this%fs%itpr_x(:,i+1,j,k)*this%Uib(i:i+1,j,k))])+&
               &  sum(this%fs%divp_y(:,i,j,k)*[sum(this%fs%itpr_y(:,i,j,k)*this%Vib(i,j-1:j,k)),sum(this%fs%itpr_y(:,i,j+1,k)*this%Vib(i,j:j+1,k))])+&
               &  sum(this%fs%divp_z(:,i,j,k)*[sum(this%fs%itpr_z(:,i,j,k)*this%Wib(i,j,k-1:k)),sum(this%fs%itpr_z(:,i,j,k+1)*this%Wib(i,j,k:k+1))]))
					end do
				end do
			end do
            ...
            do k=this%cfg%kmin_,this%cfg%kmax_+1
               do j=this%cfg%jmin_,this%cfg%jmax_+1
                  do i=this%cfg%imin_,this%cfg%imax_+1
                     this%fs%Uf=sum(this%fs%itpr_x(:,i,j,k)*this%fs%U(i-1:i,j,k))
                     this%fs%Vf=sum(this%fs%itpr_y(:,i,j,k)*this%fs%V(i,j-1:j,k))
                     this%fs%Wf=sum(this%fs%itpr_z(:,i,j,k)*this%fs%W(i,j,k-1:k))
                  end do 
               end do 
            end do 

            call this%cfg%sync(this%fs%Uf)
            call this%cfg%sync(this%fs%Vf)
            call this%cfg%sync(this%fs%Wf)
      ...
      do while (this%time%it.le.this%time%itmax)
         ...
         ibforcing: block
            ...
            do k=this%fs%cfg%kmin_,this%fs%cfg%kmax_
               do j=this%fs%cfg%jmin_,this%fs%cfg%jmax_
                  do i=this%fs%cfg%imin_,this%fs%cfg%imax_
                     if (this%fs%mask(i,j,k).eq.0) then
                        this%fs%U(i,j,k)=this%cfg%VF(i,j,k)*this%fs%U(i,j,k)+(1.0_WP-this%cfg%VF(i,j,k))*this%Uib(i,j,k)
                        this%fs%V(i,j,k)=this%cfg%VF(i,j,k)*this%fs%V(i,j,k)+(1.0_WP-this%cfg%VF(i,j,k))*this%Vib(i,j,k)
                        this%fs%W(i,j,k)=this%cfg%VF(i,j,k)*this%fs%W(i,j,k)+(1.0_WP-this%cfg%VF(i,j,k))*this%Wib(i,j,k)
                     end if
                  end do
               end do
            end do
