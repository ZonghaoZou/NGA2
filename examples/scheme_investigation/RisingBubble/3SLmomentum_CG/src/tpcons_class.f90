...
module tpcons_class
   ...
   integer, parameter, public :: noslip=7            !< No-slip condition
...
contains
   ...
   subroutine add_bcond(this,name,type,locator,face,dir,canCorrect)
      ...
      case (dirichlet, noslip) ! Added 'wall' here as it implies fixed value
   ...
   subroutine apply_bcond(this,dt,scope)
      ...
      do while (associated(my_bc))
         ...
         if (my_bc%itr%amIn) then
            ...
            case (neumann, clipped_neumann, slip, noslip, convective)
               ! =========================================================
               ...
               if (do_cell) then
                  ...
                  ! Special handling for No Slip:
                  ! Reflect (flip sign) ALL components (Normal + Tangential).
                  ! This ensures U_face = (U_in + U_ghost)/2 = 0.
                  if (my_bc%type .eq. noslip) then
                     flip_u = -1.0_WP
                     flip_v = -1.0_WP
                     flip_w = -1.0_WP
                  end if
               ...
               if (do_face) then
                  ...
                  case ('x')
                     do n = 1, my_bc%itr%n_
                        ...
                        if (my_bc%type == slip .or. my_bc%type == noslip) then
   ...
   subroutine apply_bcond_facepressure(this,P,Px,Py,Pz)
      ...
      do while (associated(my_bc))
         ...
         if (my_bc%itr%amIn) then
            ...
            ! =======================================================
            ! FREE-SLIP / NO-SLIP: Enforce Hydrostatic Balance (P_face = P_cell + rho*g*h)
            ! =======================================================
            case (slip, noslip)                    
              do n=1,my_bc%itr%n_
                i=my_bc%itr%map(1,n); j=my_bc%itr%map(2,n); k=my_bc%itr%map(3,n)
