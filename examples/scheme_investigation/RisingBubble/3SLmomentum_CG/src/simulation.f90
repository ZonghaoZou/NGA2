...
!> Scheme 3: SL Momentum (Collocated Grid)
!>
...
contains
   ...
   subroutine simulation_init
      ...
      create_flow_solver: block
         ...
         use tpcons_class,    only: noslip,slip
         ! Create flow solver
         ...
         call fs%add_bcond(name='yp_wall',type=noslip,face='y',dir=+1,canCorrect=.false.,locator=yp_locator)
         call fs%add_bcond(name='ym_wall',type=noslip,face='y',dir=-1,canCorrect=.false.,locator=ym_locator)
         ! Free-slip walls at left and right (x-direction)
   ...
   subroutine get_stats()
      ...
      real(WP) :: perimeter, d_a, dz
      
      ! ---- Rise velocity and Center of mass ----
      ! For collocated scheme, use cell-centered V (fs%V)
      gas_vol     = 0.0_WP
      gas_vel_sum = 0.0_WP
      gas_ycm_sum = 0.0_WP
      ...
      do k=vf%cfg%kmin_,vf%cfg%kmax_
         do j=vf%cfg%jmin_,vf%cfg%jmax_
            do i=vf%cfg%imin_,vf%cfg%imax_
               ...
               gas_vel_sum = gas_vel_sum + (1.0_WP - vf%VF(i,j,k)) * fs%V(i,j,k) * vf%cfg%vol(i,j,k)
