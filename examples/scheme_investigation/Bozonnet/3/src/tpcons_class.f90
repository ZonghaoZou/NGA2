...
module tpcons_class
   ...
   type :: tpcons
      ...
      real(WP), dimension(:,:,:), allocatable :: Ufold    !< Uf velocity array
      real(WP), dimension(:,:,:), allocatable :: Vfold    !< Vf velocity array
      real(WP), dimension(:,:,:), allocatable :: Wfold    !< Wf velocity array
      ! Flow divergence
...
contains
   ...
   subroutine initialize(this,cfg,name)
      ...
      allocate(this%Ufold(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%Ufold=0.0_WP
      ...
      allocate(this%Wfold(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%Wfold=0.0_WP
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_+1
        do j=this%cfg%jmin_,this%cfg%jmax_+1
           do i=this%cfg%imin_,this%cfg%imax_+1
              ! Linear interpolation in x (walls only, not Dirichlet ghosts)
              if (this%mask(i,j,k).eq.0.and.this%mask(i-1,j,k).eq.1) this%itpr_x(:,i,j,k)=[0.0_WP,1.0_WP]
              if (this%mask(i,j,k).eq.1.and.this%mask(i-1,j,k).eq.0) this%itpr_x(:,i,j,k)=[1.0_WP,0.0_WP]
              ! Linear interpolation in y (walls only, not Dirichlet ghosts)
              if (this%mask(i,j,k).eq.0.and.this%mask(i,j-1,k).eq.1) this%itpr_y(:,i,j,k)=[0.0_WP,1.0_WP]
              if (this%mask(i,j,k).eq.1.and.this%mask(i,j-1,k).eq.0) this%itpr_y(:,i,j,k)=[1.0_WP,0.0_WP]
              ! Linear interpolation in z (walls only, not Dirichlet ghosts)
              if (this%mask(i,j,k).eq.0.and.this%mask(i,j,k-1).eq.1) this%itpr_z(:,i,j,k)=[0.0_WP,1.0_WP]
              if (this%mask(i,j,k).eq.1.and.this%mask(i,j,k-1).eq.0) this%itpr_z(:,i,j,k)=[1.0_WP,0.0_WP]
           end do
        end do
      end do
   ...
   subroutine add_bcond(this,name,type,locator,face,dir,canCorrect)
      ...
      case (clipped_neumann)
         ! No masking needed. Cell-centered velocity clipping is handled
         ! in apply_bcond. We do NOT modify itpr_x here because it is
         ! shared by density interpolation, viscous stress, and pressure
         ! solver — making it one-sided destabilizes the discretization.
      case (slip)
         ! do n = 1, new_bc%itr%n_
         !    i = new_bc%itr%map(1,n); j = new_bc%itr%map(2,n); k = new_bc%itr%map(3,n)
         !    select case (new_bc%face)
         !    case ('x'); this%umask(i,j,k) = 2
         !    case ('y'); this%vmask(i,j,k) = 2
         !    case ('z'); this%wmask(i,j,k) = 2
         !    end select
         ...
         if (my_bc%itr%amIn) then
            ...
            case (neumann, clipped_neumann, slip, convective)
               ...
               if (do_cell) then
                  ...
                  ! 3. Reflect cell-centered normal velocity to prevent backflow
                  ! Instead of clipping to zero, negate the interior value so that
                  ! the face interpolation (itpr ~ [0.5, 0.5]) gives exactly zero:
                  !   U_face = 0.5*U(int) + 0.5*(-U(int)) = 0
                  if (my_bc%type .eq. clipped_neumann) then
                     do n = 1, my_bc%itr%n_
                        i = my_bc%itr%map(1,n); j = my_bc%itr%map(2,n); k = my_bc%itr%map(3,n)
                        select case (my_bc%face)
                        case ('x')
                           if (this%U(i-shift(1,my_bc%dir_lhs), j-shift(2,my_bc%dir_lhs), k-shift(3,my_bc%dir_lhs)) * my_bc%rdir .lt. 0.0_WP) &
                              this%U(i-shift(1,my_bc%dir_lhs), j-shift(2,my_bc%dir_lhs), k-shift(3,my_bc%dir_lhs)) = &
                             -this%U(i-shift(1,my_bc%dir_rhs), j-shift(2,my_bc%dir_rhs), k-shift(3,my_bc%dir_rhs))
                        case ('y')
                           if (this%V(i-shift(1,my_bc%dir_lhs), j-shift(2,my_bc%dir_lhs), k-shift(3,my_bc%dir_lhs)) * my_bc%rdir .lt. 0.0_WP) &
                              this%V(i-shift(1,my_bc%dir_lhs), j-shift(2,my_bc%dir_lhs), k-shift(3,my_bc%dir_lhs)) = &
                             -this%V(i-shift(1,my_bc%dir_rhs), j-shift(2,my_bc%dir_rhs), k-shift(3,my_bc%dir_rhs))
                        case ('z')
                           if (this%W(i-shift(1,my_bc%dir_lhs), j-shift(2,my_bc%dir_lhs), k-shift(3,my_bc%dir_lhs)) * my_bc%rdir .lt. 0.0_WP) &
                              this%W(i-shift(1,my_bc%dir_lhs), j-shift(2,my_bc%dir_lhs), k-shift(3,my_bc%dir_lhs)) = &
                             -this%W(i-shift(1,my_bc%dir_rhs), j-shift(2,my_bc%dir_rhs), k-shift(3,my_bc%dir_rhs))
                        end select
                     end do
                  end if
               end if
         ...
         if (my_bc%itr%amIn) then
            ...
            case (slip)                    
              do n=1,my_bc%itr%n_
                ...
                select case (my_bc%face)
                case ('x')
                  ! Use INTERIOR cell (i-dir), not ghost cell (i)
                  dist = 0.5_WP * real(my_bc%dir, WP) * this%cfg%dx(i-my_bc%dir)
                  hydro_inc = this%rho(i-my_bc%dir,j,k) * this%gravity(1) * dist
                  
                  Px(i-shift(1,my_bc%fdir_lhs),j,k) = P(i-my_bc%dir,j,k) + hydro_inc
                  
                case ('y')
                  ! Use INTERIOR cell (j-dir), not ghost cell (j)
                  dist = 0.5_WP * real(my_bc%dir, WP) * this%cfg%dy(j-my_bc%dir)
                  hydro_inc = this%rho(i,j-my_bc%dir,k) * this%gravity(2) * dist
                  
                  Py(i,j-shift(2,my_bc%fdir_lhs),k) = P(i,j-my_bc%dir,k) + hydro_inc
                  
                case ('z')
                  ! Use INTERIOR cell (k-dir), not ghost cell (k)
                  dist = 0.5_WP * real(my_bc%dir, WP) * this%cfg%dz(k-my_bc%dir)
                  hydro_inc = this%rho(i,j,k-my_bc%dir) * this%gravity(3) * dist
                  
                  Pz(i,j,k-shift(3,my_bc%fdir_lhs)) = P(i,j,k-my_bc%dir) + hydro_inc
   ...
   subroutine get_dmomdt(this,vf,drhoUdt,drhoVdt,drhoWdt)
      ...
      ! call this%get_cell_pgrad(vf,this%P,PgradX,PgradY,PgradZ,.false.)
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_+1
         do j=this%cfg%jmin_,this%cfg%jmax_+1
            do i=this%cfg%imin_,this%cfg%imax_+1
               ...
               ! X Viscous Flux (Normal Stress: Keep divu_x)
               FX(i,j,k)=FX(i,j,k)+this%visc_x(i,j,k)*(sum(this%divu_x(:,i,j,k)*this%U(i-1:i,j,k))+sum(this%divu_x(:,i,j,k)*this%U(i-1:i,j,k)))

               ! Y Viscous Flux (Shear Stress: Swap divv_y for grdu_y, and divu_x for grdv_x)
               FY(i,j,k)=FY(i,j,k)+this%visc_y(i,j,k)*(sum(this%grdu_y(:,i,j,k)*this%U(i,j-1:j,k))+sum(this%itpr_y(:,i,j,k)* &
               &[0.5_WP*(sum(this%grdv_x(:,i,j-1,k)*this%V(i-1:i,j-1,k))+sum(this%grdv_x(:,i+1,j-1,k)*this%V(i:i+1,j-1,k))), & 
               & 0.5_WP*(sum(this%grdv_x(:,i,j  ,k)*this%V(i-1:i,j  ,k))+sum(this%grdv_x(:,i+1,j  ,k)*this%V(i:i+1,j  ,k)))]))

               ! Z Viscous Flux (Shear Stress: Swap divw_z for grdu_z, and divu_x for grdw_x)
               FZ(i,j,k)=FZ(i,j,k)+this%visc_z(i,j,k)*(sum(this%grdu_z(:,i,j,k)*this%U(i,j,k-1:k))+sum(this%itpr_z(:,i,j,k)* &
               &[0.5_WP*(sum(this%grdw_x(:,i,j,k-1)*this%W(i-1:i,j,k-1))+sum(this%grdw_x(:,i+1,j,k-1)*this%W(i:i+1,j,k-1))), & 
               & 0.5_WP*(sum(this%grdw_x(:,i,j,k  )*this%W(i-1:i,j,k  ))+sum(this%grdw_x(:,i+1,j,k  )*this%W(i:i+1,j,k  )))]))
            end do 
         end do 
      end do
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_+1
         do j=this%cfg%jmin_,this%cfg%jmax_+1
            do i=this%cfg%imin_,this%cfg%imax_+1
               ...
               ! X Viscous Flux (Shear Stress: Swap divu_x for grdv_x, and divv_y for grdu_y)
               FX(i,j,k)=FX(i,j,k)+this%visc_x(i,j,k)*(sum(this%grdv_x(:,i,j,k)*this%V(i-1:i,j,k))+sum(this%itpr_x(:,i,j,k)* &
               &[0.5_WP*(sum(this%grdu_y(:,i-1,j,k)*this%U(i-1,j-1:j,k))+sum(this%grdu_y(:,i-1,j+1,k)*this%U(i-1,j:j+1,k))), & 
               & 0.5_WP*(sum(this%grdu_y(:,i  ,j,k)*this%U(i  ,j-1:j,k))+sum(this%grdu_y(:,i  ,j+1,k)*this%U(i  ,j:j+1,k)))]))

               ! Y Viscous Flux (Normal Stress: Keep divv_y)
               FY(i,j,k)=FY(i,j,k)+this%visc_y(i,j,k)*(sum(this%divv_y(:,i,j,k)*this%V(i,j-1:j,k))+sum(this%divv_y(:,i,j,k)*this%V(i,j-1:j,k)))

               ! Z Viscous Flux (Shear Stress: Swap divw_z for grdv_z, and divv_y for grdw_y)
               FZ(i,j,k)=FZ(i,j,k)+this%visc_z(i,j,k)*(sum(this%grdv_z(:,i,j,k)*this%V(i,j,k-1:k))+sum(this%itpr_z(:,i,j,k)* &
               &[0.5_WP*(sum(this%grdw_y(:,i,j,k-1)*this%W(i,j-1:j,k-1))+sum(this%grdw_y(:,i,j+1,k-1)*this%W(i,j:j+1,k-1))), & 
               & 0.5_WP*(sum(this%grdw_y(:,i,j,k  )*this%W(i,j-1:j,k  ))+sum(this%grdw_y(:,i,j+1,k  )*this%W(i,j:j+1,k  )))]))
            end do 
         end do 
      end do
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_+1
         do j=this%cfg%jmin_,this%cfg%jmax_+1
            do i=this%cfg%imin_,this%cfg%imax_+1
               ...
               ! X Viscous Flux
               FX(i,j,k)=FX(i,j,k)+this%visc_x(i,j,k)*(sum(this%grdw_x(:,i,j,k)*this%W(i-1:i,j,k))+sum(this%itpr_x(:,i,j,k)* &
               &[0.5_WP*(sum(this%grdu_z(:,i-1,j,k)*this%U(i-1,j,k-1:k))+sum(this%grdu_z(:,i-1,j,k+1)*this%U(i-1,j,k:k+1))), & 
               & 0.5_WP*(sum(this%grdu_z(:,i  ,j,k)*this%U(i  ,j,k-1:k))+sum(this%grdu_z(:,i  ,j,k+1)*this%U(i  ,j,k:k+1)))]))
               ! Y Viscous Flux
               FY(i,j,k)=FY(i,j,k)+this%visc_y(i,j,k)*(sum(this%grdw_y(:,i,j,k)*this%W(i,j-1:j,k))+sum(this%itpr_y(:,i,j,k)* &
               &[0.5_WP*(sum(this%grdv_z(:,i,j-1,k)*this%V(i,j-1,k-1:k))+sum(this%grdv_z(:,i,j-1,k+1)*this%V(i,j-1,k:k+1))), & 
               & 0.5_WP*(sum(this%grdv_z(:,i,j  ,k)*this%V(i,j  ,k-1:k))+sum(this%grdv_z(:,i,j  ,k+1)*this%V(i,j  ,k:k+1)))]))
               ! Z Viscous Flux
   ...
   subroutine get_cell_pgrad(this,vf,P,Pgradx,Pgrady,Pgradz,is_correction)
      use vfs_class, only: vfs
      implicit none
      class(tpcons), intent(inout) :: this
      class(vfs), intent(in) :: vf
            ...
            do j=this%cfg%jmin_,this%cfg%jmax_+1
               do i=this%cfg%imin_,this%cfg%imax_+1
                  ...
                  if (sum(rho_f).gt.0.0_WP) then
                     if (this%mask(i-1,j,k).gt.0.or.this%mask(i,j,k).gt.0) then
                        ! Near masked cell (wall or Dirichlet ghost) -- use density-weighted only to avoid ghost pressure
                        PX(i,j,k)=sum(this%itpr_x(:,i,j,k)*rho_f*P(i-1:i,j,k))/(sum(this%itpr_x(:,i,j,k)*rho_f) + tiny(1.0_WP))
                     else
                  ...
                  if (sum(rho_f).gt.0.0_WP) then
                     if (this%mask(i,j-1,k).gt.0.or.this%mask(i,j,k).gt.0) then
                        PY(i,j,k)=sum(this%itpr_y(:,i,j,k)*rho_f*P(i,j-1:j,k))/(sum(this%itpr_y(:,i,j,k)*rho_f) + tiny(1.0_WP))
                     else
                  ...
                  if (sum(rho_f).gt.0.0_WP) then
                     if (this%mask(i,j,k-1).gt.0.or.this%mask(i,j,k).gt.0) then
                        PZ(i,j,k)=sum(this%itpr_z(:,i,j,k)*rho_f*P(i,j,k-1:k))/(sum(this%itpr_z(:,i,j,k)*rho_f) + tiny(1.0_WP))
                     else
   ...
   subroutine update_faceU_correction(this,vf,dt)
      ...
      call this%get_cell_pgrad(vf,this%P,FgradX,FgradY,FgradZ,.false.)
   ...
   subroutine solve_implicit(this,vf,dt,resU,resV,resW)
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               &                                                                this%divp_y(1,i,j,k)*       this%visc_y(i,j+1,k)*this%grdu_y(-1,i,j+1,k)+&
               &                                                                this%divp_y(0,i,j,k)*       this%visc_y(i,j  ,k)*this%grdu_y( 0,i,j  ,k)+&
               &                                                                this%divp_z(1,i,j,k)*       this%visc_z(i,j,k+1)*this%grdu_z(-1,i,j,k+1)+&
               &                                                                this%divp_z(0,i,j,k)*       this%visc_z(i,j,k  )*this%grdu_z( 0,i,j,k  ))
               ...
               this%implicit%opr(4,i,j,k)=this%implicit%opr(4,i,j,k)-0.5_WP*dt*(this%divp_y(1,i,j,k)*       this%visc_y(i,j+1,k)*this%grdu_y( 0,i,j+1,k))
               this%implicit%opr(5,i,j,k)=this%implicit%opr(5,i,j,k)-0.5_WP*dt*(this%divp_y(0,i,j,k)*       this%visc_y(i,j  ,k)*this%grdu_y(-1,i,j  ,k))
               this%implicit%opr(6,i,j,k)=this%implicit%opr(6,i,j,k)-0.5_WP*dt*(this%divp_z(1,i,j,k)*       this%visc_z(i,j,k+1)*this%grdu_z( 0,i,j,k+1))
               this%implicit%opr(7,i,j,k)=this%implicit%opr(7,i,j,k)-0.5_WP*dt*(this%divp_z(0,i,j,k)*       this%visc_z(i,j,k  )*this%grdu_z(-1,i,j,k  ))
            end do
         end do
      end do
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               this%implicit%opr(1,i,j,k)=this%implicit%opr(1,i,j,k)-0.5_WP*dt*(this%divp_x(1,i,j,k)*       this%visc_x(i+1,j,k)*this%grdu_x(-1,i+1,j,k)+&
               &                                                                this%divp_x(0,i,j,k)*       this%visc_x(i  ,j,k)*this%grdu_x( 0,i  ,j,k)+&
               ...
               &                                                                this%divp_z(1,i,j,k)*       this%visc_z(i,j,k+1)*this%grdu_z(-1,i,j,k+1)+&
               &                                                                this%divp_z(0,i,j,k)*       this%visc_z(i,j,k  )*this%grdu_z( 0,i,j,k  ))
               this%implicit%opr(2,i,j,k)=this%implicit%opr(2,i,j,k)-0.5_WP*dt*(this%divp_x(1,i,j,k)*       this%visc_x(i+1,j,k)*this%grdu_x( 0,i+1,j,k))
               this%implicit%opr(3,i,j,k)=this%implicit%opr(3,i,j,k)-0.5_WP*dt*(this%divp_x(0,i,j,k)*       this%visc_x(i  ,j,k)*this%grdu_x(-1,i  ,j,k))
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               this%implicit%opr(1,i,j,k)=this%implicit%opr(1,i,j,k)-0.5_WP*dt*(this%divp_x(1,i,j,k)*       this%visc_x(i+1,j,k)*this%grdw_x(-1,i+1,j,k)+&
               &                                                                this%divp_x(0,i,j,k)*       this%visc_x(i  ,j,k)*this%grdw_x( 0,i  ,j,k)+&
               &                                                                this%divp_y(1,i,j,k)*       this%visc_y(i,j+1,k)*this%grdw_y(-1,i,j+1,k)+&
               &                                                                this%divp_y(0,i,j,k)*       this%visc_y(i,j  ,k)*this%grdw_y( 0,i,j  ,k)+&
               ...
               this%implicit%opr(2,i,j,k)=this%implicit%opr(2,i,j,k)-0.5_WP*dt*(this%divp_x(1,i,j,k)*       this%visc_x(i+1,j,k)*this%grdw_x( 0,i+1,j,k))
               this%implicit%opr(3,i,j,k)=this%implicit%opr(3,i,j,k)-0.5_WP*dt*(this%divp_x(0,i,j,k)*       this%visc_x(i  ,j,k)*this%grdw_x(-1,i  ,j,k))
               this%implicit%opr(4,i,j,k)=this%implicit%opr(4,i,j,k)-0.5_WP*dt*(this%divp_y(1,i,j,k)*       this%visc_y(i,j+1,k)*this%grdw_y( 0,i,j+1,k))
               this%implicit%opr(5,i,j,k)=this%implicit%opr(5,i,j,k)-0.5_WP*dt*(this%divp_y(0,i,j,k)*       this%visc_y(i,j  ,k)*this%grdw_y(-1,i,j  ,k))
               this%implicit%opr(6,i,j,k)=this%implicit%opr(6,i,j,k)-0.5_WP*dt*(this%divp_z(1,i,j,k)*2.0_WP*this%visc_z(i,j,k+1)*this%grdw_z( 0,i,j,k+1))
