...
module simulation
   ...
   real(WP), dimension(:,:,:), allocatable :: Ubtmp!,Vbtmp
...
contains
   ...
   subroutine simulation_init
      ...
      allocate_work_arrays: block
         ...
         allocate(Ubtmp(cfg%imino_:cfg%imaxo_,cfg%jmino_:cfg%jmaxo_,cfg%kmino_:cfg%kmaxo_));Ubtmp=0.0_WP
         ! allocate(Vbtmp(cfg%imino_:cfg%imaxo_,cfg%jmino_:cfg%jmaxo_,cfg%kmino_:cfg%kmaxo_));Vbtmp=0.0_WP
      ...
      create_flow_solver: block
         ...
         use tpcons_class,    only: dirichlet,slip,clipped_neumann
         ! Create flow solver
      ...
      initialize_velocity: block
         ...
         do n=1,mybc%itr%n_
            ...
            yval=cfg%ym(j)
            if (yval.le.Hl) then
               fs%U(i-1,j,k)=(Ul*erf((Hl-yval)/dl) + Uint*(1.0_WP + erf((yval-Hl)/(dl*deficit))))*erf(yval/dl)
               fs%Uf(i,j,k)=fs%U (i-1,j,k)
            else if (yval.gt.Hl.and.yval.le.Hl+Hg) then
               fs%U(i-1,j,k)=(Ug*erf((yval-Hl)/dg) + Uint*(1.0_WP - erf((yval-Hl)/(dl*deficit))))* &
               &           (erf((Hg+Hl-yval)/dg))+Usm*(1.0_WP +erf((yval-(Hg+Hl))/dg))
               fs%Uf(i,j,k)=fs%U (i-1,j,k)
            else
               fs%U(i-1,j,k)=Usm*(1-erf((yval-(Hl+Hg))/dg)) + Ucf*erf((yval-(Hl+Hg))/dg)
               fs%Uf(i,j,k)=fs%U (i-1,j,k)
            end if
            if (abs(yval - Hl).le.0.5_WP*dg) then 
               fs%U(i-1,j,k) = fs%U(i-1,j,k)+random_uniform(lo=-0.5_WP*a_vel,hi=+0.5_WP*a_vel)
               fs%Uf(i,j,k)  = fs%U (i-1,j,k)
               fs%V(i,j,k) = random_uniform(lo=-0.5_WP*a_vel,hi=+0.5_WP*a_vel)       
            end if 
         end do
         ...
         do k=fs%cfg%kmin_,fs%cfg%kmax_+1
            do j=fs%cfg%jmin_,fs%cfg%jmax_+1
               do i=fs%cfg%imin_,fs%cfg%imax_+1
                  if (fs%mask(i,j-1,k)==1) then
                     fs%U(i,j-1,k)=0.0_WP; fs%V(i,j-1,k)=0.0_WP; fs%W(i,j-1,k)=0.0_WP
                  end if
               end do 
            end do
         end do 

         call fs%cfg%sync(fs%U); call fs%cfg%sync(fs%Uf)
         call fs%cfg%sync(fs%V); call fs%cfg%sync(fs%Vf)
         ...
         fs%P=fs%psolv%sol
         fs%Uf=fs%Uf-resU/fs%RHOX
         fs%Vf=fs%Vf-resV/fs%RHOY
         fs%Wf=fs%Wf-resW/fs%RHOZ
   ...
   subroutine simulation_run
      use tpcons_class, only: arithmetic_visc,bcond
      implicit none
         ...
         update_temporal_variation: block
            ...
            do n=1,mybc%itr%n_
               ...
               if (abs(yval - Hl).le.0.5_WP*dg) then 
                  ...
                  fs%Uf(i,j,k)  = fs%U (i-1,j,k)
                  fs%V(i,j,k) = random_uniform(lo=-0.5_WP*a_vel,hi=+0.5_WP*a_vel)       
                  ! fs%Vf(i-1,j,k) = fs%V(i-1,j,k)*0.5_WP
                  ! fs%Vf(i-1,j+1,k) = fs%V(i-1,j,k)*0.5_WP
               end if 
               Ubtmp(i-1,j,k)=fs%U(i-1,j,k)
            end do
            call fs%cfg%sync(fs%U); call fs%cfg%sync(Ubtmp) 
            call fs%cfg%sync(fs%V)!; call fs%cfg%sync(Vbtmp) 
            
         end block update_temporal_variation

         ! Remember old velocity and face densities
         fs%Uold=fs%U; fs%Ufold=fs%Uf 
         fs%Vold=fs%V; fs%Vfold=fs%Vf 
         fs%Wold=fs%W; fs%Wfold=fs%Wf 

         ! Perform sub-iterations
         do while (time%it.le.time%itmax)
            
            ! Build mid-time velocity
            fs%U=0.5_WP*(fs%U+fs%Uold);fs%Uf=0.5_WP*(fs%Uf+fs%Ufold)
            fs%V=0.5_WP*(fs%V+fs%Vold);fs%Vf=0.5_WP*(fs%Vf+fs%Vfold)
            fs%W=0.5_WP*(fs%W+fs%Wold);fs%Wf=0.5_WP*(fs%Wf+fs%Wfold)
            ! VOF equation ====================================================
            vf%VF=vf%VFold
            if (time%it.eq.time%itmax) then
               call vf%advance(dt=time%dt,U=fs%Uf,V=fs%Vf,W=fs%Wf,Uc=fs%U,Vc=fs%V,Wc=fs%W,rho_l=fs%rho_l,rho_g=fs%rho_g)
            else
               call vf%advance_tmp(dt=time%dt,U=fs%Uf,V=fs%Vf,W=fs%Wf,Uc=fs%U,Vc=fs%V,Wc=fs%W,rho_l=fs%rho_l,rho_g=fs%rho_g)
            end if
            ...
            ! ! Re-enforce Dirichlet inflow cell+face values from saved values
            ...
            !    do n=1,mybc%itr%n_
            !       i=mybc%itr%map(1,n); j=mybc%itr%map(2,n); k=mybc%itr%map(3,n)
            !       fs%U (i-1,j,k)=Ubtmp(i-1,j,k)
            !       fs%Uf(i,j,k)=Ubtmp(i-1,j,k)
            !       ! fs%V (i,j,k)=Vbtmp(i,j,k)
            !    end do
