...
module tpns_class
   ...
   type :: tpns
      ...
      real(WP), dimension(:,:,:), allocatable :: Uhat     !< U velocity array
      real(WP), dimension(:,:,:), allocatable :: Vhat     !< V velocity array
      real(WP), dimension(:,:,:), allocatable :: What     !< W velocity array
      real(WP), dimension(:,:,:), allocatable :: Uhatold  !< U velocity array
      real(WP), dimension(:,:,:), allocatable :: Vhatold  !< V velocity array
      real(WP), dimension(:,:,:), allocatable :: Whatold  !< W velocity array
      ...
      ! Old flow variables
      real(WP), dimension(:,:,:), allocatable :: rhoold      !< New density field
      ...
      real(WP), dimension(:,:,:), allocatable :: rho_Uold !< New density field at U face
      real(WP), dimension(:,:,:), allocatable :: rho_Vold !< New density field at V face
      real(WP), dimension(:,:,:), allocatable :: rho_Wold !< New density field at W face
      ! Flow divergence
   ...
   contains
      ...
      procedure :: get_sqrt_face_density                  !< Calculate sqrt face densities by interpolation
      ...
      procedure :: get_KE_monitor
      procedure :: get_KE_monitor_implicit
      procedure :: test_implicit

      procedure :: interp_velhat
...
contains
   ...
   subroutine initialize(this,cfg,name)
      ...
      allocate(this%Uhat(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%Uhat=0.0_WP
      allocate(this%Vhat(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%Vhat=0.0_WP
      allocate(this%What(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%What=0.0_WP
      allocate(this%Uhatold(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%Uhatold=0.0_WP
      allocate(this%Vhatold(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%Vhatold=0.0_WP
      allocate(this%Whatold(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%Whatold=0.0_WP
      ...
      allocate(this%rho_Uold(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%rho_Uold=0.0_WP
      allocate(this%rho_Vold(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%rho_Vold=0.0_WP
      allocate(this%rho_Wold(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%rho_Wold=0.0_WP
   ...
   !> This assumes that rho, rhoU/V/W have been updated already
      ...
      do kk=this%cfg%kmin_,this%cfg%kmax_+1
         do jj=this%cfg%jmin_,this%cfg%jmax_+1
            do ii=this%cfg%imin_,this%cfg%imax_+1
               ...
               FX(i,j,k)=-sum(this%itpu_x(:,i,j,k)*this%rhoU(i:i+1,j,k))*sum(this%itpu_x(:,i,j,k)*this%Uhat(i:i+1,j,k)) &
               &         +this%visc   (i,j,k)*(sum(this%grdu_x(:,i,j,k)*this%Uhat(i:i+1,j,k))+sum(this%grdu_x(:,i,j,k)*this%Uhat(i:i+1,j,k))) &
               &         -this%P(i,j,k)
               ! Fluxes on y-face
               i=ii; j=jj; k=kk
               FY(i,j,k)=-sum(this%itpv_x(:,i,j,k)*this%rhoV(i-1:i,j,k))*sum(this%itpu_y(:,i,j,k)*this%Uhat(i,j-1:j,k)) &
               &         +this%visc_xy(i,j,k)*(sum(this%grdu_y(:,i,j,k)*this%Uhat(i,j-1:j,k))+sum(this%grdv_x(:,i,j,k)*this%Vhat(i-1:i,j,k)))
               ! Fluxes on z-face
               i=ii; j=jj; k=kk
               FZ(i,j,k)=-sum(this%itpw_x(:,i,j,k)*this%rhoW(i-1:i,j,k))*sum(this%itpu_z(:,i,j,k)*this%Uhat(i,j,k-1:k)) &
               &         +this%visc_zx(i,j,k)*(sum(this%grdu_z(:,i,j,k)*this%Uhat(i,j,k-1:k))+sum(this%grdw_x(:,i,j,k)*this%What(i-1:i,j,k)))
            end do
         end do
      end do
      ...
      do kk=this%cfg%kmin_,this%cfg%kmax_+1
         do jj=this%cfg%jmin_,this%cfg%jmax_+1
            do ii=this%cfg%imin_,this%cfg%imax_+1
               ! Fluxes on x-face
               i=ii; j=jj; k=kk
               FX(i,j,k)=-sum(this%itpu_y(:,i,j,k)*this%rhoU(i,j-1:j,k))*sum(this%itpv_x(:,i,j,k)*this%Vhat(i-1:i,j,k)) &
               &         +this%visc_xy(i,j,k)*(sum(this%grdv_x(:,i,j,k)*this%Vhat(i-1:i,j,k))+sum(this%grdu_y(:,i,j,k)*this%Uhat(i,j-1:j,k)))
               ! Fluxes on y-face
               i=ii-1; j=jj-1; k=kk-1
               FY(i,j,k)=-sum(this%itpv_y(:,i,j,k)*this%rhoV(i,j:j+1,k))*sum(this%itpv_y(:,i,j,k)*this%Vhat(i,j:j+1,k)) &
               &         +this%visc   (i,j,k)*(sum(this%grdv_y(:,i,j,k)*this%Vhat(i,j:j+1,k))+sum(this%grdv_y(:,i,j,k)*this%Vhat(i,j:j+1,k))) &
               &         -this%P(i,j,k)
               ! Fluxes on z-face
               i=ii; j=jj; k=kk
               FZ(i,j,k)=-sum(this%itpw_y(:,i,j,k)*this%rhoW(i,j-1:j,k))*sum(this%itpv_z(:,i,j,k)*this%Vhat(i,j,k-1:k)) &
               &         +this%visc_yz(i,j,k)*(sum(this%grdv_z(:,i,j,k)*this%Vhat(i,j,k-1:k))+sum(this%grdw_y(:,i,j,k)*this%What(i,j-1:j,k)))
            end do
         end do
      end do
      ...
      do kk=this%cfg%kmin_,this%cfg%kmax_+1
         do jj=this%cfg%jmin_,this%cfg%jmax_+1
            do ii=this%cfg%imin_,this%cfg%imax_+1
               ! Fluxes on x-face
               i=ii; j=jj; k=kk
               FX(i,j,k)=-sum(this%itpu_z(:,i,j,k)*this%rhoU(i,j,k-1:k))*sum(this%itpw_x(:,i,j,k)*this%What(i-1:i,j,k)) &
               &         +this%visc_zx(i,j,k)*(sum(this%grdw_x(:,i,j,k)*this%What(i-1:i,j,k))+sum(this%grdu_z(:,i,j,k)*this%Uhat(i,j,k-1:k)))
               ! Fluxes on y-face
               i=ii; j=jj; k=kk
               FY(i,j,k)=-sum(this%itpv_z(:,i,j,k)*this%rhoV(i,j,k-1:k))*sum(this%itpw_y(:,i,j,k)*this%What(i,j-1:j,k)) &
               &         +this%visc_yz(i,j,k)*(sum(this%grdw_y(:,i,j,k)*this%What(i,j-1:j,k))+sum(this%grdv_z(:,i,j,k)*this%Vhat(i,j,k-1:k)))
               ! Fluxes on z-face
               i=ii-1; j=jj-1; k=kk-1
               FZ(i,j,k)=-sum(this%itpw_z(:,i,j,k)*this%rhoW(i,j,k:k+1))*sum(this%itpw_z(:,i,j,k)*this%What(i,j,k:k+1)) &
               &         +this%visc   (i,j,k)*(sum(this%grdw_z(:,i,j,k)*this%What(i,j,k:k+1))+sum(this%grdw_z(:,i,j,k)*this%What(i,j,k:k+1))) &
               &         -this%P(i,j,k)
            end do
         end do
      end do
   ...
   !> Calculate the explicit momentum time derivative
   !> This assumes that rho, rhoU/V/W have been updated already
   subroutine test_implicit(this,dt,resU,resV,resW,resUold,resVold,resWold)
      implicit none
      class(tpns), intent(inout) :: this
      real(WP), intent(in) :: dt
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(in) :: resU
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(in) :: resV
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(in) :: resW
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: resUold
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: resVold
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: resWold

      integer :: i,j,k,ii,jj,kk
      real(WP), dimension(:,:,:), allocatable :: FX,FY,FZ,resUtmp,resVtmp,resWtmp
      ...
      allocate(resUtmp(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_))
      allocate(resVtmp(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_))
      allocate(resWtmp(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_))


      resUtmp=(resU*this%rho_U)/(this%rho_U+this%rho_Uold)/(this%rho_U**2)
      resVtmp=(resV*this%rho_V)/(this%rho_V+this%rho_Vold)/(this%rho_V**2)
      resWtmp=(resW*this%rho_W)/(this%rho_W+this%rho_Wold)/(this%rho_W**2)


      resUold=resUold-resU
      resVold=resVold-resV
      resWold=resWold-resW
      ...
      do kk=this%cfg%kmin_,this%cfg%kmax_+1
         do jj=this%cfg%jmin_,this%cfg%jmax_+1
            do ii=this%cfg%imin_,this%cfg%imax_+1
               ! Fluxes on x-face
               i=ii-1; j=jj-1; k=kk-1
               FX(i,j,k)=-sum(this%itpu_x(:,i,j,k)*this%rhoU(i:i+1,j,k))*sum(this%itpu_x(:,i,j,k)*resUtmp(i:i+1,j,k)) &
               &         +this%visc   (i,j,k)*(sum(this%grdu_x(:,i,j,k)*resUtmp(i:i+1,j,k))+sum(this%grdu_x(:,i,j,k)*resUtmp(i:i+1,j,k))) 
               ! Fluxes on y-face
               i=ii; j=jj; k=kk
               FY(i,j,k)=-sum(this%itpv_x(:,i,j,k)*this%rhoV(i-1:i,j,k))*sum(this%itpu_y(:,i,j,k)*resUtmp(i,j-1:j,k)) &
               &         +this%visc_xy(i,j,k)*(sum(this%grdu_y(:,i,j,k)*resUtmp(i,j-1:j,k))+sum(this%grdv_x(:,i,j,k)*resVtmp(i-1:i,j,k)))
               ! Fluxes on z-face
               i=ii; j=jj; k=kk
               FZ(i,j,k)=-sum(this%itpw_x(:,i,j,k)*this%rhoW(i-1:i,j,k))*sum(this%itpu_z(:,i,j,k)*resUtmp(i,j,k-1:k)) &
               &         +this%visc_zx(i,j,k)*(sum(this%grdu_z(:,i,j,k)*resUtmp(i,j,k-1:k))+sum(this%grdw_x(:,i,j,k)*resWtmp(i-1:i,j,k)))
            end do
         end do
      end do
      ! Time derivative of rhoU
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               resUtmp(i,j,k)=sum(this%divu_x(:,i,j,k)*FX(i-1:i,j,k))+&
               &              sum(this%divu_y(:,i,j,k)*FY(i,j:j+1,k))+&
      ...
      do kk=this%cfg%kmin_,this%cfg%kmax_+1
         do jj=this%cfg%jmin_,this%cfg%jmax_+1
            do ii=this%cfg%imin_,this%cfg%imax_+1
               ! Fluxes on x-face
               i=ii; j=jj; k=kk
               FX(i,j,k)=-sum(this%itpu_y(:,i,j,k)*this%rhoU(i,j-1:j,k))*sum(this%itpv_x(:,i,j,k)*resVtmp(i-1:i,j,k)) &
               &         +this%visc_xy(i,j,k)*(sum(this%grdv_x(:,i,j,k)*resVtmp(i-1:i,j,k))+sum(this%grdu_y(:,i,j,k)*resUtmp(i,j-1:j,k)))
               ! Fluxes on y-face
               i=ii-1; j=jj-1; k=kk-1
               FY(i,j,k)=-sum(this%itpv_y(:,i,j,k)*this%rhoV(i,j:j+1,k))*sum(this%itpv_y(:,i,j,k)*resVtmp(i,j:j+1,k)) &
               &         +this%visc   (i,j,k)*(sum(this%grdv_y(:,i,j,k)*resVtmp(i,j:j+1,k))+sum(this%grdv_y(:,i,j,k)*resVtmp(i,j:j+1,k))) 
               ! Fluxes on z-face
               i=ii; j=jj; k=kk
               FZ(i,j,k)=-sum(this%itpw_y(:,i,j,k)*this%rhoW(i,j-1:j,k))*sum(this%itpv_z(:,i,j,k)*resVtmp(i,j,k-1:k)) &
               &         +this%visc_yz(i,j,k)*(sum(this%grdv_z(:,i,j,k)*resVtmp(i,j,k-1:k))+sum(this%grdw_y(:,i,j,k)*resWtmp(i,j-1:j,k)))
            end do
         end do
      end do
      ! Time derivative of rhoV
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               resVtmp(i,j,k)=sum(this%divv_x(:,i,j,k)*FX(i:i+1,j,k))+&
               &              sum(this%divv_y(:,i,j,k)*FY(i,j-1:j,k))+&
      ...
      do kk=this%cfg%kmin_,this%cfg%kmax_+1
         do jj=this%cfg%jmin_,this%cfg%jmax_+1
            do ii=this%cfg%imin_,this%cfg%imax_+1
               ! Fluxes on x-face
               i=ii; j=jj; k=kk
               FX(i,j,k)=-sum(this%itpu_z(:,i,j,k)*this%rhoU(i,j,k-1:k))*sum(this%itpw_x(:,i,j,k)*resWtmp(i-1:i,j,k)) &
               &         +this%visc_zx(i,j,k)*(sum(this%grdw_x(:,i,j,k)*resWtmp(i-1:i,j,k))+sum(this%grdu_z(:,i,j,k)*resUtmp(i,j,k-1:k)))
               ! Fluxes on y-face
               i=ii; j=jj; k=kk
               FY(i,j,k)=-sum(this%itpv_z(:,i,j,k)*this%rhoV(i,j,k-1:k))*sum(this%itpw_y(:,i,j,k)*resWtmp(i,j-1:j,k)) &
               &         +this%visc_yz(i,j,k)*(sum(this%grdw_y(:,i,j,k)*resWtmp(i,j-1:j,k))+sum(this%grdv_z(:,i,j,k)*resVtmp(i,j,k-1:k)))
               ! Fluxes on z-face
               i=ii-1; j=jj-1; k=kk-1
               FZ(i,j,k)=-sum(this%itpw_z(:,i,j,k)*this%rhoW(i,j,k:k+1))*sum(this%itpw_z(:,i,j,k)*resWtmp(i,j,k:k+1)) &
               &         +this%visc   (i,j,k)*(sum(this%grdw_z(:,i,j,k)*resWtmp(i,j,k:k+1))+sum(this%grdw_z(:,i,j,k)*resWtmp(i,j,k:k+1))) 
            end do
         end do
      end do
      ! Time derivative of rhoW
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               resWtmp(i,j,k)=sum(this%divw_x(:,i,j,k)*FX(i:i+1,j,k))+&
               &              sum(this%divw_y(:,i,j,k)*FY(i,j:j+1,k))+&
      ...
      ! Sync it
      call this%cfg%sync(resWtmp)
      
      ! Deallocate flux arrays
      deallocate(FX,FY,FZ)

      resUold=resUold+dt*resUtmp
      resVold=resVold+dt*resVtmp
      resWold=resWold+dt*resWtmp
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               do s1=0,1
                  do s2=-1,0
                     this%psolv%opr(this%psolv%stmap(s1+s2,0,0),i,j,k)=this%psolv%opr(this%psolv%stmap(s1+s2,0,0),i,j,k)+this%divp_x(s1,i,j,k)*this%divu_x(s2,i+s1,j,k)/((this%rho_U(i+s1,j,k)+this%rho_Uold(i+s1,j,k))*this%rho_U(i+s1,j,k))
                     this%psolv%opr(this%psolv%stmap(0,s1+s2,0),i,j,k)=this%psolv%opr(this%psolv%stmap(0,s1+s2,0),i,j,k)+this%divp_y(s1,i,j,k)*this%divv_y(s2,i,j+s1,k)/((this%rho_V(i,j+s1,k)+this%rho_Vold(i,j+s1,k))*this%rho_V(i,j+s1,k))
                     this%psolv%opr(this%psolv%stmap(0,0,s1+s2),i,j,k)=this%psolv%opr(this%psolv%stmap(0,0,s1+s2),i,j,k)+this%divp_z(s1,i,j,k)*this%divw_z(s2,i,j,k+s1)/((this%rho_W(i,j,k+s1)+this%rho_Wold(i,j,k+s1))*this%rho_W(i,j,k+s1))
                  end do
               end do
   ...
   subroutine get_div(this)
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               this%div(i,j,k)=sum(this%divp_x(:,i,j,k)*this%Uhat(i:i+1,j,k))+&
               &               sum(this%divp_y(:,i,j,k)*this%Vhat(i,j:j+1,k))+&
               &               sum(this%divp_z(:,i,j,k)*this%What(i,j,k:k+1))
            end do
         end do
      end do
      ! Sync it
   ...
   !> Calculate the interpolated velocity, including overlap and ghosts
   subroutine interp_velhat(this,Ui,Vi,Wi)
      implicit none
      class(tpns), intent(inout) :: this
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(out) :: Ui !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(out) :: Vi !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      ...
      do k=this%cfg%kmino_,this%cfg%kmaxo_
         do j=this%cfg%jmino_,this%cfg%jmaxo_
            do i=this%cfg%imino_,this%cfg%imaxo_-1
               Ui(i,j,k)=sum(this%itpu_x(:,i,j,k)*this%Uhat(i:i+1,j,k))
            end do
         end do
      end do
      do k=this%cfg%kmino_,this%cfg%kmaxo_
         do j=this%cfg%jmino_,this%cfg%jmaxo_-1
            do i=this%cfg%imino_,this%cfg%imaxo_
               Vi(i,j,k)=sum(this%itpv_y(:,i,j,k)*this%Vhat(i,j:j+1,k))
            end do
         end do
      end do
      do k=this%cfg%kmino_,this%cfg%kmaxo_-1
         do j=this%cfg%jmino_,this%cfg%jmaxo_
            do i=this%cfg%imino_,this%cfg%imaxo_
               Wi(i,j,k)=sum(this%itpw_z(:,i,j,k)*this%What(i,j,k:k+1))
            end do
         end do
      end do
      ! Add last layer in each direction
   ...
   subroutine solve_implicit(this,dt,resU,resV,resW)
      ...
      this%implicit%opr(1,:,:,:)=this%rho_U**2; this%implicit%opr(2:,:,:,:)=0.0_WP
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               &                                                         this%divu_z( 0,i,j,k)*this%itpu_z( 0,i,j,k  )*rhoWm)*this%rho_U(i,j,k)/(this%rho_U(i,j,k)+this%rho_Uold(i,j,k))
               this%implicit%opr(2,i,j,k)=this%implicit%opr(2,i,j,k)+dt*(this%divu_x( 0,i,j,k)*this%itpu_x(+1,i  ,j,k)*rhoUp*this%rho_U(i+1,j,k)/(this%rho_U(i+1,j,k)+this%rho_Uold(i+1,j,k)))
               this%implicit%opr(3,i,j,k)=this%implicit%opr(3,i,j,k)+dt*(this%divu_x(-1,i,j,k)*this%itpu_x( 0,i-1,j,k)*rhoUm*this%rho_U(i-1,j,k)/(this%rho_U(i-1,j,k)+this%rho_Uold(i-1,j,k)))
               this%implicit%opr(4,i,j,k)=this%implicit%opr(4,i,j,k)+dt*(this%divu_y(+1,i,j,k)*this%itpu_y( 0,i,j+1,k)*rhoVp*this%rho_U(i,j+1,k)/(this%rho_U(i,j+1,k)+this%rho_Uold(i,j+1,k)))
               this%implicit%opr(5,i,j,k)=this%implicit%opr(5,i,j,k)+dt*(this%divu_y( 0,i,j,k)*this%itpu_y(-1,i,j  ,k)*rhoVm*this%rho_U(i,j-1,k)/(this%rho_U(i,j-1,k)+this%rho_Uold(i,j-1,k)))
               this%implicit%opr(6,i,j,k)=this%implicit%opr(6,i,j,k)+dt*(this%divu_z(+1,i,j,k)*this%itpu_z( 0,i,j,k+1)*rhoWp*this%rho_U(i,j+1,k)/(this%rho_U(i,j,k+1)+this%rho_Uold(i,j,k+1)))
               this%implicit%opr(7,i,j,k)=this%implicit%opr(7,i,j,k)+dt*(this%divu_z( 0,i,j,k)*this%itpu_z(-1,i,j,k  )*rhoWm*this%rho_U(i,j-1,k)/(this%rho_U(i,j,k-1)+this%rho_Uold(i,j,k-1)))
            end do
         end do
      end do
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               &                                                         this%divu_z( 0,i,j,k)*       this%visc_zx(i,j,k  )*this%grdu_z( 0,i,j,k  ))*this%rho_U(i,j,k)/(this%rho_U(i,j,k)+this%rho_Uold(i,j,k))
               this%implicit%opr(2,i,j,k)=this%implicit%opr(2,i,j,k)-dt*(this%divu_x( 0,i,j,k)*2.0_WP*this%visc   (i  ,j,k)*this%grdu_x(+1,i  ,j,k)*this%rho_U(i+1,j,k)/(this%rho_U(i+1,j,k)+this%rho_Uold(i+1,j,k)))
               this%implicit%opr(3,i,j,k)=this%implicit%opr(3,i,j,k)-dt*(this%divu_x(-1,i,j,k)*2.0_WP*this%visc   (i-1,j,k)*this%grdu_x( 0,i-1,j,k)*this%rho_U(i-1,j,k)/(this%rho_U(i-1,j,k)+this%rho_Uold(i-1,j,k)))
               this%implicit%opr(4,i,j,k)=this%implicit%opr(4,i,j,k)-dt*(this%divu_y(+1,i,j,k)*       this%visc_xy(i,j+1,k)*this%grdu_y( 0,i,j+1,k)*this%rho_U(i,j+1,k)/(this%rho_U(i,j+1,k)+this%rho_Uold(i,j+1,k)))
               this%implicit%opr(5,i,j,k)=this%implicit%opr(5,i,j,k)-dt*(this%divu_y( 0,i,j,k)*       this%visc_xy(i,j  ,k)*this%grdu_y(-1,i,j  ,k)*this%rho_U(i,j-1,k)/(this%rho_U(i,j-1,k)+this%rho_Uold(i,j-1,k)))
               this%implicit%opr(6,i,j,k)=this%implicit%opr(6,i,j,k)-dt*(this%divu_z(+1,i,j,k)*       this%visc_zx(i,j,k+1)*this%grdu_z( 0,i,j,k+1)*this%rho_U(i,j+1,k)/(this%rho_U(i,j,k+1)+this%rho_Uold(i,j,k+1)))
               this%implicit%opr(7,i,j,k)=this%implicit%opr(7,i,j,k)-dt*(this%divu_z( 0,i,j,k)*       this%visc_zx(i,j,k  )*this%grdu_z(-1,i,j,k  )*this%rho_U(i,j-1,k)/(this%rho_U(i,j,k-1)+this%rho_Uold(i,j,k-1)))
            end do
         end do
      end do
      ...
      resU=this%implicit%sol*this%rho_U**2
      
      ! Solve implicit V problem
      this%implicit%opr(1,:,:,:)=this%rho_V**2; this%implicit%opr(2:,:,:,:)=0.0_WP
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               &                                                         this%divv_z( 0,i,j,k)*this%itpv_z( 0,i,j,k  )*rhoWm)*this%rho_V(i,j,k)/(this%rho_V(i,j,k)+this%rho_Vold(i,j,k))
               this%implicit%opr(2,i,j,k)=this%implicit%opr(2,i,j,k)+dt*(this%divv_x(+1,i,j,k)*this%itpv_x( 0,i+1,j,k)*rhoUp*this%rho_V(i+1,j,k)/(this%rho_V(i+1,j,k)+this%rho_Vold(i+1,j,k)))
               this%implicit%opr(3,i,j,k)=this%implicit%opr(3,i,j,k)+dt*(this%divv_x( 0,i,j,k)*this%itpv_x(-1,i  ,j,k)*rhoUm*this%rho_V(i-1,j,k)/(this%rho_V(i-1,j,k)+this%rho_Vold(i-1,j,k)))
               this%implicit%opr(4,i,j,k)=this%implicit%opr(4,i,j,k)+dt*(this%divv_y( 0,i,j,k)*this%itpv_y(+1,i,j  ,k)*rhoVp*this%rho_V(i,j+1,k)/(this%rho_V(i,j+1,k)+this%rho_Vold(i,j+1,k)))
               this%implicit%opr(5,i,j,k)=this%implicit%opr(5,i,j,k)+dt*(this%divv_y(-1,i,j,k)*this%itpv_y( 0,i,j-1,k)*rhoVm*this%rho_V(i,j-1,k)/(this%rho_V(i,j-1,k)+this%rho_Vold(i,j-1,k)))
               this%implicit%opr(6,i,j,k)=this%implicit%opr(6,i,j,k)+dt*(this%divv_z(+1,i,j,k)*this%itpv_z( 0,i,j,k+1)*rhoWp*this%rho_V(i,j+1,k)/(this%rho_V(i,j,k+1)+this%rho_Vold(i,j,k+1)))
               this%implicit%opr(7,i,j,k)=this%implicit%opr(7,i,j,k)+dt*(this%divv_z( 0,i,j,k)*this%itpv_z(-1,i,j,k  )*rhoWm*this%rho_V(i,j-1,k)/(this%rho_V(i,j,k-1)+this%rho_Vold(i,j,k-1)))
            end do
         end do
      end do
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               &                                                         this%divv_z( 0,i,j,k)*       this%visc_yz(i,j,k  )*this%grdv_z( 0,i,j,k  ))*this%rho_V(i,j,k)/(this%rho_V(i,j,k)+this%rho_Vold(i,j,k))
               this%implicit%opr(2,i,j,k)=this%implicit%opr(2,i,j,k)-dt*(this%divv_x(+1,i,j,k)*       this%visc_xy(i+1,j,k)*this%grdv_x( 0,i+1,j,k)*this%rho_V(i+1,j,k)/(this%rho_V(i+1,j,k)+this%rho_Vold(i+1,j,k)))
               this%implicit%opr(3,i,j,k)=this%implicit%opr(3,i,j,k)-dt*(this%divv_x( 0,i,j,k)*       this%visc_xy(i  ,j,k)*this%grdv_x(-1,i  ,j,k)*this%rho_V(i-1,j,k)/(this%rho_V(i-1,j,k)+this%rho_Vold(i-1,j,k)))
               this%implicit%opr(4,i,j,k)=this%implicit%opr(4,i,j,k)-dt*(this%divv_y( 0,i,j,k)*2.0_WP*this%visc   (i,j  ,k)*this%grdv_y(+1,i,j  ,k)*this%rho_V(i,j+1,k)/(this%rho_V(i,j+1,k)+this%rho_Vold(i,j+1,k)))
               this%implicit%opr(5,i,j,k)=this%implicit%opr(5,i,j,k)-dt*(this%divv_y(-1,i,j,k)*2.0_WP*this%visc   (i,j-1,k)*this%grdv_y( 0,i,j-1,k)*this%rho_V(i,j-1,k)/(this%rho_V(i,j-1,k)+this%rho_Vold(i,j-1,k)))
               this%implicit%opr(6,i,j,k)=this%implicit%opr(6,i,j,k)-dt*(this%divv_z(+1,i,j,k)*       this%visc_yz(i,j,k+1)*this%grdv_z( 0,i,j,k+1)*this%rho_V(i,j+1,k)/(this%rho_V(i,j,k+1)+this%rho_Vold(i,j,k+1)))
               this%implicit%opr(7,i,j,k)=this%implicit%opr(7,i,j,k)-dt*(this%divv_z( 0,i,j,k)*       this%visc_yz(i,j,k  )*this%grdv_z(-1,i,j,k  )*this%rho_V(i,j-1,k)/(this%rho_V(i,j,k-1)+this%rho_Vold(i,j,k-1)))
            end do
         end do
      end do
      call this%implicit%setup()
      ...
      this%implicit%sol=0.0_WP
      call this%implicit%solve()
      resV=this%implicit%sol*this%rho_V**2
      
      ! Solve implicit W problem
      this%implicit%opr(1,:,:,:)=this%rho_W**2; this%implicit%opr(2:,:,:,:)=0.0_WP
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               &                                                         this%divw_z(-1,i,j,k)*this%itpw_z(+1,i,j,k-1)*rhoWm)*this%rho_W(i,j,k)/(this%rho_W(i,j,k)+this%rho_Wold(i,j,k))
               this%implicit%opr(2,i,j,k)=this%implicit%opr(2,i,j,k)+dt*(this%divw_x(+1,i,j,k)*this%itpw_x( 0,i+1,j,k)*rhoUp*this%rho_W(i+1,j,k)/(this%rho_W(i+1,j,k)+this%rho_Wold(i+1,j,k)))
               this%implicit%opr(3,i,j,k)=this%implicit%opr(3,i,j,k)+dt*(this%divw_x( 0,i,j,k)*this%itpw_x(-1,i  ,j,k)*rhoUm*this%rho_W(i-1,j,k)/(this%rho_W(i-1,j,k)+this%rho_Wold(i-1,j,k)))
               this%implicit%opr(4,i,j,k)=this%implicit%opr(4,i,j,k)+dt*(this%divw_y(+1,i,j,k)*this%itpw_y( 0,i,j+1,k)*rhoVp*this%rho_W(i,j+1,k)/(this%rho_W(i,j+1,k)+this%rho_Wold(i,j+1,k)))
               this%implicit%opr(5,i,j,k)=this%implicit%opr(5,i,j,k)+dt*(this%divw_y( 0,i,j,k)*this%itpw_y(-1,i,j  ,k)*rhoVm*this%rho_W(i,j-1,k)/(this%rho_W(i,j-1,k)+this%rho_Wold(i,j-1,k)))
               this%implicit%opr(6,i,j,k)=this%implicit%opr(6,i,j,k)+dt*(this%divw_z( 0,i,j,k)*this%itpw_z(+1,i,j,k  )*rhoWp*this%rho_W(i,j+1,k)/(this%rho_W(i,j,k+1)+this%rho_Wold(i,j,k+1)))
               this%implicit%opr(7,i,j,k)=this%implicit%opr(7,i,j,k)+dt*(this%divw_z(-1,i,j,k)*this%itpw_z( 0,i,j,k-1)*rhoWm*this%rho_W(i,j-1,k)/(this%rho_W(i,j,k-1)+this%rho_Wold(i,j,k-1)))
            end do
         end do
      end do
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               &                                                         this%divw_z(-1,i,j,k)*2.0_WP*this%visc   (i,j,k-1)*this%grdw_z(+1,i,j,k-1))*this%rho_W(i,j,k)/(this%rho_W(i,j,k)+this%rho_Wold(i,j,k))
               this%implicit%opr(2,i,j,k)=this%implicit%opr(2,i,j,k)-dt*(this%divw_x(+1,i,j,k)*       this%visc_zx(i+1,j,k)*this%grdw_x( 0,i+1,j,k)*this%rho_W(i+1,j,k)/(this%rho_W(i+1,j,k)+this%rho_Wold(i+1,j,k)))
               this%implicit%opr(3,i,j,k)=this%implicit%opr(3,i,j,k)-dt*(this%divw_x( 0,i,j,k)*       this%visc_zx(i  ,j,k)*this%grdw_x(-1,i  ,j,k)*this%rho_W(i-1,j,k)/(this%rho_W(i-1,j,k)+this%rho_Wold(i-1,j,k)))
               this%implicit%opr(4,i,j,k)=this%implicit%opr(4,i,j,k)-dt*(this%divw_y(+1,i,j,k)*       this%visc_yz(i,j+1,k)*this%grdw_y( 0,i,j+1,k)*this%rho_W(i,j+1,k)/(this%rho_W(i,j+1,k)+this%rho_Wold(i,j+1,k)))
               this%implicit%opr(5,i,j,k)=this%implicit%opr(5,i,j,k)-dt*(this%divw_y( 0,i,j,k)*       this%visc_yz(i,j  ,k)*this%grdw_y(-1,i,j  ,k)*this%rho_W(i,j-1,k)/(this%rho_W(i,j-1,k)+this%rho_Wold(i,j-1,k)))
               this%implicit%opr(6,i,j,k)=this%implicit%opr(6,i,j,k)-dt*(this%divw_z( 0,i,j,k)*2.0_WP*this%visc   (i,j,k  )*this%grdw_z(+1,i,j,k  )*this%rho_W(i,j+1,k)/(this%rho_W(i,j,k+1)+this%rho_Wold(i,j,k+1)))
               this%implicit%opr(7,i,j,k)=this%implicit%opr(7,i,j,k)-dt*(this%divw_z(-1,i,j,k)*2.0_WP*this%visc   (i,j,k-1)*this%grdw_z( 0,i,j,k-1)*this%rho_W(i,j-1,k)/(this%rho_W(i,j,k-1)+this%rho_Wold(i,j,k-1)))
            end do
         end do
      end do
      call this%implicit%setup()
      ...
      this%implicit%sol=0.0_WP
      call this%implicit%solve()
      resW=this%implicit%sol*this%rho_W**2
   ...
   subroutine update_density(this,vf)
      ...
      integer :: i,j,k
      ! Centered density
      this%rho=this%rho_l*vf%VF+this%rho_g*(1.0_WP-vf%VF)
      ! Staggered momentum vector
      this%rhoU=this%rho_l*vf%UFl(1,:,:,:)+this%rho_g*vf%UFg(1,:,:,:)
      this%rhoV=this%rho_l*vf%UFl(2,:,:,:)+this%rho_g*vf%UFg(2,:,:,:)
      this%rhoW=this%rho_l*vf%UFl(3,:,:,:)+this%rho_g*vf%UFg(3,:,:,:)
      ! Calculate sqrt of rho_U/V/W using interpolation
      do k=this%cfg%kmino_  ,this%cfg%kmaxo_
         do j=this%cfg%jmino_  ,this%cfg%jmaxo_
            do i=this%cfg%imino_+1,this%cfg%imaxo_
               this%rho_U(i,j,k)=sqrt(sum(this%itpr_x(:,i,j,k)*this%rho(i-1:i,j,k)))
            end do
         end do
      end do
      do k=this%cfg%kmino_  ,this%cfg%kmaxo_
         do j=this%cfg%jmino_+1,this%cfg%jmaxo_
            do i=this%cfg%imino_  ,this%cfg%imaxo_
               this%rho_V(i,j,k)=sqrt(sum(this%itpr_y(:,i,j,k)*this%rho(i,j-1:j,k)))
            end do
         end do
      end do
      do k=this%cfg%kmino_+1,this%cfg%kmaxo_
         do j=this%cfg%jmino_  ,this%cfg%jmaxo_
            do i=this%cfg%imino_  ,this%cfg%imaxo_
               this%rho_W(i,j,k)=sqrt(sum(this%itpr_z(:,i,j,k)*this%rho(i,j,k-1:k)))
            end do
         end do
      end do
      ...
      call this%cfg%sync(this%rhoU); call this%cfg%sync(this%rho_U)
      call this%cfg%sync(this%rhoV); call this%cfg%sync(this%rho_V)
      call this%cfg%sync(this%rhoW); call this%cfg%sync(this%rho_W)
   end subroutine update_density

   
   !> Compute face densities by interpolation
   subroutine get_sqrt_face_density(this)
      implicit none
      class(tpns), intent(inout) :: this
      integer :: i,j,k
   ...
   end subroutine get_sqrt_face_density
   
   

   subroutine get_KE_monitor(this,dt)
      implicit none
      class(tpns), intent(inout) :: this
      real(WP), intent(in) :: dt
      ...
      real(WP), dimension(:,:,:), allocatable:: FZ
      integer :: i,j,k,ii,jj,kk
      allocate(Utmp(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_))
      allocate(Vtmp(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_))
      allocate(Wtmp(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_))
      allocate(Stmp(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_))
      allocate(FX(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_))
      allocate(FY(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_))
      allocate(FZ(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_))
      ...
      calculate_dKEdt: block
         Utmp=((this%rho_U**2)*this%U**2-(this%rho_Uold**2)*this%Uold**2)/(2.0_WP*dt)
         Vtmp=((this%rho_V**2)*this%V**2-(this%rho_Vold**2)*this%Vold**2)/(2.0_WP*dt)
         Wtmp=((this%rho_W**2)*this%W**2-(this%rho_Wold**2)*this%Wold**2)/(2.0_WP*dt)
         do k=this%cfg%kmin_,this%cfg%kmax_
            do j=this%cfg%jmin_,this%cfg%jmax_
               do i=this%cfg%imin_,this%cfg%imax_
                  Stmp(i,j,k)=sum(this%itpu_x(:,i,j,k)*Utmp(i:i+1,j,k))+&
                  &           sum(this%itpv_y(:,i,j,k)*Vtmp(i,j:j+1,k))+&
                  &           sum(this%itpw_z(:,i,j,k)*Wtmp(i,j,k:k+1))
               end do
            end do
         end do
         call this%cfg%integrate_without_VF(Stmp,integral=this%dKEdt)
      end block calculate_dKEdt

      ! Get KE 
      calculate_KE: block
         Utmp=(this%rho_U**2)*this%U*this%U/2.0_WP
         Vtmp=(this%rho_V**2)*this%V*this%V/2.0_WP
         Wtmp=(this%rho_W**2)*this%W*this%W/2.0_WP
         do k=this%cfg%kmin_,this%cfg%kmax_
            do j=this%cfg%jmin_,this%cfg%jmax_
               do i=this%cfg%imin_,this%cfg%imax_
                  Stmp(i,j,k)=sum(this%itpu_x(:,i,j,k)*Utmp(i:i+1,j,k))+&
         ...
         call this%cfg%integrate_without_VF(Stmp,integral=this%KE)
      end block calculate_KE
      
      ! Get the error made in the convective term
      calculate_convecterror: block
         Utmp=0.0_WP
         Vtmp=0.0_WP
         Wtmp=0.0_WP
         ! Flux of rhoU
         do kk=this%cfg%kmin_,this%cfg%kmax_+1
            do jj=this%cfg%jmin_,this%cfg%jmax_+1
               do ii=this%cfg%imin_,this%cfg%imax_+1
                  ! Fluxes on x-face
                  i=ii-1; j=jj-1; k=kk-1
                  FX(i,j,k)=sum(this%itpu_x(:,i,j,k)*this%rhoU(i:i+1,j,k))*sum(this%itpu_x(:,i,j,k)*(this%Uhat(i:i+1,j,k)-this%Uhatold(i:i+1,j,k)))
                  ! Fluxes on y-face
                  i=ii; j=jj; k=kk
                  FY(i,j,k)=sum(this%itpv_x(:,i,j,k)*this%rhoV(i-1:i,j,k))*sum(this%itpu_y(:,i,j,k)*(this%Uhat(i,j-1:j,k)-this%Uhatold(i,j-1:j,k))) 
                  ! Fluxes on z-face
                  i=ii; j=jj; k=kk
                  FZ(i,j,k)=sum(this%itpw_x(:,i,j,k)*this%rhoW(i-1:i,j,k))*sum(this%itpu_z(:,i,j,k)*(this%Uhat(i,j,k-1:k)-this%Uhatold(i,j,k-1:k))) 
               end do
            end do
         end do
         ! Time derivative of rhoU
         do k=this%cfg%kmin_,this%cfg%kmax_
            do j=this%cfg%jmin_,this%cfg%jmax_
               do i=this%cfg%imin_,this%cfg%imax_
                  Utmp(i,j,k)=sum(this%divu_x(:,i,j,k)*FX(i-1:i,j,k))+&
                  &           sum(this%divu_y(:,i,j,k)*FY(i,j:j+1,k))+&
                  &           sum(this%divu_z(:,i,j,k)*FZ(i,j,k:k+1))
               end do
            end do
         ...
         do kk=this%cfg%kmin_,this%cfg%kmax_+1
            do jj=this%cfg%jmin_,this%cfg%jmax_+1
               do ii=this%cfg%imin_,this%cfg%imax_+1
                  ! Fluxes on x-face
                  i=ii; j=jj; k=kk
                  FX(i,j,k)=sum(this%itpu_y(:,i,j,k)*this%rhoU(i,j-1:j,k))*sum(this%itpv_x(:,i,j,k)*(this%Vhat(i-1:i,j,k)-this%Vhatold(i-1:i,j,k))) 
                  ! Fluxes on y-face
                  i=ii-1; j=jj-1; k=kk-1
                  FY(i,j,k)=sum(this%itpv_y(:,i,j,k)*this%rhoV(i,j:j+1,k))*sum(this%itpv_y(:,i,j,k)*(this%Vhat(i,j:j+1,k)-this%Vhatold(i,j:j+1,k))) 
                  ! Fluxes on z-face
                  i=ii; j=jj; k=kk
                  FZ(i,j,k)=sum(this%itpw_y(:,i,j,k)*this%rhoW(i,j-1:j,k))*sum(this%itpv_z(:,i,j,k)*(this%Vhat(i,j,k-1:k)-this%Vhatold(i,j,k-1:k))) 
               end do
            end do
         end do
         ! Time derivative of rhoV
         do k=this%cfg%kmin_,this%cfg%kmax_
            do j=this%cfg%jmin_,this%cfg%jmax_
               do i=this%cfg%imin_,this%cfg%imax_
                  Vtmp(i,j,k)=sum(this%divv_x(:,i,j,k)*FX(i:i+1,j,k))+&
                  &           sum(this%divv_y(:,i,j,k)*FY(i,j-1:j,k))+&
                  &           sum(this%divv_z(:,i,j,k)*FZ(i,j,k:k+1))
               end do
            end do
         ...
         do kk=this%cfg%kmin_,this%cfg%kmax_+1
            do jj=this%cfg%jmin_,this%cfg%jmax_+1
               do ii=this%cfg%imin_,this%cfg%imax_+1
                  ! Fluxes on x-face
                  i=ii; j=jj; k=kk
                  FX(i,j,k)=sum(this%itpu_z(:,i,j,k)*this%rhoU(i,j,k-1:k))*sum(this%itpw_x(:,i,j,k)*(this%What(i-1:i,j,k)-this%Whatold(i-1:i,j,k))) 
                  ! Fluxes on y-face
                  i=ii; j=jj; k=kk
                  FY(i,j,k)=sum(this%itpv_z(:,i,j,k)*this%rhoV(i,j,k-1:k))*sum(this%itpw_y(:,i,j,k)*(this%What(i,j-1:j,k)-this%Whatold(i,j-1:j,k))) 
                  ! Fluxes on z-face
                  i=ii-1; j=jj-1; k=kk-1
                  FZ(i,j,k)=sum(this%itpw_z(:,i,j,k)*this%rhoW(i,j,k:k+1))*sum(this%itpw_z(:,i,j,k)*(this%What(i,j,k:k+1)-this%Whatold(i,j,k:k+1))) 
               end do
            end do
         end do
         ! Time derivative of rhoW
         do k=this%cfg%kmin_,this%cfg%kmax_
            do j=this%cfg%jmin_,this%cfg%jmax_
               do i=this%cfg%imin_,this%cfg%imax_
                  Wtmp(i,j,k)=sum(this%divw_x(:,i,j,k)*FX(i:i+1,j,k))+&
                  &           sum(this%divw_y(:,i,j,k)*FY(i,j:j+1,k))+&
                  &           sum(this%divw_z(:,i,j,k)*FZ(i,j,k-1:k))
               end do
            end do
         end do
         call this%cfg%sync(Wtmp)
         do k=this%cfg%kmin_,this%cfg%kmax_
            do j=this%cfg%jmin_,this%cfg%jmax_
               do i=this%cfg%imin_,this%cfg%imax_
                  stmp(i,j,k)=sum(this%itpu_x(:,i,j,k)*Utmp(i:i+1,j,k)*this%Uhat(i:i+1,j,k))+&
                  &           sum(this%itpv_y(:,i,j,k)*Vtmp(i,j:j+1,k)*this%Vhat(i,j:j+1,k))+&
                  &           sum(this%itpw_z(:,i,j,k)*Wtmp(i,j,k:k+1)*this%What(i,j,k:k+1))
               end do
            end do
         end do
         call this%cfg%integrate_without_VF(stmp,integral=this%convect_err)
      end block calculate_convecterror

      ! Get P*div(Uhat)
      calculate_PdivU : block
         call this%get_div()
         stmp = this%div*this%P
         call this%cfg%integrate_without_VF(stmp,integral=this%PdivU)
      end block calculate_PdivU

      ! Get U^2*continuity
      calculate_uucont: block
         stmp=0.0_WP
         do k=this%cfg%kmin_,this%cfg%kmax_
            do j=this%cfg%jmin_,this%cfg%jmax_
               do i=this%cfg%imin_,this%cfg%imax_
                  stmp(i,j,k)=(this%rho(i,j,k)-this%rhoold(i,j,k))/dt&
                  &          +sum(this%divu_x(:,i,j,k)*this%rhoU(i:i+1,j,k))&
                  &          +sum(this%divv_y(:,i,j,k)*this%rhoV(i,j:j+1,k))&
                  &          +sum(this%divw_z(:,i,j,k)*this%rhoW(i,j,k:k+1))        
               end do
            end do
         end do
         call this%cfg%sync(stmp)
         do k=this%cfg%kmino_  ,this%cfg%kmaxo_
            do j=this%cfg%jmino_  ,this%cfg%jmaxo_
               do i=this%cfg%imino_+1,this%cfg%imaxo_
                  Utmp(i,j,k)=sum(this%itpr_x(:,i,j,k)*stmp(i-1:i,j,k))
               end do
            end do
         end do
         do k=this%cfg%kmino_  ,this%cfg%kmaxo_
            do j=this%cfg%jmino_+1,this%cfg%jmaxo_
               do i=this%cfg%imino_  ,this%cfg%imaxo_
                  Vtmp(i,j,k)=sum(this%itpr_y(:,i,j,k)*stmp(i,j-1:j,k))
               end do
            end do
         end do
         do k=this%cfg%kmino_+1,this%cfg%kmaxo_
            do j=this%cfg%jmino_  ,this%cfg%jmaxo_
               do i=this%cfg%imino_  ,this%cfg%imaxo_
                  Wtmp(i,j,k)=sum(this%itpr_z(:,i,j,k)*stmp(i,j,k-1:k))
               end do
            end do
         end do
         call this%cfg%sync(Utmp)
         ...
         do k=this%cfg%kmin_,this%cfg%kmax_
            do j=this%cfg%jmin_,this%cfg%jmax_
               do i=this%cfg%imin_,this%cfg%imax_
                  stmp(i,j,k)=sum(this%itpu_x(:,i,j,k)*Utmp(i:i+1,j,k)*this%Uhat(i:i+1,j,k)*this%Uhat(i:i+1,j,k))+&
                  &           sum(this%itpv_y(:,i,j,k)*Vtmp(i,j:j+1,k)*this%Vhat(i,j:j+1,k)*this%Vhat(i,j:j+1,k))+&
                  &           sum(this%itpw_z(:,i,j,k)*Wtmp(i,j,k:k+1)*this%What(i,j,k:k+1)*this%What(i,j,k:k+1))
               end do
            end do
         end do
         stmp=stmp*0.5_WP
   ...
   subroutine get_KE_monitor_implicit(this,dt,resUold,resVold,resWold)
      implicit none
      class(tpns), intent(inout) :: this
      real(WP), intent(in) :: dt
      real(WP), dimension(:,:,:), allocatable:: Utmp !U direction temp field for calculating properties 
      ...
      calculate_convecterror: block
         ...
         Vtmp=0.0_WP
         Wtmp=0.0_WP
         resUold=this%Uhatold+this%rho_U/(this%rho_U+this%rho_Uold)*resUold/(this%rho_U**2)
         resVold=this%Vhatold+this%rho_V/(this%rho_V+this%rho_Vold)*resVold/(this%rho_V**2)
         resWold=this%Whatold+this%rho_W/(this%rho_W+this%rho_Wold)*resWold/(this%rho_W**2)
         ! Flux of rhoU
         do kk=this%cfg%kmin_,this%cfg%kmax_+1
            do jj=this%cfg%jmin_,this%cfg%jmax_+1
               do ii=this%cfg%imin_,this%cfg%imax_+1
                  ! Fluxes on x-face
                  i=ii-1; j=jj-1; k=kk-1
                  FX(i,j,k)=sum(this%itpu_x(:,i,j,k)*this%rhoU(i:i+1,j,k))*sum(this%itpu_x(:,i,j,k)*(this%Uhat(i:i+1,j,k)-resUold(i:i+1,j,k)))
                  ! Fluxes on y-face
                  i=ii; j=jj; k=kk
                  FY(i,j,k)=sum(this%itpv_x(:,i,j,k)*this%rhoV(i-1:i,j,k))*sum(this%itpu_y(:,i,j,k)*(this%Uhat(i,j-1:j,k)-resUold(i,j-1:j,k))) 
                  ! Fluxes on z-face
                  i=ii; j=jj; k=kk
                  FZ(i,j,k)=sum(this%itpw_x(:,i,j,k)*this%rhoW(i-1:i,j,k))*sum(this%itpu_z(:,i,j,k)*(this%Uhat(i,j,k-1:k)-resUold(i,j,k-1:k))) 
               end do
            end do
         end do
         ! Time derivative of rhoU
         ...
         do kk=this%cfg%kmin_,this%cfg%kmax_+1
            do jj=this%cfg%jmin_,this%cfg%jmax_+1
               do ii=this%cfg%imin_,this%cfg%imax_+1
                  ! Fluxes on x-face
                  i=ii; j=jj; k=kk
                  FX(i,j,k)=sum(this%itpu_y(:,i,j,k)*this%rhoU(i,j-1:j,k))*sum(this%itpv_x(:,i,j,k)*(this%Vhat(i-1:i,j,k)-resVold(i-1:i,j,k))) 
                  ! Fluxes on y-face
                  i=ii-1; j=jj-1; k=kk-1
                  FY(i,j,k)=sum(this%itpv_y(:,i,j,k)*this%rhoV(i,j:j+1,k))*sum(this%itpv_y(:,i,j,k)*(this%Vhat(i,j:j+1,k)-resVold(i,j:j+1,k))) 
                  ! Fluxes on z-face
                  i=ii; j=jj; k=kk
                  FZ(i,j,k)=sum(this%itpw_y(:,i,j,k)*this%rhoW(i,j-1:j,k))*sum(this%itpv_z(:,i,j,k)*(this%Vhat(i,j,k-1:k)-resVold(i,j,k-1:k))) 
               end do
            end do
         end do
         ! Time derivative of rhoV
         ...
         do kk=this%cfg%kmin_,this%cfg%kmax_+1
            do jj=this%cfg%jmin_,this%cfg%jmax_+1
               do ii=this%cfg%imin_,this%cfg%imax_+1
                  ! Fluxes on x-face
                  i=ii; j=jj; k=kk
                  FX(i,j,k)=sum(this%itpu_z(:,i,j,k)*this%rhoU(i,j,k-1:k))*sum(this%itpw_x(:,i,j,k)*(this%What(i-1:i,j,k)-resWold(i-1:i,j,k))) 
                  ! Fluxes on y-face
                  i=ii; j=jj; k=kk
                  FY(i,j,k)=sum(this%itpv_z(:,i,j,k)*this%rhoV(i,j,k-1:k))*sum(this%itpw_y(:,i,j,k)*(this%What(i,j-1:j,k)-resWold(i,j-1:j,k))) 
                  ! Fluxes on z-face
                  i=ii-1; j=jj-1; k=kk-1
                  FZ(i,j,k)=sum(this%itpw_z(:,i,j,k)*this%rhoW(i,j,k:k+1))*sum(this%itpw_z(:,i,j,k)*(this%What(i,j,k:k+1)-resWold(i,j,k:k+1))) 
               end do
            end do
         end do
         ! Time derivative of rhoW
      ...
      deallocate(Utmp,Vtmp,Wtmp,stmp,FX,FY,FZ)
   end subroutine get_KE_monitor_implicit
   ...
   subroutine addsrc_gravity(this,resU,resV,resW)
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               if (this%umask(i,j,k).eq.0) resU(i,j,k)=resU(i,j,k)+(this%rho_U(i,j,k)**2)*this%gravity(1)
               if (this%vmask(i,j,k).eq.0) resV(i,j,k)=resV(i,j,k)+(this%rho_V(i,j,k)**2)*this%gravity(2)
               if (this%wmask(i,j,k).eq.0) resW(i,j,k)=resW(i,j,k)+(this%rho_W(i,j,k)**2)*this%gravity(3)
            end do
         end do
      end do
