...
contains
   ...
   subroutine simulation_init
      ...
      allocate_work_arrays: block
         ...
         allocate(Uihat(cfg%imino_:cfg%imaxo_,cfg%jmino_:cfg%jmaxo_,cfg%kmino_:cfg%kmaxo_))
         allocate(Vihat(cfg%imino_:cfg%imaxo_,cfg%jmino_:cfg%jmaxo_,cfg%kmino_:cfg%kmaxo_))
         allocate(Wihat(cfg%imino_:cfg%imaxo_,cfg%jmino_:cfg%jmaxo_,cfg%kmino_:cfg%kmaxo_))
      ...
      create_ensight: block
         ...
         call ens_out%add_vector('velocityhat',Uihat,Vihat,Wihat)
   ...
   subroutine simulation_run
      ...
      do while (.not.time%done())
         ...
         fs%rhoold=fs%rho
         ! Remember old velocity and face density
         ...
         do while (time%it.le.time%itmax)
            ...
            if (time%it.eq.time%itmax) then   
               call vf%advance(dt=time%dt,U=fs%Uhat,V=fs%Vhat,W=fs%What)
            else
               call vf%advance_tmp(dt=time%dt,U=fs%Uhat,V=fs%Vhat,W=fs%What)
            end if
            ! Update sqrt face density, momentum vector
            call fs%update_density(vf=vf)
            ...
            resU=-fs%U*(fs%rho_U**2)+((fs%rho_Uold**2)*fs%Uold+time%dt*resU)
            resV=-fs%V*(fs%rho_V**2)+((fs%rho_Vold**2)*fs%Vold+time%dt*resV)
            resW=-fs%W*(fs%rho_W**2)+((fs%rho_Wold**2)*fs%Wold+time%dt*resW)

            ! Form implicit residuals
            ! call fs%solve_implicit(time%dt,resU,resV,resW)

            ! Only need this for outputing error terms in running implicit
            ! resUold=resU
            ! resVold=resV
            ! resWold=resW
            ! Apply these residuals and synchronize the velocity field
            fs%U=fs%U+resU/(fs%rho_U**2); call cfg%sync(fs%U)
            fs%V=fs%V+resV/(fs%rho_V**2); call cfg%sync(fs%V)
            fs%W=fs%W+resW/(fs%rho_W**2); call cfg%sync(fs%W)

            ! Build up uhat
            fs%Uhat=(fs%rho_U*fs%U+fs%rho_Uold*fs%Uold)/(fs%rho_U+fs%rho_Uold)
            fs%Vhat=(fs%rho_V*fs%V+fs%rho_Vold*fs%Vold)/(fs%rho_V+fs%rho_Vold)
            fs%What=(fs%rho_W*fs%W+fs%rho_Wold*fs%Wold)/(fs%rho_W+fs%rho_Wold)
         ...
         call fs%get_KE_monitor(dt=time%dt)
         ! call fs%get_KE_monitor_implicit(dt=time%dt,resUold,resVold,resWold)
