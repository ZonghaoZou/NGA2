...
contains
   ...
   subroutine simulation_init(this)
      ...
      create_smesh: block
         ...
         this%smesh%varname(2)='curv2p'
         this%smesh%varname(3)='ccl_film'
         this%smesh%varname(4)='ccl_lig'
               ...
               do i=this%vf%cfg%imin_,this%vf%cfg%imax_
                  ...
                  do nplane=1,getNumberOfPlanes(this%vf%liquid_gas_interface(i,j,k))
                     if (getNumberOfVertices(this%vf%interface_polygon(nplane,i,j,k)).gt.0) then
                        ...
                        this%smesh%var(3,np)=real(this%bu%ccl_film%id(i,j,k),WP)
                        this%smesh%var(4,np)=real(this%bu%ccl_lig%id(i,j,k),WP)
   ...
   subroutine step(this,lp)
      ...
      attempt_transfer : block
         ...
         if (this%bu%use_film_transfer) call this%bu%transfer_films(lp)
         call this%tfilm%stop()
         call this%tlig%start()
         if (this%bu%use_lig_transfer) call this%bu%transfer_ligs(lp)
         call this%tlig%stop()
         call this%tdrop%start()
         if (this%bu%use_drop_transfer) call this%bu%transfer_drops(lp)
