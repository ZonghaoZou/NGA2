...
contains
   ...
   subroutine add_surface_tension_jump_thin(this,dt,div,vf,SFX,SFY,SFZ)
      use messager,  only: die
      use vfs_class, only: vfs
      ...
      real(WP), intent(inout) :: dt     !< Timestep size over which to advance
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: div  !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: SFX  !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: SFY  !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: SFZ  !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      class(vfs), intent(inout) :: vf
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_+1
         do j=this%cfg%jmin_,this%cfg%jmax_+1
            do i=this%cfg%imin_,this%cfg%imax_+1
               ...
               ! if (mycurv.gt.0.0_WP) mycurv = 1.0_WP/0.005_WP
               ! if (mycurv.lt.0.0_WP) mycurv = -1.0_WP/0.005_WP
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               &                        +sum(this%divp_y(:,i,j,k)*this%DPjy(i,j:j+1,k)/this%rho_V(i,j:j+1,k))&
               &                        +sum(this%divp_z(:,i,j,k)*this%DPjz(i,j,k:k+1)/this%rho_W(i,j,k:k+1)))
               if (this%Pjx(i,j,k).gt.0.0_WP) AHsum1 = AHsum1 + this%Pjx(i,j,k)
               if (this%Pjx(i,j,k).lt.0.0_WP) AHsum2 = AHsum2 + this%Pjx(i,j,k)
            end do
         end do
      end do
      SFX = this%Pjx
      SFY = this%Pjy
      SFZ = this%Pjz
      print *, AHsum1, AHsum2, AHsum1+AHsum2
