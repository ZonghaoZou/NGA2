         ...
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               do n=1,getNumberOfPlanes(this%liquid_gas_interface(i,j,k))
                  ...
                  ! norm=normalize([this%cfg%xm(i),this%cfg%ym(j),this%cfg%zm(k)])
                  if (dot_product(plane(1:3),norm).gt. 0.0_WP) mycurv(n) = 1.0_WP/0.005_WP
                  if (dot_product(plane(1:3),norm).lt. 0.0_WP) mycurv(n) = -1.0_WP/0.005_WP
                  ! if (mycurv(n).gt.0.0_WP) 
                  ! if (mycurv(n).lt.0.0_WP) mycurv(n) = -1.0_WP/0.005_WP
               end do
