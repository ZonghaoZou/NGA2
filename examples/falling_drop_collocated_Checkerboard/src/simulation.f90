...
contains
   ...
   subroutine simulation_init
      ...
      create_flow_solver: block
         ...
         ps=hypre_str(cfg=cfg,name='Pressure',method=pcg,nst=13)
         ps%maxlevel=24
      ...
      initialize_velocity: block
         ...
         integer :: i,j,k
         ! Initialize density
         fs%rho=fs%rho_l*vf%VF+fs%rho_g*(1.0_WP-vf%VF); call fs%update_faceRHO(rho=fs%rho)
         if (falling) then
            ! Initialize velocity
   ...
   subroutine simulation_run
      ...
      do while (.not.time%done())
         ...
         do while (time%it.le.time%itmax)
            ...
            ! else
            !    call vf%advance_tmp(dt=time%dt,U=fs%U,V=fs%V,W=fs%W,rho_l=fs%rho_l,rho_g=fs%rho_g)
            ! end if
            ! ! Update face density and momentum vector
            ...
            call fs%psolv%solve()
            call fs%shift_p(fs%psolv%sol)
            call fs%get_pcorrection()
            ! Corrector step
            call fs%get_pgrad_collocated(fs%P,resU,resV,resW)
            ! call fs%get_pgrad_collocated(fs%psolv%sol,resU,resV,resW)
            ! fs%P=fs%psolv%sol
