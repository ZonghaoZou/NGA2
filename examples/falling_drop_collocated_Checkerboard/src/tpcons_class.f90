...
module tpcons_class
   ...
   type :: tpcons
      ...
      ! Pressure correction (Shashank)
      real(WP), dimension(:,:,:,:), allocatable :: PC_coeff
      real(WP), dimension(8,8)                  :: PINV
   ...
   contains
      ...
      procedure :: get_pgrad_collocated                   !< Calculate pressure gradient for collocated grid
      procedure :: get_pcorrection                        !< Calculate the pressure correction based on Shashank's work
   end type tpcons
...
contains
   ...
   subroutine initialize(this,cfg,name)
      ...
      allocate(this%PC_coeff(1:8,this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%PC_coeff=0.0_WP
   ...
   subroutine init_metrics(this)
      ...
      ! Create coefficients for P hat
      do k=this%cfg%kmino_,this%cfg%kmaxo_
         do j=this%cfg%jmino_,this%cfg%jmaxo_
            do i=this%cfg%imino_,this%cfg%imaxo_
               this%PC_coeff(1,i,j,k)=1
               this%PC_coeff(2,i,j,k)=merge(1, -1, mod(i,2)    ==0)
               this%PC_coeff(3,i,j,k)=merge(1, -1, mod(j,2)    ==0)
               this%PC_coeff(4,i,j,k)=merge(1, -1, mod(i+j,2)  ==0)
               this%PC_coeff(5,i,j,k)=merge(1, -1, mod(k,2)    ==0)
               this%PC_coeff(6,i,j,k)=merge(1, -1, mod(i+k,2)  ==0)
               this%PC_coeff(7,i,j,k)=merge(1, -1, mod(j+k,2)  ==0)
               this%PC_coeff(8,i,j,k)=merge(1, -1, mod(i+j+k,2)==0)
            end do
         end do
      end do
      ...
      invert_Pcoeff: block 
         ...
         call dgetrf(8,8,this%PINV,8,ipiv,info)
         lwork = -1
         allocate(work(1))
   ...
   subroutine setup(this,pressure_solver,implicit_solver)
      ...
      this%psolv%stc(8, :)=[+2, 0, 0]
      this%psolv%stc(9, :)=[-2, 0, 0]
      this%psolv%stc(10,:)=[ 0,+2, 0]
      this%psolv%stc(11,:)=[ 0,-2, 0]
      this%psolv%stc(12,:)=[ 0, 0,+2]
      this%psolv%stc(13,:)=[ 0, 0,-2]
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ! Set Laplacian
               this%psolv%opr(1 ,i,j,k)=sum(this%divp_x(:,i,j,k)*[this%itpr_x(0,i,j,k)*(this%divp_x(1,i-1,j,k)*this%itpr_x(-1,i,j,k)  &
                                       &+sum(this%divp_x(:,i,j,k)*[this%itpr_x(0,i,j,k),this%itpr_x(-1,i+1,j,k)])),                            &
                                       & this%itpr_x(-1,i+1,j,k)*(this%divp_x(0,i+1,j,k)*this%itpr_x(0,i+1,j,k)                       &
                                       &+sum(this%divp_x(:,i,j,k)*[this%itpr_x(0,i,j,k),this%itpr_x(-1,i+1,j,k)]) ) ])                         &
                                       &+sum(this%divp_y(:,i,j,k)*[this%itpr_y(0,i,j,k)*(this%divp_y(1,i,j-1,k)*this%itpr_y(-1,i,j,k) &
                                       &+sum(this%divp_y(:,i,j,k)*[this%itpr_y(0,i,j,k),this%itpr_y(-1,i,j+1,k)])),                            &
                                       & this%itpr_y(-1,i,j+1,k)*(this%divp_y(0,i,j+1,k)*this%itpr_y(0,i,j+1,k)                       &
                                       &+sum(this%divp_y(:,i,j,k)*[this%itpr_y(0,i,j,k),this%itpr_y(-1,i,j+1,k)]) ) ])                         &
                                       &+sum(this%divp_z(:,i,j,k)*[this%itpr_z(0,i,j,k)*(this%divp_z(1,i,j,k-1)*this%itpr_z(-1,i,j,k) &
                                       &+sum(this%divp_z(:,i,j,k)*[this%itpr_z(0,i,j,k),this%itpr_z(-1,i,j,k+1)])),                            &
                                       & this%itpr_z(-1,i,j,k+1)*(this%divp_z(0,i,j,k+1)*this%itpr_z(0,i,j,k+1)                       &
                                       &+sum(this%divp_z(:,i,j,k)*[this%itpr_z(0,i,j,k),this%itpr_z(-1,i,j,k+1)]) ) ])                         
               this%psolv%opr(2 ,i,j,k)=this%divp_x(1,i,j,k)*this%itpr_x(0,i+1,j,k)*                                                  &
                                       &(sum(this%divp_x(:,i,j,k)*[this%itpr_x(0,i,j,k),this%itpr_x(-1,i+1,j,k)])                     &
                                       &+sum(this%divp_x(:,i+1,j,k)*[this%itpr_x(0,i+1,j,k),this%itpr_x(-1,i+2,j,k)]))               
               this%psolv%opr(3 ,i,j,k)=this%divp_x(0,i,j,k)*this%itpr_x(-1,i,j,k)*                                                   &
                                       &(sum(this%divp_x(:,i-1,j,k)*[this%itpr_x(0,i-1,j,k),this%itpr_x(-1,i,j,k)])                   &
                                       &+sum(this%divp_x(:,i,j,k)*[this%itpr_x(0,i,j,k),this%itpr_x(-1,i+1,j,k)]) ) 
               this%psolv%opr(4 ,i,j,k)=this%divp_y(1,i,j,k)*this%itpr_y(0,i,j+1,k)*                                                  &
                                        &(sum(this%divp_y(:,i,j,k)*[this%itpr_y(0,i,j,k),this%itpr_y(-1,i,j+1,k)])                    &
                                        &+sum(this%divp_y(:,i,j+1,k)*[this%itpr_y(0,i,j+1,k),this%itpr_y(-1,i,j+2,k)]))               
               this%psolv%opr(5 ,i,j,k)=this%divp_y(0,i,j,k)*this%itpr_y(-1,i,j,k)*                                                   &
                                       &(sum(this%divp_y(:,i,j-1,k)*[this%itpr_y(0,i,j-1,k),this%itpr_y(-1,i,j,k)])                   &
                                       &+sum(this%divp_y(:,i,j,k)*[this%itpr_y(0,i,j,k),this%itpr_y(-1,i,j+1,k)]) ) 
               this%psolv%opr(6 ,i,j,k)=this%divp_z(1,i,j,k)*this%itpr_z(0,i,j,k+1)*                                                  &
                                       &(sum(this%divp_z(:,i,j,k)*[this%itpr_z(0,i,j,k),this%itpr_z(-1,i,j,k+1)])                     &
                                       &+sum(this%divp_z(:,i,j,k+1)*[this%itpr_z(0,i,j,k+1),this%itpr_z(-1,i,j,k+2)]))               
               this%psolv%opr(7 ,i,j,k)=this%divp_z(0,i,j,k)*this%itpr_z(-1,i,j,k)*                                                   &
                                       &(sum(this%divp_z(:,i,j,k-1)*[this%itpr_z(0,i,j,k-1),this%itpr_z(-1,i,j,k)])                   &
                                       &+sum(this%divp_z(:,i,j,k)*[this%itpr_z(0,i,j,k),this%itpr_z(-1,i,j,k+1)]) ) 
               this%psolv%opr(8 ,i,j,k)=this%divp_x(1,i,j,k)*this%itpr_x(0,i+1,j,k)*this%divp_x(1,i+1,j,k)*this%itpr_x(0,i+2,j,k)
               this%psolv%opr(9 ,i,j,k)=this%divp_x(0,i,j,k)*this%itpr_x(-1,i,j,k)*this%divp_x(0,i-1,j,k)*this%itpr_x(-1,i-1,j,k)
               this%psolv%opr(10,i,j,k)=this%divp_y(1,i,j,k)*this%itpr_y(0,i,j+1,k)*this%divp_y(1,i,j+1,k)*this%itpr_y(0,i,j+2,k)
               this%psolv%opr(11,i,j,k)=this%divp_y(0,i,j,k)*this%itpr_y(-1,i,j,k)*this%divp_y(0,i,j-1,k)*this%itpr_y(-1,i,j-1,k)
               this%psolv%opr(12,i,j,k)=this%divp_z(1,i,j,k)*this%itpr_z(0,i,j,k+1)*this%divp_z(1,i,j,k+1)*this%itpr_z(0,i,j,k+2)
               this%psolv%opr(13,i,j,k)=this%divp_z(0,i,j,k)*this%itpr_z(-1,i,j,k)*this%divp_z(0,i,j,k-1)*this%itpr_z(-1,i,j,k-1)
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_+1
         do j=this%cfg%jmin_,this%cfg%jmax_+1
            do i=this%cfg%imin_,this%cfg%imax_+1
               ...
               ! &         -sum(this%itpr_x(:,i,j,k)*this%P(i-1:i,j,k))
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               &              sum(this%divp_z(:,i,j,k)*FZ(i,j,k:k+1))!+this%Pjx(i,j,k)
            end do
         end do
      end do
      ! Sync it
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_+1
         do j=this%cfg%jmin_,this%cfg%jmax_+1
            do i=this%cfg%imin_,this%cfg%imax_+1
               ...
               ! &         -sum(this%itpr_y(:,i,j,k)*this%P(i,j-1:j,k)) 
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               &              sum(this%divp_y(:,i,j,k)*FY(i,j:j+1,k))+&
               &              sum(this%divp_z(:,i,j,k)*FZ(i,j,k:k+1))!+this%Pjy(i,j,k)
            end do
         end do
      end do
      ! Sync it
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_+1
         do j=this%cfg%jmin_,this%cfg%jmax_+1
            do i=this%cfg%imin_,this%cfg%imax_+1
               ...
               ! &         -sum(this%itpr_z(:,i,j,k)*this%P(i,j,k-1:k)) 
            end do 
         end do 
      end do
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               &              sum(this%divp_y(:,i,j,k)*FY(i,j:j+1,k))+&
               &              sum(this%divp_z(:,i,j,k)*FZ(i,j,k:k+1))!+this%Pjz(i,j,k)
            end do
         end do
      end do
      ! Sync it
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               this%psolv%opr(1 ,i,j,k)=sum(this%divp_x(:,i,j,k)*[this%itpr_x(0,i,j,k)*(this%divp_x(1,i-1,j,k)*this%itpr_x(-1,i,j,k)/this%rho(i-1,j,k)  &
                                       &+sum(this%divp_x(:,i,j,k)*[this%itpr_x(0,i,j,k),this%itpr_x(-1,i+1,j,k)])/this%rho(i,j,k)),                              &
                                       & this%itpr_x(-1,i+1,j,k)*(this%divp_x(0,i+1,j,k)*this%itpr_x(0,i+1,j,k)/this%rho(i+1,j,k)                       &
                                       &+sum(this%divp_x(:,i,j,k)*[this%itpr_x(0,i,j,k),this%itpr_x(-1,i+1,j,k)])/this%rho(i,j,k)) ])                            &
                                       &+sum(this%divp_y(:,i,j,k)*[this%itpr_y(0,i,j,k)*(this%divp_y(1,i,j-1,k)*this%itpr_y(-1,i,j,k)/this%rho(i,j-1,k) &
                                       &+sum(this%divp_y(:,i,j,k)*[this%itpr_y(0,i,j,k),this%itpr_y(-1,i,j+1,k)])/this%rho(i,j,k)),                              &
                                       & this%itpr_y(-1,i,j+1,k)*(this%divp_y(0,i,j+1,k)*this%itpr_y(0,i,j+1,k)/this%rho(i,j+1,k)                       &
                                       &+sum(this%divp_y(:,i,j,k)*[this%itpr_y(0,i,j,k),this%itpr_y(-1,i,j+1,k)])/this%rho(i,j,k)) ])                            &
                                       &+sum(this%divp_z(:,i,j,k)*[this%itpr_z(0,i,j,k)*(this%divp_z(1,i,j,k-1)*this%itpr_z(-1,i,j,k)/this%rho(i,j,k-1) &
                                       &+sum(this%divp_z(:,i,j,k)*[this%itpr_z(0,i,j,k),this%itpr_z(-1,i,j,k+1)])/this%rho(i,j,k)),                              &
                                       & this%itpr_z(-1,i,j,k+1)*(this%divp_z(0,i,j,k+1)*this%itpr_z(0,i,j,k+1)/this%rho(i,j,k+1)                       &
                                       &+sum(this%divp_z(:,i,j,k)*[this%itpr_z(0,i,j,k),this%itpr_z(-1,i,j,k+1)])/this%rho(i,j,k)) ])                         
               this%psolv%opr(2 ,i,j,k)=this%divp_x(1,i,j,k)*this%itpr_x(0,i+1,j,k)*                                                                    &
                                       &(sum(this%divp_x(:,i,j,k)*[this%itpr_x(0,i,j,k),this%itpr_x(-1,i+1,j,k)])/this%rho(i,j,k)                       &
                                       &+sum(this%divp_x(:,i+1,j,k)*[this%itpr_x(0,i+1,j,k),this%itpr_x(-1,i+2,j,k)])/this%rho(i+1,j,k))               
               this%psolv%opr(3 ,i,j,k)=this%divp_x(0,i,j,k)*this%itpr_x(-1,i,j,k)*                                                                     &
                                       &(sum(this%divp_x(:,i-1,j,k)*[this%itpr_x(0,i-1,j,k),this%itpr_x(-1,i,j,k)])/this%rho(i-1,j,k)                   &
                                       &+sum(this%divp_x(:,i,j,k)*[this%itpr_x(0,i,j,k),this%itpr_x(-1,i+1,j,k)])/this%rho(i,j,k)) 
               this%psolv%opr(4 ,i,j,k)=this%divp_y(1,i,j,k)*this%itpr_y(0,i,j+1,k)*                                                                    &
                                        &(sum(this%divp_y(:,i,j,k)*[this%itpr_y(0,i,j,k),this%itpr_y(-1,i,j+1,k)])/this%rho(i,j,k)                      &
                                        &+sum(this%divp_y(:,i,j+1,k)*[this%itpr_y(0,i,j+1,k),this%itpr_y(-1,i,j+2,k)])/this%rho(i,j+1,k))               
               this%psolv%opr(5 ,i,j,k)=this%divp_y(0,i,j,k)*this%itpr_y(-1,i,j,k)*                                                                     &
                                       &(sum(this%divp_y(:,i,j-1,k)*[this%itpr_y(0,i,j-1,k),this%itpr_y(-1,i,j,k)])/this%rho(i,j-1,k)                   &
                                       &+sum(this%divp_y(:,i,j,k)*[this%itpr_y(0,i,j,k),this%itpr_y(-1,i,j+1,k)])/this%rho(i,j,k)) 
               this%psolv%opr(6 ,i,j,k)=this%divp_z(1,i,j,k)*this%itpr_z(0,i,j,k+1)*                                                                    &
                                       &(sum(this%divp_z(:,i,j,k)*[this%itpr_z(0,i,j,k),this%itpr_z(-1,i,j,k+1)])/this%rho(i,j,k)                       &
                                       &+sum(this%divp_z(:,i,j,k+1)*[this%itpr_z(0,i,j,k+1),this%itpr_z(-1,i,j,k+2)])/this%rho(i,j,k+1))               
               this%psolv%opr(7 ,i,j,k)=this%divp_z(0,i,j,k)*this%itpr_z(-1,i,j,k)*                                                                     &
                                       &(sum(this%divp_z(:,i,j,k-1)*[this%itpr_z(0,i,j,k-1),this%itpr_z(-1,i,j,k)])/this%rho(i,j,k-1)                   &
                                       &+sum(this%divp_z(:,i,j,k)*[this%itpr_z(0,i,j,k),this%itpr_z(-1,i,j,k+1)])/this%rho(i,j,k)) 
               this%psolv%opr(8 ,i,j,k)=this%divp_x(1,i,j,k)*this%itpr_x(0,i+1,j,k)*this%divp_x(1,i+1,j,k)*this%itpr_x(0,i+2,j,k)/this%rho(i+1,j,k)
               this%psolv%opr(9 ,i,j,k)=this%divp_x(0,i,j,k)*this%itpr_x(-1,i,j,k)*this%divp_x(0,i-1,j,k)*this%itpr_x(-1,i-1,j,k)/this%rho(i-1,j,k)
               this%psolv%opr(10,i,j,k)=this%divp_y(1,i,j,k)*this%itpr_y(0,i,j+1,k)*this%divp_y(1,i,j+1,k)*this%itpr_y(0,i,j+2,k)/this%rho(i,j+1,k)
               this%psolv%opr(11,i,j,k)=this%divp_y(0,i,j,k)*this%itpr_y(-1,i,j,k)*this%divp_y(0,i,j-1,k)*this%itpr_y(-1,i,j-1,k)/this%rho(i,j-1,k)
               this%psolv%opr(12,i,j,k)=this%divp_z(1,i,j,k)*this%itpr_z(0,i,j,k+1)*this%divp_z(1,i,j,k+1)*this%itpr_z(0,i,j,k+2)/this%rho(i,j,k+1)
               this%psolv%opr(13,i,j,k)=this%divp_z(0,i,j,k)*this%itpr_z(-1,i,j,k)*this%divp_z(0,i,j,k-1)*this%itpr_z(-1,i,j,k-1)/this%rho(i,j,k-1)
   ...
   !> Calculate the pressure gradient based on P
   subroutine get_pgrad_collocated(this,P,Pgradx,Pgrady,Pgradz)
      implicit none
      class(tpcons), intent(inout) :: this
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(in)  :: P      !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(out) :: Pgradx !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      ...
      call this%cfg%sync(Pgrady)
      call this%cfg%sync(Pgradz)
   end subroutine get_pgrad_collocated
   
   subroutine get_pcorrection(this)
      implicit none
      class(tpcons), intent(inout) :: this
      ...
      do ii=1,8
         do k=this%cfg%kmin_,this%cfg%kmax_
            do j=this%cfg%jmin_,this%cfg%jmax_
               do i=this%cfg%imin_,this%cfg%imax_
                  b(ii)=b(ii)+this%PC_coeff(ii,i,j,k)*this%psolv%sol(i,j,k)*this%cfg%vol(i,j,k)
               end do 
            end do 
         end do 
      end do
      ...
      call this%cfg%sync(this%P)
   end subroutine get_pcorrection
   ...
   !> Compute U/V/Wf from U/V/W field
   subroutine update_faceU(this)
      implicit none
      class(tpcons), intent(inout) :: this
      integer :: i,j,k
      ! Calculate square root of face densities
      do k=this%cfg%kmino_  ,this%cfg%kmaxo_; do j=this%cfg%jmino_  ,this%cfg%jmaxo_; do i=this%cfg%imino_+1,this%cfg%imaxo_
         this%Uf(i,j,k)=sum(this%itpr_x(:,i,j,k)*this%U(i-1:i,j,k))
      end do; end do; end do
      do k=this%cfg%kmino_  ,this%cfg%kmaxo_; do j=this%cfg%jmino_+1,this%cfg%jmaxo_; do i=this%cfg%imino_  ,this%cfg%imaxo_
         this%Vf(i,j,k)=sum(this%itpr_y(:,i,j,k)*this%V(i,j-1:j,k))
      end do; end do; end do
      do k=this%cfg%kmino_+1,this%cfg%kmaxo_; do j=this%cfg%jmino_  ,this%cfg%jmaxo_; do i=this%cfg%imino_  ,this%cfg%imaxo_
         this%Wf(i,j,k)=sum(this%itpr_z(:,i,j,k)*this%W(i,j,k-1:k))
      end do; end do; end do
      ! Handle non-periodic borders
      if (.not.this%cfg%xper.and.this%cfg%iproc.eq.1) this%Uf(this%cfg%imino,:,:)=this%U(this%cfg%imino,:,:)
      if (.not.this%cfg%yper.and.this%cfg%jproc.eq.1) this%Vf(:,this%cfg%jmino,:)=this%V(:,this%cfg%jmino,:)
      if (.not.this%cfg%zper.and.this%cfg%kproc.eq.1) this%Wf(:,:,this%cfg%kmino)=this%W(:,:,this%cfg%kmino)
      ! Synchronize boundaries
